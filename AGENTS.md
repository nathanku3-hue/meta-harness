...
- For repo-owned fresh work, recover active Claims first, then use at most one fresh read-only logical planner boot per exact WorldHead + owner-objective revision to propose the positive-value frontier up to local capacity. Capacity is a ceiling, not a quota; unused slots are correct when no additional positive-value Outcome exists. Planner input is reconstructed from durable truth; planner candidates are disposable semantics and never authority or continuity.
