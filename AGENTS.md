...
- For repo-owned fresh work, recover active Claims first, then use at most one fresh read-only logical planner boot to fill unused initial local capacity. Planner input is reconstructed from durable truth; planner candidates are disposable semantics and never authority or continuity.
