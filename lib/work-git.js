...
const { readCandidateMaterialization } = require("./candidate-materialization-record");
const { protocolRoot, withWorldAuthorityLock } = require("./world-authority");
const { dirtyManifestDigest, readAttemptEntry } = require("./execution-permit");
const { gitExecutableForWorkspace } = require("./git-command");
const { bindOutcomeClaim, readOutcomeClaimSession } = require("./outcome-claim");
...
const { readProductProof } = require("./work-product-proof");
const {
...
const CLAIM_WORK_SESSION_POINTER_SCHEMA = "claim-work-session-pointer/v1";
...
const OWNER_GOAL_INGRESS_SCHEMA = "owner-goal-ingress/v1";
const OWNER_GOAL_INGRESS_FILE = "owner-goal-ingress.json";
const MAX_OWNER_GOAL_INGRESS_BYTES = 128 * 1024;
...
function ownerGoalIngressPath(repositoryPath, options) {
  return path.join(protocolRoot(repositoryPath, options), OWNER_GOAL_INGRESS_FILE);
}

function ownerGoalIngressDigest(content) {
  return `sha256:${crypto.createHash("sha256").update(Buffer.from(content, "utf8")).digest("hex")}`;
}

function validateOwnerGoalIngress(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    fail("MH_OWNER_GOAL_INGRESS", "owner-goal ingress must be an object");
  }
  const actual = Object.keys(value).sort();
  const expected = ["schemaVersion", "content", "contentDigest"].sort();
  if (JSON.stringify(actual) !== JSON.stringify(expected)
      || value.schemaVersion !== OWNER_GOAL_INGRESS_SCHEMA
      || typeof value.content !== "string"
      || value.content.trim() === ""
      || value.content !== value.content.trim()
      || Buffer.byteLength(value.content, "utf8") > MAX_OWNER_GOAL_INGRESS_BYTES
      || value.contentDigest !== ownerGoalIngressDigest(value.content)) {
    fail("MH_OWNER_GOAL_INGRESS", "owner-goal ingress is invalid");
  }
  return Object.freeze({ ...value });
}

function readOwnerGoalIngress(repositoryPath, { optional = true } = {}) {
  const filePath = ownerGoalIngressPath(repositoryPath, { create: false });
  let parsed;
  try {
    parsed = JSON.parse(fs.readFileSync(filePath, "utf8"));
  } catch (error) {
    if (optional && error?.code === "ENOENT") return null;
    fail("MH_OWNER_GOAL_INGRESS", `owner-goal ingress is unreadable: ${error.message}`);
  }
  return validateOwnerGoalIngress(parsed);
}

function withOwnerGoalIngressLock(repositoryPath, operation) {
  ...
  fail("MH_WORLD_AUTHORITY_BUSY", "owner-goal ingress authority remained busy after bounded contention retry");
}

function persistOwnerGoalIngress(repositoryPath, content) {
  if (typeof content !== "string" || content.trim() === "") {
    fail("MH_OWNER_GOAL_INGRESS_VALUE", "owner goal must be a non-empty application-level string");
  }
  const normalized = content.trim();
  if (Buffer.byteLength(normalized, "utf8") > MAX_OWNER_GOAL_INGRESS_BYTES) {
    fail("MH_OWNER_GOAL_INGRESS_VALUE", `owner goal exceeds ${MAX_OWNER_GOAL_INGRESS_BYTES} bytes`);
  }
  const ingress = validateOwnerGoalIngress({
    schemaVersion: OWNER_GOAL_INGRESS_SCHEMA,
    content: normalized,
    contentDigest: ownerGoalIngressDigest(normalized),
  });
  return withOwnerGoalIngressLock(repositoryPath, () => {
    writeJsonAtomic(ownerGoalIngressPath(repositoryPath), ingress);
    return readOwnerGoalIngress(repositoryPath, { optional: false });
  });
}

function clearOwnerGoalIngress(repositoryPath, { expectedContent = null } = {}) {
  return withOwnerGoalIngressLock(repositoryPath, () => {
    const current = readOwnerGoalIngress(repositoryPath, { optional: true });
    if (!current) return null;
    if (expectedContent !== null && current.content !== String(expectedContent).trim()) return current;
    try {
      fs.unlinkSync(ownerGoalIngressPath(repositoryPath));
    } catch (error) {
      if (error?.code !== "ENOENT") {
        fail("MH_OWNER_GOAL_INGRESS_WRITE", `cannot clear owner-goal ingress: ${error.message}`);
      }
    }
    return null;
  });
...
function captureWorkspaceBoundary(workspacePath, allowedPaths = ["."]) {
  const inspected = inspectWorkspace(workspacePath, allowedPaths);
  const indexDiff = String(runGit(workspacePath, ["diff", "--cached", "--binary"]).stdout || "");
  const candidatePaths = dirtyPaths(inspected);
  return {
    head: inspected.head,
    branch: inspected.branch,
    indexDiff,
    dirtyManifestDigest: dirtyManifestDigest({ inspected }),
    treeOid: prospectiveCandidateTree(workspacePath, inspected.head, candidatePaths, os.tmpdir()),
    inspected,
  };
}
...
function workspaceRegistryDirectory(root, options) {
  return registryDirectory(commonGitDirectory(root), options);
}
...
function generationAttemptEntry(root, session, custody, generation, { optional = false } = {}) {
  return readAttemptEntry({
    repositoryPath: root,
    session,
...
function latestPriorCandidateSeal(directory, sessionDigest, workspaceId, beforeGeneration) {
  for (let generation = beforeGeneration - 1; generation >= 1; generation -= 1) {
    const seal = readCandidateSeal(directory, sessionDigest, workspaceId, generation, { optional: true });
    if (seal) return seal;
  }
  return null;
}

function previousGenerationContinuity(root, session, custody, directory) {
  if (custody.generation === 1) {
    return { priorSeal: null, priorStop: null, priorForwardMotionProof: null, retainedSeal: null };
  }
  const generation = custody.generation - 1;
  generationAttemptEntry(root, session, custody, generation);
  const seal = readCandidateSeal(directory, session.sessionDigest, custody.workspaceId, generation, { optional: true });
  if (seal) {
    if (seal.baseHead !== custody.baseHead || custody.expectedDirtyManifestDigest !== seal.dirtyManifestDigest) {
      fail("MH_WORKSPACE_CUSTODY_MISMATCH", "ACTIVE repair generation is not based on the exact preceding candidate seal");
    }
    return { priorSeal: seal, priorStop: null, priorForwardMotionProof: null, retainedSeal: seal };
  }
  const stop = findWorkerStopForGeneration(root, session.sessionDigest, custody.workspaceId, generation);
  if (!stop || custody.expectedDirtyManifestDigest !== stop.endBoundary.dirtyManifestDigest) {
    fail("MH_WORKSPACE_CUSTODY_MISMATCH", "ACTIVE generation lacks exact preceding candidate-seal or worker-stop continuity");
  }
  const proof = findForwardMotionProofForStop(root, stop.stopDigest);
  if (!proof || proof.disposition !== "CONTINUE_WITH_ALTERNATIVE") {
    fail("MH_WORKSPACE_CUSTODY_MISMATCH", "worker STOP cannot advance a generation without an exact CONTINUE_WITH_ALTERNATIVE proof");
  }
  return {
    priorSeal: null,
    priorStop: stop,
    priorForwardMotionProof: proof,
    retainedSeal: latestPriorCandidateSeal(directory, session.sessionDigest, custody.workspaceId, generation),
  };
}

function resolveActiveWorkspaceContinuation(root, session, custody) {
  ...
  const boundary = captureWorkspaceBoundary(canonicalWorkspace, session.allowedPaths);
  const inspected = boundary.inspected;
  ...
  const continuity = previousGenerationContinuity(root, session, current, directory);
  const currentSeal = readCandidateSeal(directory, session.sessionDigest, current.workspaceId, current.generation, { optional: true });
  const currentEntry = generationAttemptEntry(root, session, current, current.generation, { optional: true });
  const currentProductProof = readProductProof(directory, session.sessionDigest, current.workspaceId, current.generation, { optional: true });

  if (currentSeal) {
    if (!currentEntry) {
      fail("MH_WORKSPACE_CUSTODY_MISMATCH", "durable candidate seal exists without its admitted AttemptEntry");
    }
    assertCandidateSealCurrent({
      workspacePath: canonicalWorkspace,
      stateDirectory: directory,
      seal: currentSeal,
      code: "MH_WORKSPACE_CUSTODY_MISMATCH",
    });
    if (currentProductProof && (currentProductProof.candidateSealDigest !== currentSeal.sealDigest
        || currentProductProof.candidateTreeOid !== currentSeal.candidateTreeOid)) {
      fail("MH_WORKSPACE_CUSTODY_MISMATCH", "persisted product proof does not belong to the current sealed candidate");
    }
    return Object.freeze({
      custody: current,
      kind: "SEALED_CANDIDATE",
      candidateSeal: currentSeal,
      priorSeal: continuity.priorSeal,
      retainedCandidatePaths: continuity.retainedSeal?.candidatePaths || [],
      attemptEntry: currentEntry,
      productProof: currentProductProof,
    });
  }

  if (currentProductProof) {
    fail("MH_WORKSPACE_CUSTODY_MISMATCH", "persisted product proof exists without a current candidate seal");
  }
  const currentStop = findWorkerStopForGeneration(root, session.sessionDigest, current.workspaceId, current.generation);
  if (currentStop) {
    if (!currentEntry) fail("MH_WORKSPACE_CUSTODY_MISMATCH", "durable worker STOP exists without its admitted AttemptEntry");
    assertWorkerStopBoundaryCurrent(currentStop, stopBoundaryEvidence(boundary));
    return Object.freeze({
      custody: current,
      kind: "WORKER_STOP",
      candidateSeal: null,
      ...
      retainedCandidatePaths: continuity.retainedSeal?.candidatePaths || [],
      attemptEntry: currentEntry,
      workerStop: currentStop,
      forwardMotionProof: findForwardMotionProofForStop(root, currentStop.stopDigest),
      productProof: null,
    });
  }

  const materialization = currentEntry
    ...
    : null;
  if (materialization) {
    if (materialization.attemptEntryDigest !== currentEntry.entryDigest) {
      fail("MH_WORKSPACE_CUSTODY_MISMATCH", "durable materialization target belongs to a different AttemptEntry");
    }
    if (boundary.indexDiff !== "") {
      fail("MH_WORKSPACE_CUSTODY_MISMATCH", "real Git index changed while candidate materialization was in progress");
    }
    return Object.freeze({
      custody: current,
      kind: "MATERIALIZING",
  ...
  if (continuity.priorStop) {
    assertWorkerStopBoundaryCurrent(continuity.priorStop, stopBoundaryEvidence(boundary));
  }
  if (current.generation === 1
      && current.expectedDirtyManifestDigest !== dirtyManifestDigest({ inspected: { entries: [] } })) {
    fail("MH_WORKSPACE_CUSTODY_MISMATCH", "generation 1 custody baseline is not the exact clean-manifest identity");
  }

  if (current.generation === 1) {
    const cleanDigest = dirtyManifestDigest({ inspected: { entries: [] } });
    if (!inspected.clean || inspected.staged.length > 0 || current.expectedDirtyManifestDigest !== cleanDigest) {
      fail("MH_WORKSPACE_CUSTODY_MISMATCH", "generation 1 baseline is not the exact clean sealed base");
    }
  } else if (continuity.priorSeal) {
    assertCandidateSealCurrent({
      workspacePath: canonicalWorkspace,
      stateDirectory: directory,
      seal: continuity.priorSeal,
      code: "MH_WORKSPACE_CUSTODY_MISMATCH",
    });
  }
  ...
  return Object.freeze({
    custody: current,
    kind: currentEntry ? "ENTERED_NO_SEAL" : "BASELINE",
    candidateSeal: null,
    priorSeal: continuity.priorSeal,
    retainedCandidatePaths: continuity.retainedSeal?.candidatePaths || [],
    attemptEntry: currentEntry,
...
function validateCustodyWorkspace(root, session, custody) {
  return resolveActiveWorkspaceContinuation(root, session, custody).custody;
}
...
function persistWorkSession(repositoryPath, session, workspace) {
  ...
  if (session.origin.type === "REPO_OUTCOME") {
    ...
    const claimStem = session.origin.claimDigest.slice("sha256:".length);
    bindOutcomeClaim({
      repositoryPath: root,
    ...
    writeJsonAtomic(path.join(claimsDirectory, `${claimStem}.json`), {
      schemaVersion: CLAIM_WORK_SESSION_POINTER_SCHEMA,
      claimDigest: session.origin.claimDigest,
      ...pointerBody,
    });
  } else {
    ...
    clearOwnerGoalIngress(root, { expectedContent: session.productResult });
  }
  ...
  return {
    ...
    schemaPath: path.join(directory, `${resultStem}.worker-result.schema.json`),
    agentOutputPath: path.join(directory, `${resultStem}.agent.json`),
  };
}

function readClaimWorkPointer(root, claimDigest, { optional = false } = {}) {
  if (!/^sha256:[a-f0-9]{64}$/u.test(String(claimDigest || ""))) {
    fail("MH_WORK_RESUME", "claimDigest must be a sha256 digest");
  }
  const directory = stateDirectory(root);
  const pointerPath = path.join(directory, "claims", `${claimDigest.slice("sha256:".length)}.json`);
  let pointer;
  try {
    pointer = JSON.parse(fs.readFileSync(pointerPath, "utf8"));
  } catch (error) {
    if (optional && error?.code === "ENOENT") return null;
    fail("MH_WORK_RESUME", `no claim-addressed work session: ${error.message}`);
  }
  if (!pointer || pointer.schemaVersion !== CLAIM_WORK_SESSION_POINTER_SCHEMA
    || pointer.claimDigest !== claimDigest
    ...
    fail("MH_WORK_RESUME", "claim-addressed work-session pointer is invalid");
  }
  if (!samePath(pointer.repositoryRoot, root) || !/^[a-f0-9]{64}\.json$/u.test(pointer.sessionFile)) {
    fail("MH_WORK_RESUME", "claim-addressed work-session pointer does not belong to this repository");
  }
  const sessionPath = path.resolve(directory, pointer.sessionFile);
  if (!samePath(path.dirname(sessionPath), directory)) {
    fail("MH_WORK_RESUME", "claim-addressed work-session pointer escapes the persisted session directory");
  }
...
function recoverBankedExecutionResult({ repositoryPath, sessionDigest, workspaceId }) {
  ...
  const productProof = readProductProof(directory, sessionDigest, workspaceId, custody.generation, { optional: true });
  if (productProof && (productProof.candidateSealDigest !== seal.sealDigest
      || productProof.candidateTreeOid !== seal.candidateTreeOid
      || productProof.state === "FAILED")) {
    fail("MH_WORK_BANK_RECOVERY_EVIDENCE", "BANK recovery product proof does not authorize the recovered product outcome");
  }
...
function sessionForCustody(root, custody, directory) {
  if (!samePath(custody.repositoryRoot, root)) {
    fail("MH_WORKSPACE_CUSTODY_MISMATCH", "workspace custody belongs to a different repository root");
  }
  const sessionPath = path.join(directory, `${custody.sessionDigest.slice("sha256:".length)}.json`);
  const session = bindWorkSessionToRepository(loadWorkSession(sessionPath), root);
  if (session.sessionDigest !== custody.sessionDigest) {
    fail("MH_WORK_RESUME", "immutable work-session identity does not match workspace custody");
  }
  return session;
}
...
function latestWorkSessionState(repositoryPath) {
  const root = repositoryRoot(repositoryPath);
  const directory = stateDirectory(root);
  const registryDir = workspaceRegistryDirectory(root, { create: false });
  const activeDirect = [];
  const terminalDirect = [];

  for (const stored of listWorkspaceCustodies(registryDir)) {
    const session = sessionForCustody(root, stored, directory);
    if (session.origin.type !== "OWNER_GOAL") continue;
    let custody = stored;
    if (custody.state === "ACTIVE") {
    ...
    if (custody.state === "ACTIVE") {
      const active = validateCustodyWorkspace(root, session, custody);
      activeDirect.push({ session, custody: active });
      continue;
    }
    if (custody.state.startsWith("TERMINAL_")) terminalDirect.push({ session, custody });
  }

  if (activeDirect.length > 1) {
    fail("MH_WORK_RESUME_CONFLICT", "more than one ACTIVE direct work session exists for this repository", {
      workspaceIds: activeDirect.map((entry) => entry.custody.workspaceId).sort(),
      sessionDigests: activeDirect.map((entry) => entry.session.sessionDigest).sort(),
    });
  }
  if (activeDirect.length === 1) {
    const [{ session, custody }] = activeDirect;
    resumeRecords.set(session, { repositoryRoot: root, workspaceId: custody.workspaceId });
    return Object.freeze({ state: "ACTIVE", session, custodyState: custody.state });
  }
  if (terminalDirect.length > 0) {
    terminalDirect.sort((left, right) => Date.parse(right.custody.updatedAt) - Date.parse(left.custody.updatedAt)
      || right.custody.workspaceId.localeCompare(left.custody.workspaceId));
    return Object.freeze({ state: "TERMINAL", session: null, custodyState: terminalDirect[0].custody.state });
  }
  return Object.freeze({ state: "NONE", session: null, custodyState: null });
}

function claimWorkSessionState(repositoryPath, claimDigest) {
  const root = repositoryRoot(repositoryPath);
  const stored = readClaimWorkPointer(root, claimDigest, { optional: true });
  if (!stored) {
    const session = readOutcomeClaimSession(root, claimDigest, { optional: true });
    if (!session) return Object.freeze({ state: "NONE", session: null, custodyState: null, workspaceId: null });
    return Object.freeze({ state: "PENDING", session: bindWorkSessionToRepository(session, root), custodyState: null, workspaceId: null });
  }

  const { directory, pointer, sessionPath } = stored;
  const session = bindWorkSessionToRepository(loadWorkSession(sessionPath), root);
  const expectedFile = `${session.sessionDigest.slice("sha256:".length)}.json`;
  if (session.origin.type !== "REPO_OUTCOME"
      || session.origin.claimDigest !== claimDigest
      || pointer.sessionDigest !== session.sessionDigest
      || pointer.sessionFile !== expectedFile) {
    fail("MH_WORK_RESUME", "persisted claim/session identity does not match the workspace record");
  }
  const preWorkspaceSession = readOutcomeClaimSession(root, claimDigest, { optional: true });
  if (preWorkspaceSession && preWorkspaceSession.sessionDigest !== session.sessionDigest) {
    fail("MH_OUTCOME_CLAIM_SESSION", "workspace-bound session differs from the durable pre-workspace Claim session");
  }
  ...
  if (!samePath(pointer.workspace.path, custody.workspacePath)) {
    fail("MH_WORKSPACE_CUSTODY_MISMATCH", "persisted claim workspace path does not match controller custody");
  }
  if (custody.state !== "ACTIVE") {
    return Object.freeze({
      state: "TERMINAL",
  ...
  return Object.freeze({ state: "ACTIVE", session, custodyState: active.state, workspaceId: active.workspaceId });
}

function loadClaimWorkSession(repositoryPath, claimDigest) {
  const current = claimWorkSessionState(repositoryPath, claimDigest);
  if (current.state === "NONE") fail("MH_WORK_RESUME", "no resumable claim-addressed work session exists");
  if (!["PENDING", "ACTIVE"].includes(current.state)) {
    fail("MH_WORKSPACE_NOT_EXECUTABLE", `claim workspace is terminal or inactive: ${current.custodyState}`);
  }
...
module.exports = {
  ...
  loadClaimWorkSession,
  loadLatestWorkSession,
  managedWorktreeRoot,
  ownerGoalIngressPath,
  parsePorcelainZ,
  pathAllowed,
