...
function ownerGoalIngressPath(repositoryPath) {
  return path.join(protocolRoot(repositoryPath), OWNER_GOAL_INGRESS_FILE);
}
...
function readOwnerGoalIngress(repositoryPath, { optional = true } = {}) {
  const filePath = ownerGoalIngressPath(repositoryPath);
  let parsed;
  try {
