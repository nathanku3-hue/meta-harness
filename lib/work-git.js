...
const WORK_SESSION_POINTER_SCHEMA = "work-session-pointer/v4";
...
function latestWorkSessionState(repositoryPath) {
  ...
  const { pointer, sessionPath } = latest;
  const custody = readWorkspaceCustody(workspaceRegistryDirectory(root), pointer.workspace.workspaceId);
