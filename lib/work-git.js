...
function previousGenerationSeal(root, session, custody, directory) {
  if (custody.generation === 1) return null;
  const seal = readCandidateSeal(directory, session.sessionDigest, custody.workspaceId, custody.generation - 1);
  generationAttemptEntry(root, session, custody, custody.generation - 1);
...
function resolveActiveWorkspaceContinuation(root, session, custody) {
  ...
  const priorSeal = previousGenerationSeal(root, session, current, directory);
  ...
  const currentProof = readProductProof(directory, session.sessionDigest, current.workspaceId, current.generation, { optional: true });
  ...
  if (currentSeal) {
    ...
    if (currentProof && (currentProof.candidateSealDigest !== currentSeal.sealDigest
        || currentProof.candidateTreeOid !== currentSeal.candidateTreeOid)) {
