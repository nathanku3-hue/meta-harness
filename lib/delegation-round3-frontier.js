...
} = require("./delegation-round3-lane-candidate");
...
const FRONTIER_CANDIDATE_JSON_SCHEMA = Object.freeze({
  ...
  properties: {
    schemaVersion: { const: DELEGATION_FRONTIER_SCHEMA },
    laneDecisions: {
      type: "array",
      ...
      items: {
        ...
        properties: {
          ...
          decision: { enum: ["CONTINUE", "HOLD", "OBSOLETE"] },
    ...
    newOutcomes: {
      type: "array",
      maxItems: MAX_FRONTIER_OUTCOMES,
      items: NEW_LANE_CANDIDATE_JSON_SCHEMA,
    },
    blockers: {
      type: "array",
    ...
    gate: {
      ...
      properties: {
        kind: { enum: ["CONTINUE", "FORWARD_GATE", "OWNER_DECISION"] },
...
const GRILL_CANDIDATE_JSON_SCHEMA = Object.freeze({
  ...
  properties: {
    schemaVersion: { const: DELEGATION_GRILL_SCHEMA },
    verdict: { enum: ["ACCEPT", "REPLACE"] },
...
function validationContext(options = {}) {
  ...
  const liveOutcomeDigests = new Set(liveLanes.map((lane) => lane.outcomeDigest));
  const availableSlots = options.availableSlots === undefined ? MAX_FRONTIER_LANES - liveLanes.length : options.availableSlots;
  if (!Number.isInteger(availableSlots) || availableSlots < 0 || availableSlots > MAX_FRONTIER_LANES) {
    fail("MH_DELEGATION_R3_FRONTIER_CAPACITY", `availableSlots must be an integer from 0 to ${MAX_FRONTIER_LANES}`);
  }
  ...
  return { liveLanes, liveIds: new Set(liveIds), liveOutcomeDigests, availableSlots, knownEvidenceRefs };
}
...
function validateDelegationFrontierCandidate(value, options = {}) {
  ...
  const lifecycleReleasedSlots = laneDecisions.filter((entry) => entry.decision !== "CONTINUE").length;
  const refillSlots = Math.min(MAX_FRONTIER_LANES, context.availableSlots + lifecycleReleasedSlots);
  if (!Array.isArray(value.newOutcomes) || value.newOutcomes.length > refillSlots || value.newOutcomes.length > MAX_FRONTIER_OUTCOMES) {
    fail("MH_DELEGATION_R3_FRONTIER_CAPACITY", "frontier new Outcomes exceed capacity available after its HOLD/OBSOLETE lifecycle decisions");
  }
  const outcomeIds = new Set();
  const newOutcomes = value.newOutcomes.map((entry, index) => validateDelegationLaneCandidate(entry, index, {
    knownEvidenceRefs: context.knownEvidenceRefs,
    liveOutcomeDigests: context.liveOutcomeDigests,
    seenIds: outcomeIds,
  }));
  ...
  if (value.gate.kind === "FORWARD_GATE" && (continuingLanes > 0 || newOutcomes.length > 0)) {
  ...
  if (value.gate.kind === "CONTINUE" && continuingLanes === 0 && newOutcomes.length === 0) {
    fail("MH_DELEGATION_R3_GATE", "CONTINUE requires at least one continuing or newly proposed autonomous lane");
  }
...
function semanticFrontier(value) {
  ...
  return {
    ...
    newOutcomes: frontier.newOutcomes.map(({
      id,
      desiredState,
      preconditions,
      evidenceRequirement,
