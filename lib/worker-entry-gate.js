 ...
 * @param {object} [input.entryAuthorityInput] optional raw pre-planning entry-authority input
...
function buildWorkerEntryGate({
  ...
  if (entryAuthorityInput !== undefined) {
    const entryAuthority = evaluateEntryAuthority(entryAuthorityInput);
    gate.entry_authority = entryAuthority;
    gate.required_inputs.entry_authority_verdict = entryAuthority.verdict;

    if (entryAuthority.verdict !== "PASS_CURRENT" || entryAuthority.ok !== true) {
      reasons.push({
        code: `ENTRY_AUTHORITY_${entryAuthority.verdict}`,
        detail: entryAuthority.next_action,
      });
    }

    if (entryAuthority.mutates !== false
      || entryAuthority.executes_child_commands !== false
      || entryAuthority.network !== false) {
      reasons.push({
        code: "ENTRY_AUTHORITY_NOT_READ_ONLY",
        detail: "entry authority must be read-only, process-free, and network-free",
      });
    }
  }
