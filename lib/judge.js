...
function gitContext(targetRoot, input) {
  const root = gitOutput(targetRoot, ["rev-parse", "--show-toplevel"]);
  if (!root.ok) return { ok: false, code: "JUDGE_INPUT_TARGET_NOT_GIT", message: "target is not a git repository" };
  const gitRoot = fs.realpathSync(root.stdout);
  const realTarget = fs.realpathSync(targetRoot);
  if (!isInside(gitRoot, realTarget)) {
  ...
  const base = gitOutput(targetRoot, ["rev-parse", "--verify", `${input.base_ref}^{commit}`]);
  ...
  const head = gitOutput(targetRoot, ["rev-parse", "--verify", "HEAD^{commit}"]);
  if (!head.ok) return { ok: false, code: "JUDGE_INPUT_BASE_REF_INVALID", message: "HEAD does not resolve to a commit" };
  const mergeBase = gitOutput(targetRoot, ["merge-base", base.stdout, head.stdout]);
...
async function check(options = {}) {
  ...
  const git = gitContext(targetRoot, validated.input);
  ...
  const changes = changedFiles(targetRoot, git.mergeBase);
  const context = {
    targetRoot,
