...
const { assertDelegationEvidenceLane } = require("./delegation-round3-lane-authority");
const {
  applyGrill,
...
const { applyLifecycle, resolveDelegationSnapshots } = require("./delegation-round3-host");
const {
...
function laneStateSnapshot(records) {
  return Object.freeze(records.map((record) => Object.freeze({
    delegationId: record.delegationId,
    laneKey: record.acceptanceLane.laneKey,
    taskId: record.acceptanceLane.taskId,
    workspaceId: record.acceptanceLane.workspaceId,
    launchStatus: record.acceptanceLane.launchStatus,
    resultDigest: record.acceptanceLane.result?.resultDigest || null,
  })).sort((left, right) => left.delegationId.localeCompare(right.delegationId)
    || left.laneKey.localeCompare(right.laneKey)));
}

function sameLaneState(leftRecords, rightRecords) {
  return JSON.stringify(laneStateSnapshot(leftRecords)) === JSON.stringify(laneStateSnapshot(rightRecords));
}
...
function assertEvidenceLifecycle(repositoryPath, records) {
  for (const record of records) {
    if (record.acceptanceLane.acceptance !== "PENDING") continue;
    assertDelegationEvidenceLane(repositoryPath, record.acceptanceLane.outcomeDigest, {
      laneKey: record.acceptanceLane.laneKey,
    });
  }
}
...
function appendCloseActions(records, lifecycleActions, closedLaneIds) {
  for (const record of records) {
    if (!record.acceptanceLane.result) continue;
    const id = laneIdentity(record.acceptanceLane);
    if (closedLaneIds.has(id)) continue;
    closedLaneIds.add(id);
...
async function runDelegationRound3({
  ...
  let family = acceptedFamily(await resolveDelegationSnapshots(delegations, host, MAX_DELEGATION_FAMILY));
  const initialGate = gateForFamily(gate, family);
  let records = familyLaneRecords(family);
  ...
  let planned = null;
  let grillRuns = 0;
  let staleHostAttempts = 0;

  while (true) {
    appendCloseActions(records, lifecycleActions, closedLaneIds);
    const backlog = sortedUnlandedResults(repositoryPath, current, records);
    for (const record of backlog) {
      const accepted = family.find((entry) => entry.acceptance.delegationId === record.delegationId)?.acceptance;
      if (!accepted) fail("MH_DELEGATION_R3_FAMILY", "result-bearing lane lost its retained delegation acceptance");
    ...
    current = readCurrentWorldState(repositoryPath, { now: now() });
    assertEvidenceLifecycle(repositoryPath, records);
    ...
    let refreshedFamily = family;
    let refreshedRecords = records;
    if (family.length > 0) {
      const refreshed = await resolveDelegationSnapshots(
        delegations,
        host,
        MAX_DELEGATION_FAMILY,
        { forceRefresh: true },
      );
      refreshedFamily = acceptedFamily(refreshed);
      refreshedRecords = familyLaneRecords(refreshedFamily);
    }
    const latest = readCurrentWorldState(repositoryPath, { now: now() });
    const worldChanged = latest.head.headDigest !== planned.input.world.head.headDigest;
    const lanesChanged = !sameLaneState(records, refreshedRecords);
    if (worldChanged || lanesChanged) {
      staleHostAttempts += 1;
      if (staleHostAttempts > 2) {
        fail("MH_DELEGATION_R3_FRONTIER_STALE", "delegation World or retained lane state kept changing during bounded reconciliation", {
          worldChanged,
          lanesChanged,
          before: laneStateSnapshot(records),
          after: laneStateSnapshot(refreshedRecords),
        });
      }
