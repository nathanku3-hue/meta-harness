...
function scoutTimeoutSeconds(scout) {
  return positiveInteger(scout.timeout_seconds ?? DEFAULT_FANOUT_BUDGET.timeout_seconds, "scout.timeout_seconds");
}
...
function validateScoutFanout(scouts, inputBudget = {}) {
  ...
  const maxScoutTimeout = scouts.reduce((max, scout) => Math.max(max, scoutTimeoutSeconds(scout)), 0);
  ...
  checks.push({
    ...
    status: maxScoutTimeout <= budget.timeout_seconds ? "pass" : "fail",
    observed: maxScoutTimeout,
