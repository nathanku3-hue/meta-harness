...
function exactPromptText(prompt) {
  ...
  const [block] = prompt;
  if (!block || block.type !== "text" || typeof block.text !== "string" || block.text.length === 0) return null;
  return block.text;
}
...
function createAcpTransport(context, {
  ...
  async function prompt(params, { signal, sendUpdate }) {
    ...
    const text = exactPromptText(params?.prompt);
    if (text === null) {
      invalidParams(acp, "session/prompt requires exactly one non-empty text block", { field: "prompt" });
    }
