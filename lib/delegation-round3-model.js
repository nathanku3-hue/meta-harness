...
function buildDelegationFrontierPrompt(input) {
  return [
    ...
    "",
    `Released capacity: ${input.availableSlots} lane(s); total live-lane bound: ${MAX_FRONTIER_LANES}.`,
    "",
    "FRONTIER LAWS",
    ...
    "- Capacity is a ceiling, not a quota. Propose fewer new Outcomes, including zero, when there is no additional positive-value work.",
    "- New Outcomes must be independently acceptance-readable and must follow from Endgame + current World + named evidence.",
    "- For each new Outcome, predict only the smallest exact expectedWritePaths plus journeyState, doNow, and stopOnlyIf needed to execute it. These are disposable semantic/footprint predictions, not authority.",
    "- Do not choose validation commands, Git remote/branch, commit/push policy, task identity, workspace identity, runtime, browser, or publication authority; the controller derives those mechanically.",
...
function buildDelegationGrillPrompt(input, frontier) {
  return [
    ...
    "",
    "CHALLENGE LAWS",
    "- Reject unnecessary decomposition, duplicate work, stale lanes, and owner gates that autonomous evidence can resolve.",
