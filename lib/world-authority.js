...
const OBJECT_KINDS = new Set([
  ...
  "delegation-frontiers",
  "work-results",
  "execution-closures",
  "worker-stops",
  "forward-motion-proofs",
...
function protocolRoot(repositoryPath, { create = true } = {}) {
...
function readContendedLock(repositoryPath) {
  ...
  try {
    ...
    if (error?.code === "ENOENT") {
      fail("MH_WORLD_AUTHORITY_BUSY", "world-authority lock changed during acquisition");
    }
    fail("MH_WORLD_AUTHORITY_LOCK", `world-authority lock is unreadable: ${error.message}`);
  }
  try {
    ...
    if (error?.code !== "MH_WORLD_AUTHORITY_LOCK") throw error;
    let stat;
    try {
    ...
    if (Date.now() - stat.mtimeMs < WORLD_AUTHORITY_LOCK_STALE_MS) {
      fail("MH_WORLD_AUTHORITY_BUSY", "world-authority lock is still being initialized by another controller");
    }
    throw error;
  }
}
...
function acquireWorldAuthorityLock(repositoryPath, now = new Date()) {
  ...
  const observed = readContendedLock(repositoryPath);
  const existingRaw = observed.raw;
  const existing = observed.lock;
  ...
  const verifiedObservation = readContendedLock(repositoryPath);
  const verifiedRaw = verifiedObservation.raw;
  const verified = verifiedObservation.lock;
