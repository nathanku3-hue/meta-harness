...
function attemptEntryPath(repositoryPath, origin, ordinal) {
  ...
  fail("MH_ATTEMPT_ENTRY_ORIGIN", "attempt entry origin must be REPO_DECISION or OWNER_GOAL");
}
...
function readLock(repositoryPath) {
  try {
    return validateLock(JSON.parse(fs.readFileSync(lockPath(repositoryPath), "utf8")));
  } catch (error) {
...
function acquireWorldAuthorityLock(repositoryPath, now = new Date()) {
  ...
  const existing = readLock(repositoryPath);
  ...
  const verified = readLock(repositoryPath);
...
function admissionForHead(repositoryPath, headDigest) {
  ...
  for (const candidate of firstAttemptEntries(repositoryPath)) {
    let decision;
    try {
      decision = readImmutableJson(repositoryPath, "decisions", candidate.decisionDigest);
    } catch (error) {
      if (error?.code === "MH_WORLD_AUTHORITY_MISSING") continue;
      throw error;
    }
    if (decision?.worldHeadDigest === headDigest) matches.push(candidate);
  }
  if (matches.length > 1) {
