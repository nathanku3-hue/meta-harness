...
async function runWork({
  ...
  const productProofResolution = resolveProductProof(repositoryPath, session.base.commit);
  ...
  try {
    ...
    if (startingContinuation.kind === "ENTERED_NO_SEAL") {
      ...
      agentResult = {
        observableResult: "No durable candidate was sealed before the controller interruption.",
        blocker: "The coding generation was already admitted but produced no durable candidate seal; replay is refused.",
      ...
      if (continuation.kind === "SEALED_CANDIDATE") {
        ...
        agentResult = agentResult || {
          ...
          blocker: "",
          nextAction: "Use the recovered result.",
        };
        attemptMetric = {
          attempt,
          generation: custody.generation,
        ...
        const materializedPaths = [...new Set([
          ...(continuation.priorSeal?.candidatePaths || []),
      ...
      if (validationPassed) {
        if (!productProof) {
          ...
          productProof = productProofResolution.available
            ? verifyProductProof({
              workspacePath: workspace.workspacePath,
              dependencySourcePath: workspace.repositoryRoot,
              session,
              candidateSeal,
              resolution: productProofResolution,
              env,
            })
            : unavailableProductProof({ session, candidateSeal, resolution: productProofResolution });
        ...
        const delivered = outcome === "BANKED_UNPROVEN"
          ? "Retain the banked candidate, but do not claim the requested product result complete without independent product proof."
        ...
        const result = {
          ...
          delivery,
          blocker: outcome === "DONE" ? "none" : "Independent product proof is unavailable for this sealed base.",
    ...
    const blocker = productProofFailureText(productProof) || validationText || agentResult?.blocker || "The coding worker did not produce a verifiable candidate.";
    const delivery = {
