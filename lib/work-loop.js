...
function missingValidationResult(session, repositoryPath) {
  return {
    ...
    blocker: "External validation is required. Standalone --goal derives package.json scripts.test from the sealed base commit; otherwise provide an explicit work-session/v4 with exact validation argv.",
    nextAction: "Provide exact controller validation through work-session/v4.",
    attempts: 0,
    sessionDigest: session.sessionDigest,
    executionPermits: [],
  };
