...
async function runWork({
  ...
  try {
    for (let attempt = 1; attempt <= session.maxAttempts; attempt += 1) {
      ...
      if (onAttemptStart !== undefined) {
        if (typeof onAttemptStart !== "function") {
          fail("MH_WORK_ATTEMPT_HOOK", "onAttemptStart must be a function when provided");
        }
        onAttemptStart({ attempt, executionPermit, session, workspace });
      }
