...
function missingValidationResult(session, repositoryPath) {
  return {
    ...
    blocker: "External validation is required. Standalone --goal supports a root package.json scripts.test; otherwise provide an explicit work-session/v3 with exact validation argv.",
    nextAction: "Provide exact controller validation through work-session/v3.",
    attempts: 0,
    sessionDigest: session.sessionDigest,
    executionPermits: [],
  };
