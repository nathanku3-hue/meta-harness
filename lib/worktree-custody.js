...
function runGit(args, cwd, timeoutMs = 30000, gitExecutablePath = "git") {
  const result = spawnSync(gitExecutablePath, args, {
    cwd,
    encoding: "utf8",
    timeout: timeoutMs,
    windowsHide: true,
...
/** Native path for Node filesystem calls and spawn cwd. */
...
/** Path for a Windows Git executable invoked from WSL. */
function gitPath(value, dialectHint = "") {
  const native = nativePath(value);
  const windowsGit = isWindowsDrivePath(dialectHint) || /\.exe$/i.test(String(dialectHint || ""));
  if (process.platform !== "win32" && windowsGit && isWslMountPath(native)) {
    return wslToWindowsDrive(native);
  }
  return native;
}

/** Resolve existing path components so Windows 8.3 aliases compare canonically. */
function comparisonNativePath(value) {
  ...
  if (fs.existsSync(current)) {
    try {
      ...
      // Keep the native spelling when realpath is unavailable for this component.
    }
  }
...
/** Comparison-only identity; never pass this value to fs or spawnSync. */
function comparisonKey(value) {
  const native = comparisonNativePath(value).replace(/\\/g, "/");
  const normalized = path.posix.normalize(native);
  if (process.platform === "win32" || isWindowsDrivePath(value) || isWslMountPath(normalized)) {
...
function hasGitMetadata(root) {
  return fs.existsSync(path.join(nativePath(root), ".git"));
}

function resolveOwner(root, { gitExecutablePath = "git" } = {}) {
  const nativeRoot = nativePath(root);
  const top = runGit(["rev-parse", "--show-toplevel"], nativeRoot, 30000, gitExecutablePath);
  const common = runGit(
  ...
  return {
    ok: true,
    topLevel,
    topLevelGit: gitPath(topLevel, top.stdout),
    gitCommonDir: commonDir,
    gitCommonDirGit: gitPath(commonDir, common.stdout),
    ownerRoot,
    approvedWorktreesRoot: path.join(ownerRoot, ".worktrees"),
  };
}

function listWorktreesPorcelain(ownerRoot, { gitExecutablePath = "git" } = {}) {
  const res = runGit(
    ["worktree", "list", "--porcelain"],
    nativePath(ownerRoot),
    30000,
    gitExecutablePath,
  );
  if (!res.ok) {
  ...
  for (const block of blocks) {
    ...
    if (entry.path) {
      entry.gitPath = entry.path;
      entry.path = nativePath(entry.path);
      entries.push(entry);
    }
  }
...
function classifyExternalLeak(entryPath, ownerRoot) {
  ...
  const owner = ownerRoot ? nativePath(ownerRoot) : "";
  ...
  // WSL mounts are valid only when they remain inside the owning repository.
  if (
    (lower.includes("e:\\mnt\\") || lower.includes("\\mnt\\e\\") || lower.includes("/mnt/e/")) &&
    (!owner || !isPathInside(entryPath, owner))
  ) {
    return { leak: true, class: "mnt_path", detail: "worktree under mnt path outside owner root" };
  }
...
function assertManagedDestination(ownerRoot, destinationPath) {
  const owner = nativePath(ownerRoot);
  const dest = nativePath(destinationPath);
  const findings = [];
...
function auditKnownCreatorRoots() {
  const roots = [
    nativePath(path.join(process.env.USERPROFILE || "C:\\Users\\Lenovo", ".devspace", "worktrees")),
    nativePath(path.join(process.env.USERPROFILE || "C:\\Users\\Lenovo", ".cursor", "worktrees")),
  ];
  const leaks = [];
...
function checkWorktreeCustody({
  ...
  const owner = resolveOwner(root, { gitExecutablePath });
  if (!owner.ok) {
    return {
      status: "fail",
  ...
  const listed = listWorktreesPorcelain(owner.ownerRoot, { gitExecutablePath });
  if (!listed.ok) {
    return {
      status: "fail",
  ...
  const excludeRequired = fs.existsSync(path.join(owner.ownerRoot, ".worktrees")) || linked.length > 0;
  const excludeOk = hasLocalWorktreesExclude(owner.ownerRoot);
  details.local_exclude_required = excludeRequired;
  details.local_exclude = excludeOk;
  if (excludeRequired && !excludeOk) {
    findings.push({
      severity: "fail",
