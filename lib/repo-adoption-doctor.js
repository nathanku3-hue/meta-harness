...
const FINDING_ORDER = [
  "ADOPT_DIRTY_RECOVERY_FIRST",
  "ADOPT_SYNC_MISSING_TEMPLATES",
  "ADOPT_ACTION_LAW_CONFLICT",
...
function addDirtyRecoveryFinding(findings, targetRoot) {
  const gitState = getRepoGitState(targetRoot);
  if (!gitState.isGitRepo || gitState.is_clean) return;
  findings.push(finding(
    "ADOPT_DIRTY_RECOVERY_FIRST",
    "dirty checkout requires recovery-first authority classification before generic code review",
    "warn",
    "preserve the checkout; establish provenance only from retained evidence, authority/custody, protected current work, reversibility, and the exact repair set before Architecture/Code Quality/Tests/Performance review",
    "Recovery",
    `Git observes ${gitState.dirty.count} dirty item(s); staged=${gitState.dirty.has_staged}; untracked=${gitState.dirty.has_untracked}; mutation origin is not established by Git`,
  ));
}
...
function addActionLawFindings(findings, targetRoot) {
  const actionLaw = scanHostGuidanceActionLawConflicts({ targetRoot });
  if (actionLaw.status === "PASS") {
    return;
  }
  findings.push(finding(
    "ADOPT_ACTION_LAW_CONFLICT",
    "operative repository guidance conflicts with outcome-first action law",
    "warn",
    "remove or narrow universal review/SAW/fixed section-by-section pauses/routine-approval requirements for ordinary reversible work",
    "Adoption",
    actionLaw.items.map((item) => `${item.path}: ${item.detail}`).join("; "),
  ));
}
...
function diagnoseRepoAdoption({ sourceRoot, targetRoot }) {
  ...
  addDirtyRecoveryFinding(findings, resolvedTarget);
  addSyncFindings(findings, resolvedSource, resolvedTarget);
  addActionLawFindings(findings, resolvedTarget);
