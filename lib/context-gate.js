...
const { appendEvent } = require("./harness-state");
...
function normalizeOverrideContextGate(value) {
  if (!value) return null;
  const candidate = typeof value === "string"
    ? { reason: value, code: undefined, actor: "human" }
    : value;
  const validation = validateBypass(candidate);
  if (!validation.ok) {
...
function runContextGate(options = {}) {
  const targetRoot = path.resolve(options.targetRoot || options.cwd || process.cwd());
  const override = normalizeOverrideContextGate(options.overrideContextGate);
  ...
  if (override) {
    ...
    result.markdown = renderContextGateMarkdown(result.artifact);
  }
