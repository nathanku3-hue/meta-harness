...
function checkContextQuality(options = {}) {
  ...
  const roundId = options.roundId ? safeRoundId(options.roundId) : defaultRoundId(cwd);
  ...
  const hintsResult = readContextHints({ cwd, now, hintsPath: options.hintsPath });
...
function runContextGate(options = {}) {
  const cwd = path.resolve(options.cwd || options.targetRoot || process.cwd());
  const result = checkContextQuality({ ...options, cwd });
  ...
  if (options.commitArtifact) {
    assertCommittedArtifactClean(cwd, paths.jsonPath, jsonString, paths.markdownPath, markdownString);
  }
