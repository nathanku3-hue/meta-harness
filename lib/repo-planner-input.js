...
const { readOutcomeClaim, REPOSITORY_ACTIVE_CLAIM_BOUND } = require("./outcome-claim");
...
function ownerIntent(repositoryPath, productDirection) {
  ...
  return Object.freeze({
    ...
    activeDirective: objective ? Object.freeze({
      ...
      ...(objective.ingressDigest ? { ingressDigest: objective.ingressDigest } : {}),
    }) : null,
  });
}
...
function commitmentState(entry) {
  ...
  if (entry.type === "EXTERNAL_OPEN") return "external_open";
...
function compileRepoPlannerInput({
  ...
  const occupiedSlots = activeCommitments.length;
  return Object.freeze({
    ...
    capacity: Object.freeze({
      ...
      occupiedSlots,
      availableSlots: Math.max(0, REPOSITORY_ACTIVE_CLAIM_BOUND - occupiedSlots),
    }),
