...
const DEFAULT_REQUIREMENTS = Object.freeze({
  github_security: Object.freeze({
    required: true,
    fields: Object.freeze(["status", "source", "checked_at", "commit"]),
...
function validatePassEvidence(key, evidence, requirement) {
  ...
  for (const field of requirement.fields) {
    ...
    if (!isNonEmptyString(evidence[field])) errors.push(field);
  }
  if (isNonEmptyString(evidence.checked_at) && !validDateTime(evidence.checked_at)) {
    errors.push("checked_at must be a valid date-time");
  }
  if (requirement.fields.includes("commit") && isNonEmptyString(evidence.commit) && !/^[0-9a-f]{40}$/i.test(evidence.commit)) {
    errors.push("commit must be a 40-character hash");
  }
