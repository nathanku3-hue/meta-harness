...
const WORKSPACE_LEASE_STALE_MS = 30_000;
...
function registryDirectory(gitCommonDirectory) {
...
function acquireWorkspaceExecutionLease({ registryDir, workspaceId, now = new Date() }) {
  ...
  if (processIsLive(existing.pid) || ageMs < WORKSPACE_LEASE_STALE_MS) {
