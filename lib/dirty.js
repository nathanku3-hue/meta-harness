...
function commandGate(argv, context = {}) {
  ...
  if (options.dirty === undefined) fail("gate scope requires --dirty <path>");
  if (options.scope === undefined) fail("gate scope requires --scope <path>");
  const dirtyPath = requireOptionValue(options.dirty, "gate scope --dirty");
  const scopePath = requireOptionValue(options.scope, "gate scope --scope");
