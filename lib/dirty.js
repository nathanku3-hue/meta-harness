...
function readScope(repoRoot, scopePath) {
  ...
  const normalized = {
    ...
    decision_inbox_path: scope.decision_inbox_path || DEFAULT_INBOX,
  };
...
function writeDecisionInbox(repoRoot, scope, result, dirtyOutputPath) {
  const decisions = result.classifications.filter((item) => item.action === "DECISION");
  if (decisions.length === 0) return;
  const inboxPath = resolveInside(repoRoot, scope.decision_inbox_path, "decision inbox");
  const inbox = readJsonFile(inboxPath, { v: 1, decisions: [] });
  const existing = new Set((inbox.decisions || []).map((item) => item.id));
  for (const item of decisions) {
    const id = `D-${item.decision_state_hash.slice(0, 10)}`;
    if (existing.has(id)) continue;
    inbox.decisions.push({
      id,
      kind: "dirty_work_scope_decision",
      question: `Approve touching inherited dirty path ${item.path}?`,
      recommended: "hold",
      state_hash: item.decision_state_hash,
...
function commandDirty(argv, context = {}) {
  ...
  if (action === "snapshot") {
    if (!options.out) fail("dirty snapshot requires --out <path>");
    const outPath = resolveInside(repoRoot, options.out, "snapshot output");
  ...
  if (action === "classify") {
    for (const flag of ["before", "after", "scope", "out"]) {
      if (!options[flag]) fail(`dirty classify requires --${flag} <path>`);
    }
    ...
    const result = classifyDirtyWork(before, after, scope);
    ...
    writeDecisionInbox(repoRoot, scope, result, toSlash(path.relative(repoRoot, outPath)));
...
function commandGate(argv, context = {}) {
  const repoRoot = resolveGitRoot(context.cwd || process.cwd());
  const { positional, options } = parseArgs(argv);
  if (positional[0] !== "scope") fail(`unknown gate action: ${positional[0] || "missing"}`);
  if (!options.dirty) fail("gate scope requires --dirty <path>");
  if (!options.scope) fail("gate scope requires --scope <path>");
  const scope = readScope(repoRoot, options.scope);
  const dirty = readRequiredJson(repoRoot, options.dirty, "dirty classification");
