...
function prepareProductProofVerifier({
  ...
  if (!resolution?.available
      || resolution.baseCommit !== session?.base?.commit
      || candidateSeal?.baseHead !== session?.base?.commit
      || candidateSeal?.sessionDigest !== session?.sessionDigest) {
    fail("MH_WORK_PRODUCT_PROOF", "product proof verifier requires the exact sealed session, candidate, and base-owned proof specification");
  }
  assertVerifierAvailable();
  ...
  const baseProofPath = path.join(verifierRoot, "base-proof");
  ...
  try {
    const baseTreeOid = String(runGit(workspacePath, ["rev-parse", `${resolution.baseCommit}^{tree}`], { env }).stdout || "").trim();
    materializeTree(workspacePath, candidateSeal.candidateTreeOid, candidatePath, candidateIndex, env);
    materializeTree(workspacePath, baseTreeOid, baseProofPath, baseIndex, env);
    const programTarget = path.join(baseProofPath, ...resolution.program.path.split("/"));
    const programStat = fs.lstatSync(programTarget);
    if (!programStat.isFile() || programStat.isSymbolicLink()) {
      fail("MH_WORK_PRODUCT_PROOF", "base-owned product proof program did not materialize as a regular file");
    }
    fs.mkdirSync(sessionPath, { recursive: true });
    fs.writeFileSync(path.join(sessionPath, "work-session.json"), `${JSON.stringify(session, null, 2)}\n`, "utf8");
    fs.mkdirSync(sandboxRoot, { recursive: true });
    writeSandboxScaffold(sandboxRoot);
    for (const relative of ["candidate", "base-proof", "session"]) {
      fs.mkdirSync(path.join(sandboxRoot, relative), { recursive: true });
    }
    const candidateTarget = path.join(sandboxRoot, "candidate");
    const baseProofTarget = path.join(sandboxRoot, "base-proof");
    return {
      verifierRoot,
      sandboxRoot,
      mounts: [
        { source: candidatePath, target: candidateTarget },
        { source: baseProofPath, target: baseProofTarget },
        ...
        ...proofDependencyMounts(dependencySourcePath, baseProofPath, baseProofTarget),
      ],
      sandboxCwd: "/base-proof",
      validationEnv: {
        ...sandboxEnvironment(env),
        META_HARNESS_CANDIDATE_ROOT: "/candidate",
        META_HARNESS_BASE_PROOF_ROOT: "/base-proof",
        META_HARNESS_SESSION_PATH: "/session/work-session.json",
      },
    };
  } catch (error) {
    fs.rmSync(verifierRoot, { recursive: true, force: true });
...
function verifyProductProof({
  ...
  const prepared = prepareProductProofVerifier({ workspacePath, dependencySourcePath, session, candidateSeal, resolution, env });
  try {
    const command = {
      argv: [resolution.runtime, `/base-proof/${resolution.program.path}`],
      cwd: "/base-proof",
      timeoutSeconds: resolution.timeoutSeconds,
    };
    const result = runSandboxedCommand(prepared, command);
