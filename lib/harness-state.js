...
const { assertTruthReconciled, renderCanonicalStatus } = require("./truth-reconciler");
...
function appendEvent(context, event, options = {}) {
  ensureHarness(context);
  const timestamp = options.timestamp;
  const clock = typeof timestamp === "string" ? () => timestamp : nowIso;
  return eventStore.appendEvent(harnessPath(context, "events.jsonl"), event, clock);
}
...
function workerReportTemplate() {
  return `User journey executed: <complete journey actually run>
...
- Compatibility-only CLI inputs and machine event metadata are not rendered into this default report surface.
...
function renderStatus(context = {}) {
  return renderCanonicalStatus({
    events: readEvents(context),
    targetRoot: context.cwd || process.cwd(),
  });
}

function refreshStatus(context = {}) {
  assertCanonicalMutationPaths(context.cwd || process.cwd(), { allowMissingStatus: true });
  const status = renderStatus(context);
  writeTextAtomic(harnessPath(context, "status.md"), status);
  assertTruthReconciled({ targetRoot: context.cwd || process.cwd() });
  return status;
}
