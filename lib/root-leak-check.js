...
function runGitInspection(args, cwd) {
  return runGit(args, cwd);
}

function collectFileInventory(root, { skipDirectories = [] } = {}) {
  const resolvedRoot = nativePath(root);
  const skipped = new Set(skipDirectories.map((name) => String(name)));
  const files = [];
  ...
  function walk(directory, relativeDirectory) {
    ...
    for (const entry of entries) {
      ...
      if (stat.isDirectory()) {
        if (!skipped.has(entry.name)) walk(entryPath, relative);
        continue;
      }
      if (stat.isSymbolicLink()) {
        const link = fs.readlinkSync(entryPath);
        files.push({
          path: toSlash(relative),
          type: "symlink",
          size: Buffer.byteLength(link, "utf8"),
          sha256: sha256Bytes(Buffer.from(link, "utf8")),
          link,
        });
        continue;
      }
  ...
  walk(resolvedRoot, "");
  return {
    root: toSlash(resolvedRoot),
    root_identity: pathIdentity(resolvedRoot),
    files,
  };
}
...
function scanOwner(ownerPath, ownerKind) {
  ...
  for (const entry of registered) {
    ...
    registeredWorktrees.push({
      path: toSlash(entryPath),
      path_identity: pathIdentity(entryPath),
      classification,
      owner_kind: ownerKind,
  ...
  const physicalWorktrees = physical.map((physicalPath) => ({
    path: toSlash(physicalPath),
    path_identity: pathIdentity(physicalPath),
    owner_kind: ownerKind,
    registered: registeredCanonical.has(pathIdentity(physicalPath)),
  }));
  ...
  for (const adminPath of listImmediateDirectories(adminRoot)) {
    ...
    const administrationEntry = {
      path: toSlash(adminPath),
      path_identity: pathIdentity(adminPath),
      owner_kind: ownerKind,
      git_pointer: null,
      valid: false,
    };
    if (!fs.existsSync(gitdirFile)) {
      administrationEntries.push(administrationEntry);
    ...
    administrationEntry.git_pointer = rawTarget;
    administrationEntry.target = toSlash(target);
    administrationEntry.target_identity = pathIdentity(target);
    administrationEntry.valid = fs.existsSync(target);
    administrationEntries.push(administrationEntry);
...
function scanRepositoryLayout({ targetRoot, creatorRoots = defaultCreatorRoots() } = {}) {
  ...
  for (const stableOwner of listImmediateDirectories(stableOwnersRoot)) {
    if (fs.existsSync(path.join(stableOwner, ".git"))) {
      ...
      scans.push({
        ...
        registeredWorktrees: [],
        physicalWorktrees: physical.map((entry) => ({
          path: toSlash(entry),
          path_identity: pathIdentity(entry),
          owner_kind: "unregistered-stable-owner",
          registered: false,
        })),
        administrationEntries: [],
      });
  ...
  const registeredWorktrees = scans.flatMap((scan) => scan.registeredWorktrees || [])
    .sort((left, right) => left.path_identity.localeCompare(right.path_identity));
  const physicalWorktrees = scans.flatMap((scan) => scan.physicalWorktrees || [])
    .sort((left, right) => left.path_identity.localeCompare(right.path_identity));
  const administrationEntries = scans.flatMap((scan) => scan.administrationEntries || [])
    .sort((left, right) => left.path_identity.localeCompare(right.path_identity));
