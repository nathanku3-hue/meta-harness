...
function effectivePermissionDecision(candidate, override) {
  for (const value of [override, candidate?.permission_decision, candidate?.promotion_decision]) {
...
function preflightSkillPromotion({ targetRoot, skillName, permissionDecision = null }) {
  ...
  if (candidate) {
    ...
    const authorization = effectivePermissionDecision(candidate, permissionDecision);
    if ((diff.added_allowed_tools.length > 0 || diff.removed_forbidden_paths.length > 0) && !authorization) {
      blockers.push(blocker("permission-expansion", "candidate expands permissions relative to active/prototype baseline without a permission decision"));
    }
  }
  ...
  return {
    ...
    permission_authorization: candidate ? effectivePermissionDecision(candidate, permissionDecision) : null,
    blockers,
  };
}
