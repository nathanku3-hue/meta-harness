...
function isAdoptedSecurityPolicy(root) {
...
function checkSecurityPolicy(root, mode = "local") {
  const policyPath = path.join(root, ".meta-harness", "security-policy.json");
  let stat;
  try {
    ...
    if (error && error.code === "ENOENT") {
      const required = mode === "strict" || mode === "release";
      return subcheck(
        "SEC_POLICY_001",
        "security_policy",
        required ? "fail" : "skip",
        required ? ".meta-harness/security-policy.json is required in strict/release mode" : "security policy not adopted",
        "Adopt .meta-harness/security-policy.json before strict/release readiness",
  ...
  if (!stat.isFile() || stat.isSymbolicLink()) {
    return subcheck("SEC_POLICY_001", "security_policy", "fail", ".meta-harness/security-policy.json must be a regular file", "Replace the installed security policy with a regular JSON file");
  }
...
function readPolicyRedactionSurfaces(root) {
  try {
    const policy = readJson(root, ".meta-harness/security-policy.json");
    if (
      Array.isArray(policy.redaction_surfaces)
      && policy.redaction_surfaces.length > 0
      && policy.redaction_surfaces.every((surface) => typeof surface === "string" && surface.trim().length > 0)
    ) {
      return policy.redaction_surfaces;
    }
  } catch (error) {
    // Missing or malformed policy falls back to scanRedactionSurfaces defaults.
  }
  return undefined;
}

function checkRedaction(root, policyAdopted = false) {
  const result = scanRedactionSurfaces({
    targetRoot: root,
    surfaces: policyAdopted ? readPolicyRedactionSurfaces(root) : undefined
  });
...
function checkInstalledTrustContract(root) {
  const result = checkTrustPolicy({ targetRoot: root });
  if (result.status === "PASS") {
    return subcheck("SEC_TRUST_001", "trust_contract", "pass");
  }
  const first = (result.items || []).slice(0, 5).map((item) => `${item.path}: ${item.detail || item.status}`).join(", ");
  return subcheck("SEC_TRUST_001", "trust_contract", "fail", `malformed installed trust contract: ${first || "validation failed"}`, "Repair .meta-harness/skill-distillations.json");
}
...
async function checkSecurityBaseline({
  ...
  const resolvedPackageRoot = packageResolution ? (packageRoot || null) : (packageRoot || root);
  const packageOptions = { packageResolution };
  const policyAdopted = isAdoptedSecurityPolicy(root);
  const checks = [
    checkSecurityPolicy(root, mode),
    checkWorkflowPermissions(root),
    checkWorkflowTriggers(root),
    checkWorkflowUntrustedInput(root),
    checkLifecycleScripts(resolvedPackageRoot, mode, packageOptions),
    checkPackageMetadata(resolvedPackageRoot, packageOptions),
    checkReproducibility(resolvedPackageRoot, { ...packageOptions, repositoryRoot: root }),
    checkPackageDryRun(resolvedPackageRoot, noExec, packageOptions),
    checkInstalledTrustContract(root),
    checkRedaction(root, policyAdopted)
  ];
