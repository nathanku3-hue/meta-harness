...
const { assertTerminalEndgameCovered } = require("./terminal-endgame");
...
function compileRepoDecision(repositoryPath, worldState) {
  ...
  if (decision.value.decision.type === "NO_DISPATCH") {
    const endgameCoverage = decision.value.decision.reason === "USE_PRODUCT"
      ? assertTerminalEndgameCovered(repositoryPath, worldState)
      : null;
