...
function evidenceCheck(id, name, evidenceResult) {
  return check(id, name, evidenceResult.status, evidenceResult.reason, evidenceResult.next_action, { requiredForLocal: false, details: evidenceResult.details });
}

function checkExternalEvidence(policy) {
  return evidenceCheck(CHECK_IDS.externalEvidence, "github-security-evidence", evaluateReleaseEvidence(policy).githubSecurity);
}
...
function checkFullReleaseEvidence(policy) {
  return evidenceCheck(CHECK_IDS.fullReleaseEvidence, "full-release-evidence", evaluateReleaseEvidence(policy).fullRelease);
}
...
async function runReleaseCheck(options = {}) {
  ...
  const releaseEvidence = evaluateReleaseEvidence(policyResult.policy);
  const checks = [
    cleanTree,
    ...policyResult.checks,
    ...
    evidenceCheck(CHECK_IDS.externalEvidence, "github-security-evidence", releaseEvidence.githubSecurity),
    evidenceCheck(CHECK_IDS.fullReleaseEvidence, "full-release-evidence", releaseEvidence.fullRelease),
  ];
...
module.exports = {
  ...
  runReleaseCheck,
  _test: { checkExternalEvidence, checkFullReleaseEvidence, checkPackageMetadata, checkReleasePolicy, validateReleasePolicy },
};
