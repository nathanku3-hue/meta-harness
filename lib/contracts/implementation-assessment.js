 ...
 * Accepts undestructured input; strict-validates envelope before field access.
 */
function evaluateImplementationFacts(input) {
  const envelope = input === undefined ? {} : input;
  try {
    assertStrictJsonData(envelope);
  } catch (err) {
    return fail("FAILED", [reason(
      err.code || "STRICT_JSON",
      `evaluateImplementationFacts input: ${err.message}`,
    )]);
  }
  if (!isOrdinaryPlainObject(envelope)) {
    return fail("FAILED", [reason("INPUT_REQUIRED", "evaluateImplementationFacts input must be a plain object")]);
  }

  const runSpec = envelope.runSpec;
  const authorizationReceipt = envelope.authorizationReceipt;
  const workspaceAttestation = envelope.workspaceAttestation;
  const startCheck = envelope.startCheck;
  const trustedImplementationFacts = envelope.trustedImplementationFacts;
  const workspacePolicy = envelope.workspacePolicy;
