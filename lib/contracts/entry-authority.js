...
function validateSource(source) {
  ...
  if (!TRUSTED_SOURCE_KINDS.has(source.kind)) {
    reasons.push(reason(
      ...
      "R3 accepts only a controller-authorized RunSpec; other source classes require real verification before use",
    ));
  }
...
function validateExpected(expected) {
  ...
  if (!exactKeys(expected, controllerKeys)) {
    return {
      ok: false,
      reasons: [reason(
...
function evaluateEntryAuthority(input) {
  ...
  const repositoryMatches = observed.repositoryId === expected.repository.repositoryId
    && observed.objectFormat === expected.repository.objectFormat;
  if (repositoryMatches
    && observed.productBytesPresent === true
    ...
    return result(RESULT.CUSTODY_REQUIRED, [reason(
      ...
      "product bytes are present in the expected repository but are not reachable from the named Git authority",
    )]);
  }
