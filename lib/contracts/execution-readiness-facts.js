 ...
 */
function compareRepositoryIdentity(observed, expectedRepository) {
  const reasons = [];
  if (observed.repositoryId !== expectedRepository.repositoryId) {
    reasons.push(reason("READINESS_REPOSITORY_MISMATCH", "repositoryId does not match expected repository"));
  }
  if (observed.objectFormat !== expectedRepository.objectFormat) {
    reasons.push(reason("READINESS_OBJECT_FORMAT_MISMATCH", "objectFormat does not match expected repository"));
  }
  if (observed.observedHeadRevision !== expectedRepository.expectedBaseRevision) {
    reasons.push(reason(
      "READINESS_HEAD_MISMATCH",
      "observedHeadRevision must equal expectedBaseRevision",
    ));
  }
...
function bindReadinessToRunSpec(facts, runSpec, runSpecDigest) {
  ...
  const identity = compareRepositoryIdentity(facts, runSpec.repository);
