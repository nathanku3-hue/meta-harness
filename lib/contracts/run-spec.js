...
function validateRunSpec(spec, options = {}) {
  ...
  if (!isOrdinaryPlainObject(validation)
    ...
    for (let i = 0; i < validation.commands.length; i += 1) {
      const cmd = validation.commands[i];
      const cmdReasons = [];
      validateValidationCommand(cmd, i, cmdReasons, maxTimeout);
      reasons.push(...cmdReasons);
      if (cmdReasons.length > 0) continue;
      let commandId;
      try {
        commandId = commandIdFromSpecCommand(cmd);
      } catch (err) {
        reasons.push(reason(
          err.code || "COMMAND_ID_COMPUTE_FAILED",
          `validation.commands[${i}]: ${err.message}`,
        ));
        continue;
      }
      if (seenCommandIds.has(commandId)) {
        reasons.push(reason(
          ...
          `validation.commands[${i}] duplicates commandId of commands[${seenCommandIds.get(commandId)}]`,
        ));
      } else {
        seenCommandIds.set(commandId, i);
      }
    }
  }
