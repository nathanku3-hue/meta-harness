...
function strictFail(label, err) {
  return {
    ok: false,
    verdict: "FAILED",
    reasons: [reason(err.code || "STRICT_JSON", `${label}: ${err.message}`)],
  };
}
 ...
 */
function evaluateWorkspaceStart(input) {
  ...
  try {
    ...
    const s = strictFail("evaluateWorkspaceStart input", err);
    return fail(s.verdict, s.reasons);
  }
  if (!isOrdinaryPlainObject(envelope)) {
    return fail("FAILED", [reason("INPUT_REQUIRED", "evaluateWorkspaceStart input must be a plain object")]);
  }
