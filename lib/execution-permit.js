...
const { findExecutionClosureForDecision, findExecutionClosureForOrigin } = require("./execution-closure");
...
function validateAttemptEntry(value) {
  ...
  if (value.origin.type === "OWNER_GOAL") {
  ...
  } else if (value.origin.type === "REPO_OUTCOME") {
    exactKeys(value.origin, ["type", "outcomeDigest", "claimDigest"], "attemptEntry.origin");
    if (!isDigest(value.origin.outcomeDigest) || !isDigest(value.origin.claimDigest)) {
      fail("MH_ATTEMPT_ENTRY_ORIGIN", "AttemptEntry outcomeDigest and claimDigest must be sha256 digests");
    }
    ...
    fail("MH_ATTEMPT_ENTRY_ORIGIN", "AttemptEntry origin.type must be OWNER_GOAL, REPO_OUTCOME, or legacy REPO_DECISION");
  }
...
function attemptEntryOrigin(session, workspaceId) {
  if (!session?.origin) {
    fail("MH_ATTEMPT_ENTRY_SESSION", "AttemptEntry requires a sealed work-session origin");
  }
  ...
  return session.origin.type === "REPO_DECISION"
    ? { type: "REPO_DECISION", decisionDigest: session.origin.decisionDigest }
    : { type: "OWNER_GOAL", workspaceId };
}

function readAttemptEntry({ repositoryPath, session, workspaceId, generation, optional = false }) {
  const originPath = attemptEntryOrigin(session, workspaceId);
  const filePath = attemptEntryPath(repositoryPath, originPath, generation);
  let parsed;
  try {
    parsed = JSON.parse(fs.readFileSync(filePath, "utf8"));
  } catch (error) {
  ...
  const entry = validateAttemptEntry(parsed);
  if (entry.sessionDigest !== session.sessionDigest
      || entry.workspaceId !== workspaceId
      || entry.generation !== generation
      || entry.ordinal !== generation
      || JSON.stringify(entry.origin) !== JSON.stringify(session.origin)) {
    fail("MH_ATTEMPT_ENTRY_READ", "AttemptEntry path identity does not match the active work session");
  }
  return entry;
}

function readAttemptEntriesForSession({ repositoryPath, session, workspaceId }) {
  const entries = [];
  let sawGap = false;
  for (let generation = 1; generation <= session.maxAttempts; generation += 1) {
    const entry = readAttemptEntry({ repositoryPath, session, workspaceId, generation, optional: true });
    if (!entry) {
      sawGap = true;
      continue;
    }
    if (sawGap) {
      fail("MH_ATTEMPT_ENTRY_READ", "AttemptEntry generations must be contiguous from generation 1");
    }
    entries.push(entry);
  }
...
function readEnteredAttemptPermit(stateDirectory, attemptEntry) {
  const entry = validateAttemptEntry(attemptEntry);
  const parsed = readPersistedPermit(stateDirectory, { permitId: entry.permitId, permitDigest: entry.permitDigest });
  if (parsed.permitId !== entry.permitId
      || parsed.permitDigest !== entry.permitDigest
      || parsed.attemptId !== entry.attemptId
      || parsed.generation !== entry.generation
      || parsed.authority.sessionDigest !== entry.sessionDigest
      || parsed.authority.workspaceId !== entry.workspaceId) {
    fail("MH_ATTEMPT_ENTRY_MISMATCH", "persisted execution permit does not match its entered AttemptEntry");
  }
  return parsed;
}
...
function enterExecutionAttempt({ stateDirectory, permit, session, entryCapability = "CODE_PROPOSE", now = new Date() }) {
  ...
  const originPath = attemptEntryOrigin(session, validated.authority.workspaceId);
    ...
    return withWorldAuthorityLock(repositoryPath, () => {
      ...
      if (entry.ordinal > 1) {
        ...
        if (first.sessionDigest !== entry.sessionDigest || first.workspaceId !== entry.workspaceId) {
          fail("MH_ATTEMPT_ENTRY_CONTINUATION", "bounded repair must retain the claimed Outcome session and workspace authority");
        }
        if (findExecutionClosureForOrigin(repositoryPath, session.origin)) {
          fail("MH_ATTEMPT_ENTRY_CONTINUATION", "bounded repair cannot enter after aggregate Outcome execution closure");
        }
      }
      writeAuthorityCreateOnlyJson(filePath, entry, "MH_EXECUTION_PERMIT_REPLAY");
      return entry;
...
function assertExecutionPermitCurrent({
  ...
  if (!samePath(validated.authority.repositoryRoot, repositoryRoot)
      ...
      || validated.authority.baseHead !== session.base.commit
