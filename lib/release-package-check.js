...
function publishPackageChecks(targetRoot, packageInfo, lifecycleCheck, ids) {
  ...
  try {
    ...
    if (leaks.length > 0) checks.push(fail(ids.forbiddenPath, "forbidden-package-paths", `forbidden package paths included: ${leaks.join(", ")}`, "Update package files allowlist or npm ignore rules", { requiredForLocal: false, details: { leaks } }));
    else checks.push(pass(ids.forbiddenPath, "forbidden-package-paths", "package file list excludes local state, secrets, runtime data, demos, and dependency trees", { requiredForLocal: false, details: { file_count: dryCanon.paths.length } }));
    ...
    checks.push(pass(ids.tarballSmoke, "tarball-smoke", "packed tarball installs into an isolated temp project with --ignore-scripts", { requiredForLocal: false, details: { tarball: path.basename(tarballPath) } }));
    ...
    const smoke = spawnSync(process.execPath, [binPath, "--help"], { cwd: projectRoot, encoding: "utf8", shell: false, stdio: ["ignore", "pipe", "pipe"], timeout: 20_000 });
    if (smoke.error || smoke.status !== 0) {
      checks.push(fail(ids.cliSmoke, "cli-smoke", `installed CLI smoke failed: ${commandSummary(smoke)}`, "Fix package bin entry or CLI startup", { requiredForLocal: false }));
      return checks;
    }
    checks.push(pass(ids.cliSmoke, "cli-smoke", "installed CLI starts successfully from the packed tarball", { requiredForLocal: false, details: { bin: toSlash(path.relative(projectRoot, binPath)) } }));
    return checks;
  } finally {
