...
const { inspectPostPhaseReflectionGuidance } = require("./active-guidance");
...
const ACTIVE_GUIDANCE_GATE_FIRST_CONFLICTS = [
  { pattern: /\baudit\s+before\s+plan\b/i, detail: "active guidance requires audit before planning" },
  { pattern: /\baudit\s+after\s+(?:the\s+)?worker(?:'s)?\s+plan\b/i, detail: "active guidance requires audit after the worker plan" },
  { pattern: /\bwait(?:ing)?\s+for\s+(?:an?\s+)?audit\b/i, detail: "active guidance waits for audit before execution" },
  { pattern: /\bpending\s+(?:an?\s+)?audit(?:\s+approval)?\b/i, detail: "active guidance makes audit a pending gate" },
  { pattern: /\baudit\s+approval\s+before\s+(?:coding|implementation|execution)\b/i, detail: "active guidance requires audit approval before execution" },
  { pattern: /\bimplementation\s+hold\b[^\n]*\baudit\b/i, detail: "active guidance holds implementation for audit" },
  { pattern: /\bshow\s+(?:a\s+)?detailed\s+plan\b[^\n]*\bwait\b/i, detail: "active guidance requires a detailed plan and wait" },
  { pattern: /\bambiguity\s+confirmation\s+before\s+go\b/i, detail: "active guidance requires blanket ambiguity confirmation" },
  { pattern: /\bfirst[-\s]round\s+audit\b/i, detail: "active guidance requires a first-round audit" },
  { pattern: /\bsecond[-\s]round\b[^\n]*\baudit\b/i, detail: "active guidance requires a second audit round" },
  { pattern: /\b(?:do not|cannot|must not)\s+begin\b[^\n]*\buntil\s+(?:an?\s+)?audit\b/i, detail: "active guidance forbids execution until audit" },
  { pattern: /\bmandatory\s+review\s+before\s+all\s+new\s+work\b/i, detail: "active guidance requires universal review before new work" },
  { pattern: /\b(?:review|saw)\s+after\s+every\s+round\b/i, detail: "active guidance requires review or SAW after every round" },
  { pattern: /\bbig\s+change\b[^\n]*\barchitecture\b[^\n]*\b(?:pause|wait|stop)\b[^\n]*\b(?:go|approval|authorization)\b/i, detail: "active guidance turns big-change architecture review into a routine owner gate" },
  { pattern: /\b(?:after|following)\s+(?:the\s+)?architecture\b[^\n]*\b(?:pause|wait|stop)\b[^\n]*\b(?:go|approval|authorization)\b/i, detail: "active guidance pauses after architecture review for routine owner approval" },
  { pattern: /\bdecision\s+needed\s*:\s*(?:authorize|approve)\s+the\s+next\s+(?:packet|round|task|work)\b/i, detail: "active guidance requires routine owner approval before ordinary continuation" },
  { pattern: /\b(?:ask|request|require)\s+(?:the\s+owner\s+)?(?:for\s+)?(?:go|approval|authorization)\s+before\s+(?:ordinary|routine|reversible)\s+work\b/i, detail: "active guidance requires routine owner approval before ordinary continuation" },
];
const ACTIVE_GUIDANCE_GATE_FIRST_SAFE_LINES = [
  /\b(?:audit|review|plan)\b[^\n]*\bdoes not imply (?:a )?pause\b/i,
  /\bdo not\s+(?:wait for|request|require|pause for)\b[^\n]*\b(?:audit|approval|confirmation)\b/i,
  /\b(?:audit|review)\s+is not (?:a )?(?:route|phase|approval|gate|requirement)\b/i,
  /\bwithout\s+(?:waiting for|requesting)\b[^\n]*\b(?:audit|approval|confirmation)\b/i,
  /\b(?:do not|never)\s+(?:require|mandate)\b[^\n]*\b(?:review|saw)\b[^\n]*\b(?:before\s+all\s+new\s+work|after\s+every\s+round)\b/i,
  /\b(?:do not|never)\s+(?:pause|wait|stop)\b[^\n]*\b(?:after|following)\s+(?:the\s+)?architecture\b[^\n]*\b(?:go|approval|authorization)\b/i,
  /\b(?:do not|never)\s+(?:ask|request|require)\b[^\n]*\b(?:go|approval|authorization)\b[^\n]*\bbefore\s+(?:ordinary|routine|reversible)\s+work\b/i,
];
...
function gateFirstGuidanceConflicts(text) {
  const details = [];
  for (const line of String(text || "").split(/\r?\n/)) {
    if (!line.trim()) continue;
    if (ACTIVE_GUIDANCE_GATE_FIRST_SAFE_LINES.some((pattern) => pattern.test(line))) continue;
    for (const conflict of ACTIVE_GUIDANCE_GATE_FIRST_CONFLICTS) {
      if (conflict.pattern.test(line)) details.push(conflict.detail);
    }
  }
  return details;
}
...
function checkTemplateSync({ sourceRoot, targetRoot }) {
  ...
  const activeGuidance = inspectPostPhaseReflectionGuidance({ sourceRoot, targetRoot });
  if (activeGuidance) {
    items.push(activeGuidance);
  }
...
function activeHostGuidanceFilesUnder(targetRoot) {
  const resolvedTarget = path.resolve(targetRoot);
  return ACTIVE_GUIDANCE_PATHS
    .map((relativePath) => path.join(resolvedTarget, ...relativePath.split("/")))
    .filter((filePath) => isRegularFile(filePath))
    .sort((left, right) => relativePath(resolvedTarget, left).localeCompare(relativePath(resolvedTarget, right)));
}

function activeGuidanceFilesUnder(targetRoot) {
  const resolvedTarget = path.resolve(targetRoot);
  const files = activeHostGuidanceFilesUnder(targetRoot);
...
function scanHostGuidanceActionLawConflicts({ targetRoot }) {
  const resolvedTarget = path.resolve(targetRoot);
  const files = activeHostGuidanceFilesUnder(targetRoot);
  const items = [];
  for (const filePath of files) {
    const text = fs.readFileSync(filePath, "utf8");
    for (const detail of gateFirstGuidanceConflicts(text)) {
      items.push({
        status: "REJECTED",
