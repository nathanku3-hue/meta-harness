...
const ACTIVE_GUIDANCE_CONFLICTS = [
  {
    pattern: /final responses? must use the Ship-Fast PM Brief/i,
    detail: "active guidance requires the worker-report artifact as the final chat response",
  },
  {
    pattern: /final (?:chat )?(?:answer|response) must start with `?Outcome`?, `?Round`?, `?Progress`?, and `?Confidence`?/i,
    detail: "active guidance requires artifact metadata at the start of final chat",
  },
];
...
function scanContracts({ targetRoot }) {
  const resolvedTarget = path.resolve(targetRoot);
  const files = markdownFilesUnder(targetRoot, [
    ".meta-harness/contracts",
    ".meta-harness/templates/contracts",
    ".meta-harness/workers",
  ]);
  const activeGuidanceFiles = ACTIVE_GUIDANCE_PATHS
    .map((relative) => path.join(resolvedTarget, ...relative.split("/")))
  ...
  return {
    ...
    checked: files.length + activeGuidanceFiles.length,
    items,
  };
}
