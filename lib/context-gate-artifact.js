...
function activeDimensions(options = {}) {
  if (Array.isArray(options.dimensions)) {
    return options.dimensions;
  }
  return contextGateGovernance(options.governance).dimensions;
}

function verdictFor(scores, structuralHardBlockers, evidenceGapDimensions, unknownDimensions, options = {}) {
  const dimensions = activeDimensions(options);
  ...
  const scoreValues = dimensions.map((dimension) =>
    Number.isInteger(scores[dimension]) ? scores[dimension] : 1
  );
...
function artifactFromEvaluations(input, options = {}) {
  const {
    roundId,
    generatedAt,
    transition,
  ...
  const governance = options.governance === undefined ? inputGovernance : options.governance;
  const explicitDimensions = options.dimensions === undefined ? inputDimensions : options.dimensions;
  const dimensions = activeDimensions({ governance, dimensions: explicitDimensions });
  ...
  for (const evaluation of evaluations) {
    if (!dimensionSet.has(evaluation.dimension)) {
      continue;
    }
...
function finalizeArtifact(artifact, options = {}) {
  const final = {
    ...artifact,
...
function renderContextGateMarkdown(artifact, options = {}) {
  const dimensions = activeDimensions(options);
  ...
  const overrideLines = artifact.override
    ...
    : "- none";
  const fingerprintLines = artifact.fingerprint
    ? [
        `- governance_hash: ${artifact.fingerprint.governance_hash}`,
        `- evaluation_hash: ${artifact.fingerprint.evaluation_hash}`,
        `- evidence_hash: ${artifact.fingerprint.evidence_hash}`,
      ].join("\n")
    : "- none";
