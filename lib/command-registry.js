...
const commandSpecs = [
  {
    name: "work",
    primary: false,
    summary: "Carry one accepted product result through coding and validation.",
    usage: [
      "meta-harness work <repository> --goal <result> [--allow <path>] [--base <HEAD|ref|oid>] [--dry-run] [--json]",
      "meta-harness work <repository> --session <work-session.json> [--dry-run] [--json]",
      "meta-harness work <repository> [--dry-run] [--json]",
      "meta-harness work <repository> --resume [--json]",
    ],
    handler: "./commands/work",
  },
  {
    name: "init",
    summary: "Create advisory repository-local harness files.",
    usage: "meta-harness init",
    handler: "./commands/init",
  },
  {
    name: "authority",
    summary: "Bootstrap the external create-only owner authority pin.",
    usage: "meta-harness authority bootstrap --owner-public-key-file <path> [--json]",
    handler: "./commands/authority",
  },
  {
    name: "execute",
    summary: "Execute one v2 semantic-kernel change; pre-0.4 requests are unsupported.",
  ...
  },
  {
    ...
    usage: "meta-harness worker-report [worker-id] --stream <stream> --task <user-journey> --result <observable-result> --human-summary <user-accomplished-or-learned> --blocker <product-blocker|none> --next-action <next-executable-product-action> --outcome <DONE|PARTIAL_WITH_EXPLICIT_SCOPE|REJECTED> --requested-work-type <type> --actual-work-type <type>",
  ...
  },
  {
    ...
    usage: [
      ...
      "meta-harness context packet <round-id> --for <review|planning> [--json] [--out <path>]",
  ...
  {
    name: "layout",
    summary: "Create a hash-bound layout manifest or close one provably safe target.",
    usage: [
      "meta-harness layout manifest --target <repo> --output <path> [--copy <path>] [--json]",
      "meta-harness layout close --target <path> --manifest <path> --receipt <path> [--manifest-copy <path>] [--json]",
    ],
    handler: "./commands/layout",
  },
  {
    name: "portfolio",
    summary: "Run a report-only audit against the external portfolio custody policy.",
    usage: "meta-harness portfolio audit --policy <path> --output <path> [--copy <path>] [--json]",
    handler: "./commands/portfolio",
  },
  {
    name: "dirty",
  ...
  {
    name: "release",
    summary: "Create or verify one immutable release candidate without rebuilding during verification.",
    usage: "meta-harness release candidate <create|verify-preterminal|verify-publication> [evidence options] | meta-harness release publish --from-env --json",
...
function commandMetadata(spec) {
  const primary = spec.primary === true;
  return {
    name: spec.name,
    public: primary,
...
function commandUsages({ advanced = false } = {}) {
  return commandSpecs
    .filter((spec) => advanced || spec.primary === true)
    .flatMap((spec) => Array.isArray(spec.usage) ? spec.usage : [spec.usage]);
}
...
function renderHelp({ advanced = false } = {}) {
  if (!advanced) {
    return `meta-harness

A coding system that carries one accepted product result into code, validation, repair, and a locally banked result.
...
State the software result once. With no result, Meta-Harness continues the mechanically correct active work or stops.
Normal work does not require workflow commands, session selection, resume flags, validation selection, or commit decisions.
Use \`meta-harness inspect\` only when you want a coarse diagnostic view.
`;
  }
  return `meta-harness diagnostics

The product CLI has no advanced workflow console.

Usage:
  meta-harness inspect

Lifecycle commands, actor routing, session selection, custody controls, and validation controls are internal implementation surfaces.
`;
}
