...
A coding system that carries one accepted product result into code, validation, and an observable result.

Usage:
  ${commandUsages().join("\n  ")}

The default surface is product-facing: outcome, current state, action, result, blocker, and next step.
Run \`meta-harness help --advanced\` for evidence, custody, release, and maintenance commands.
`;
  }
  return `meta-harness advanced commands

Internal evidence, custody, release, and maintenance surfaces.

Usage:
  ${commandUsages({ advanced: true }).join("\n  ")}
