...
function evaluateProductOutcome(state) {
  ...
  if (state.events.length > 0) evidence.push(".meta-harness/events.jsonl advisory history");
  ...
  if (/\b(exit criteria|acceptance|user flow|for whom|why|outcome)\b/i.test(`${goal}\n${currentTruth}`)) score += 1;
