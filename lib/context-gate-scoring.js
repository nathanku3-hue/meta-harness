...
const { EXECUTION_TRANSITIONS } = require("./context-gate-constants");
...
function evaluateProductOutcome(state) {
  const goal = sectionAfterLabel(state.statusText, "Goal");
  const currentTruth = sectionAfterLabel(state.statusText, "Current truth");
  const nextAction = sectionAfterLabel(state.statusText, "Next action");
  const evidence = [];
...
function evaluateScopeBoundary(state, transition) {
  const combined = `${state.statusText}\n${state.decisionLogText}`.toLowerCase();
  const stopCriteria = sectionAfterLabel(state.statusText, "Stop criteria");
  ...
  if (!hasOutOfScope && EXECUTION_TRANSITIONS.has(transition)) {
...
function evaluateEvidencePlan(state) {
  ...
  const statusEvidence = sectionAfterLabel(state.statusText, "Last verified");
  const combined = `${state.statusText}\n${state.readmeText}`;
  const evidence = [];
...
function evaluateHandoffCompleteness(state) {
  const nextAction = sectionAfterLabel(state.statusText, "Next action");
  const stopCriteria = sectionAfterLabel(state.statusText, "Stop criteria");
  const pendingDecisions = sectionAfterLabel(state.statusText, "Pending human decisions");
...
function evaluateContext(state, transition, now) {
  return [
    evaluateProductOutcome(state),
    evaluateScopeBoundary(state, transition),
