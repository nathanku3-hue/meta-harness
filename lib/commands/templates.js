...
module.exports = async function runTemplates(args, context) {
  ...
  if (action === "install") {
    ...
    let preflight;
    try {
      preflight = inspectPostPhaseReflectionGuidance({
        sourceRoot,
        targetRoot: context.cwd,
      });
    } catch (error) {
      fail(`Cannot install active guidance: ${error.message}`);
    }
    if (preflight && preflight.status === "REJECTED") {
      fail(`Cannot install active guidance: ${preflight.detail}`);
    }
    ...
    let activeGuidance;
    try {
      activeGuidance = installPostPhaseReflectionGuidance({
        sourceRoot,
        targetRoot: context.cwd,
      });
    } catch (error) {
    ...
    writeLine(context, `- ${activeGuidance.path} (${activeGuidance.action})`);
