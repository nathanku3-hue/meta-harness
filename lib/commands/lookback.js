...
const { semanticStatusField } = require("../context-status-fields");
const {
  eventTime,
  fieldFromLatest,
  harnessPath,
...
function renderLookback(context) {
  ...
  let statusGoal = "";
  try {
    statusGoal = semanticStatusField(
      context.fs.readFileSync(harnessPath(context, "status.md"), "utf8"),
      "Goal",
    );
  } catch {
    statusGoal = "";
  ...
  const lines = [
    ...
    "## Original Goal",
    "",
    initEvent.goal || statusGoal || "Not set.",
    "",
    "## Timeline",
    "",
  ];
