...
module.exports = async function runPoll(args, context) {
  ...
  if (options.rollup) {
    ...
    if (operatorPlanVerify) {
      const readRes = readOperatorExecutionPlanArtifact(context, operatorPlanVerify, packetBoundaryRepos);
      operatorExecutionPlanArtifactValidation = operatorPlanArtifactValidation.buildOperatorExecutionPlanArtifactValidation({
        ...
        artifact: readRes.artifact,
        readError: readRes.readError,
        pathOk: true,
        childPathOk: true,
      });
    }
    // Attach validation for visibility when requested (does not alter derived plan or key order assumptions for non-verify paths)
  ...
  if (options.writeOperatorExecutionPlan !== undefined) {
    fail("--write-operator-execution-plan requires poll --rollup");
  }
  if (options.verifyOperatorExecutionPlan !== undefined) {
    fail("--verify-operator-execution-plan requires poll --rollup");
  }
