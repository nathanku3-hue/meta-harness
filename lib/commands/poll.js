...
const { attachEntryAuthorityToRollup } = require("../entry-authority");
const { attachWorkerEntryGateToRollup } = require("../worker-entry-gate");
...
function readEntryAuthorityRunSpecOption(options, context) {
  const inlineInput = optionValue(options.entryAuthorityRunSpec);
  const inputFile = optionValue(options.entryAuthorityRunSpecFile);
  if (inlineInput !== undefined && inputFile !== undefined) {
    fail("Use only one of --entry-authority-run-spec or --entry-authority-run-spec-file");
  }
  if (inlineInput !== undefined) {
    if (inlineInput === true || String(inlineInput).trim() === "") {
      fail("--entry-authority-run-spec requires a controller input JSON object");
    }
    return parseApprovalReceiptJson(String(inlineInput), "--entry-authority-run-spec");
  }
  if (inputFile !== undefined) {
    if (inputFile === true || String(inputFile).trim() === "") {
      fail("--entry-authority-run-spec-file requires a file path");
    }
    const filePath = path.resolve(context.cwd, String(inputFile));
    if (!fileExists(filePath)) {
      fail(`--entry-authority-run-spec-file not found: ${inputFile}`);
    }
    return parseApprovalReceiptJson(readText(filePath), "--entry-authority-run-spec-file");
  }
  return undefined;
}
...
module.exports = async function runPoll(args, context) {
  ...
  if (options.rollup) {
    ...
    // 22A-H + 22B: always emit execution_readiness then worker_entry_gate on verify-op
    if (operatorExecutionPlanArtifactValidation) {
      ...
      const selectedPath = rollup.selected_repo_resolution?.ok === true
        ? path.resolve(context.cwd, rollup.selected_repo_resolution.path)
        : context.cwd;
      const targetPaths = Array.isArray(opArtifactReadRes?.artifact?.operator_execution_plan?.target_paths)
        ? opArtifactReadRes.artifact.operator_execution_plan.target_paths
        : [];
      attachEntryAuthorityToRollup(rollup, {
        checkoutPath: selectedPath,
        controllerInput: readEntryAuthorityRunSpecOption(options, context),
        productPaths: targetPaths,
      });
