...
} = require("../research-decision-handoff");
const {
...
function runResearchHandoff(options, context) {
  const report = optionValue(options.report);
  if (!report || report === true) fail("mcp research handoff requires --report");
  const question = optionValue(options.question);
  if (!question || question === true) fail("mcp research handoff requires --question");
  const handoff = buildResearchDecisionHandoff({
    question,
    sourceReportPath: String(report),
    reportText: readReport(context, report),
  });
  if (options.json) writeJson(context, handoff);
  else writeOut(context, renderResearchDecisionHandoffMarkdown(handoff));
  return { exitCode: 0 };
}
...
function runResearch(args, context) {
  ...
  if (positional[0] === "handoff") return runResearchHandoff(options, context);
  fail(`unknown mcp research action: ${positional[0] || "missing"}`);
}

function runPublishProcess(args, context) {
  const { options } = parseArgs(args);
  const { processPublishQueue } = require("../publish-processor");
  const dryRun = Boolean(options["dry-run"] || options.dryRun);
  const result = processPublishQueue(context.cwd, { dryRun });
  writeLine(context, `Processed ${result.processed} publishing job(s).`);
  return { exitCode: 0 };
}
...
module.exports = async function runMcp(args, context) {
  ...
  if (area === "publish-process") return runPublishProcess(rest, context);
