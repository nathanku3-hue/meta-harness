...
const { isDrainCompleteResult, runWork } = require("../work-loop");
const {
...
function automaticRequest(repositoryPath, productResult = null, { env, model, signal } = {}) {
  ...
  const explicitResult = typeof productResult === "string" && productResult.trim() !== ""
    ? productResult.trim()
    : null;
  const ingress = explicitResult ? null : readOwnerGoalIngress(repositoryPath, { optional: true });
  const journey = reduceJourneyState({ ownerResult: explicitResult || ingress?.content || null, activeSession: null });
...
async function runAutomaticProductResult(productResult, context) {
  ...
  if (request.type === "ENDGAME_INCOMPLETE") {
    ...
    writeLine(
      context,
      `Replan: ${missing.length} owner-required product destination${missing.length === 1 ? "" : "s"} remain without durable PROVEN coverage.`,
    );
    return { exitCode: 1 };
  }
