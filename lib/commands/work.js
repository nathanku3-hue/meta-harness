...
function resolveSession(context, repositoryPath, options) {
  ...
  if (decisionPlaneMode === "resume") {
    return assertRepoDecisionSessionCurrent(repositoryPath, loadLatestWorkSession(repositoryPath));
  }
  if (decisionPlaneMode === "decision") {
    return compileRepoDecisionSession(repositoryPath);
  }
  ...
  if (options.resume !== undefined) {
    const resumed = loadLatestWorkSession(repositoryPath);
    return repoDecisionDigestFromSession(resumed)
      ? assertRepoDecisionSessionCurrent(repositoryPath, resumed)
      : resumed;
  }
  if (options.session !== undefined) {
    const value = optionValue(options.session);
    if (!value || value === true) fail("--session requires a JSON file");
    return bindWorkSessionToRepository(
  ...
  if (fs.existsSync(conventional)) {
    return bindWorkSessionToRepository(loadWorkSession(conventional), repositoryPath);
  }
...
module.exports = async function commandWork(argv, context) {
  ...
  const result = await runWork({
    ...
    env: context.env,
    onAttemptStart: ({ executionPermit }) => recordRepoDecisionAttempt(repositoryPath, session, { executionPermit }),
  });
  recordRepoDecisionResult(repositoryPath, session, result);
