...
const { repoControlPlaneEnabled } = require("../repo-proposal-set");
...
function resolveWorkRequest(context, repositoryPath, options) {
  if (repoControlPlaneEnabled(repositoryPath)) {
    if (options.goal !== undefined || options.session !== undefined || options.allow !== undefined || options.base !== undefined) {
      fail(
        "this repository has opted into repo proposal authority; repo-owned material work comes from current .meta-harness/repo-proposals.json",
      );
    }
    if (options.dryRun !== undefined) {
      fail("repo-owned parallel work does not expose a dry-run lifecycle; use current proposal/World diagnostics instead");
    }
...
function renderHuman(context, result) {
  if (result.schemaVersion === "repo-work-wave-result/v1") {
    ...
    const needsReplan = result.outcomes.filter((entry) => ["BLOCKED", "INVALIDATED_REPLAN"].includes(entry.state)).length;
    ...
    if (running > 0 && needsReplan === 0) {
      writeLine(context, `Working — ${running} repository outcome${running === 1 ? " is" : "s are"} already executing under durable Claim authority.`);
      return;
    }
    writeLine(context, "Blocked: repository reconciliation requires fresh current-World proposals or resolution of the reported outcome blocker.");
    return;
  }
  if (result.outcome === "DONE") {
...
function automaticRequest(repositoryPath, productResult = null, { env, model } = {}) {
  ...
  if (!productResult && !activeSession && repoControlPlaneEnabled(repositoryPath)) {
    return { type: "REPO_WAVE" };
  }
  const journey = reduceJourneyState({ ownerResult: productResult, activeSession });

  if (journey.next.kind === "OWNER_INPUT") {
    return { type: "OWNER_INPUT", journey };
  }
  if (journey.next.operation === "RESUME") {
    return dispatch(activeSession);
  }
