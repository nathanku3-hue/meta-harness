...
const ENV = Object.freeze({
  ...
  expectedOwnerKeyId: "META_HARNESS_EXPECTED_OWNER_KEY_ID",
...
function loadPortableTerminalRelease(context, intentPath, assetRoot, expectedOwnerKeyId) {
  ...
  if (owner.pin.ownerKeyId !== expectedOwnerKeyId) {
    fail("portable owner pin differs from trusted publisher owner-key anchor");
  }
...
function trustedOwnerKeyId(options, fromEnv) {
  const value = fromEnv
    ? process.env[ENV.expectedOwnerKeyId]
    : optionValue(options.expectedOwnerKeyId);
  if (typeof value !== "string" || !/^sha256:[a-f0-9]{64}$/.test(value)) {
    fail(`${fromEnv ? ENV.expectedOwnerKeyId : "--expected-owner-key-id"} must be one sha256 digest`);
  }
  return value;
}
...
async function handleIntentPrepare(context, options) {
  assertAllowedOptions(options, new Set([
  ...
  const outputPath = prepareCreateOnlyOutput(context, optionValue(options.output), "publication intent draft output");
  if (path.basename(outputPath) !== "publication-intent-unsigned.json") {
    fail("publication intent draft output filename must be publication-intent-unsigned.json");
  }
  ...
  const prepared = preparePublicationIntent({
    repositoryPath: context.cwd,
    sliceAuthorization: terminal.authorization,
    packageCandidate: terminal.packageCandidate,
    releaseCandidate: terminal.releaseCandidate,
  ...
  writeCreateOnlyJson(outputPath, prepared.publicationIntentDraft);
  printResult(context, {
    schemaVersion: "release-publication-intent-prepare-result/v1",
    ok: true,
    releaseCandidateDigest: prepared.publicationIntentDraft.releaseCandidateDigest,
    assetManifestDigest: prepared.publicationIntentDraft.assetManifestDigest,
    assetCount: prepared.publicationIntentDraft.assets.length,
...
async function handleIntentCommit(context, options) {
  assertAllowedOptions(options, new Set([
    ...TERMINAL_EVIDENCE_OPTIONS,
    "controllerInstanceId",
    "publicationIntent",
  ...
  const terminal = loadTerminalRelease(context, options, false);
  const signedIntent = readRegularJson(
    context,
    optionValue(options.publicationIntent),
    "signed PublicationIntent",
  ).value;
  const recorded = recordPublicationIntent({
    ...
    releaseCandidate: terminal.releaseCandidate,
    terminalAssessment: terminal.terminalAssessment,
    publicationIntent: signedIntent,
    controllerInstanceId: controllerInstance(options, false),
  });
  writeCreateOnlyJson(outputPath, recorded.publicationIntent);
  printResult(context, {
    schemaVersion: "release-publication-intent-commit-result/v1",
    ok: true,
...
function portableInputs(context, options, fromEnv) {
  const expectedOwnerKeyId = trustedOwnerKeyId(options, fromEnv);
  ...
  return {
    ...
    assetRoot,
    expectedOwnerKeyId,
    portable: loadPortableTerminalRelease(context, intentPath, assetRoot, expectedOwnerKeyId),
  };
}
...
async function handleIntentVerify(context, options) {
  ...
  const result = verifyPublicationIntent({
    ...
    ownerPin: portable.owner.pin,
    expectedOwnerKeyId: portable.publicationIntent.ownerKeyId,
  });
  printResult(context, { ...result, ok: true }, options.json !== undefined);
  return { exitCode: 0 };
}
...
async function handleReconcile(context, options) {
  ...
  const publicationIntent = validatePublicationIntent(
    ...
    ).value,
    {
      ...
      releaseCandidate: terminal.releaseCandidate,
      terminalAssessment: terminal.terminalAssessment,
      ownerPin: terminal.owner.pin,
      expectedOwnerKeyId: terminal.owner.pin.ownerKeyId,
    },
  );
...
module.exports = async function commandRelease(argv, context) {
  ...
  if (extra.length > 0) {
    fail("usage: meta-harness release candidate <create|verify-preterminal|verify-publication> | release intent <prepare|commit|verify> | release publish | release reconcile");
  }
  ...
  if (action === "intent") {
    if (subaction === "prepare") return handleIntentPrepare(context, options);
    if (subaction === "commit") return handleIntentCommit(context, options);
