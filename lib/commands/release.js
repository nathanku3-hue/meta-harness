...
function loadTerminalRelease(context, options, fromEnv) {
  ...
  assertControllerCommitted(state.stateRoot, "integrated-candidate/v1", frozen.integrated.candidateDigest, frozen.integrated);
  assertControllerCommitted(state.stateRoot, "package-candidate/v1", frozen.packageCandidate.packageCandidateDigest, frozen.packageCandidate);
  assertControllerCommitted(state.stateRoot, "release-candidate/v1", frozen.releaseCandidate.releaseCandidateDigest, frozen.releaseCandidate);
  assertControllerCommitted(state.stateRoot, "black-box-proof/v1", proof.proofDigest, proof);
