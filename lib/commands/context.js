...
const PACKET_AUDIENCES = new Set(["review", "planning"]);
...
async function runCheck(args, context) {
  ...
  const result = await runContextGateCheck({
    ...
    overrideContextGate,
    enforceTruthPreGate: true,
    options,
  });

  if (overrideContextGate && !result?.truth_blocked) {
  ...
  return { exitCode: result?.truth_blocked ? 1 : 0 };
}
...
async function runPacket(args, context) {
  ...
  if (!audience || audience === true) fail("context packet requires --for <review|planning>");
  if (audience === "worker") {
    fail("standalone worker context packets are retired; `meta-harness work` compiles worker authority from a single-use ExecutionPermit");
  }
  if (!PACKET_AUDIENCES.has(audience)) {
    fail(`invalid --for value "${audience}". Expected review or planning`);
  }
