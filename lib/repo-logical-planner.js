...
function buildLogicalPlannerPrompt(plannerInput) {
  ...
  return [
    "LOGICAL_PLANNER_AUTODISPATCH_V4",
    ...
    "- REPLAN_REQUIRED, INVALIDATED_REPLAN, and EXECUTION_ABORTED handoffs are autonomous continuation obligations, not owner work.",
    "- Use continuesFromTransitionDigests only as exact continuation lineage to autonomous handoffs exposed in this planner input. It does not assert that predecessor blockers are resolved.",
    "- Multiple independently valuable Outcomes may continue the same transition, and one Outcome may continue multiple transitions when that is genuinely the smallest coherent product action.",
    "- An activeContinuationClaimDigests entry means a durable continuation already covers that handoff; do not duplicate it merely to satisfy continuation, though additional independent positive-value successors remain admissible.",
    "- Never continue from BLOCKED or OWNER_REQUIRED through continuesFromTransitionDigests.",
    ...
    "- Empty proposals is valid only when this exact truth + objective epoch has neither an uncovered autonomous continuation obligation nor any positive-value fresh Outcome.",
    "",
    ...
    "",
    "Return planner-candidate-batch/v3 only.",
  ].join("\n");
}
