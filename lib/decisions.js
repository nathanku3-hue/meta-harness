...
function validateInbox(rawInbox) {
  ...
  for (const decision of normalized) {
    ...
    if (seenStateHashes.has(decision.state_hash)) {
      fail(`duplicate decision state_hash: ${decision.state_hash}`);
    }
    seenIds.set(decision.id, decision);
    seenStateHashes.set(decision.state_hash, decision);
  }
...
function upsertDecision(inbox, input) {
  ...
  const existingState = inbox.decisions.find((decision) => decision.state_hash === identityData.identity.source_state_hash);
  if (existingState) {
    fail(`duplicate decision state_hash: ${identityData.identity.source_state_hash}`);
  }
  const decision = {
