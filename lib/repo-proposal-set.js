...
const REPO_PROPOSAL_SET_SCHEMA = "repo-proposal-set/v1";
const REPO_PROPOSAL_SET_DOMAIN = "meta-harness-repo-proposal-set-bytes/v1";
...
function validateProposal(value, index) {
  ...
  validateResolvedBase(value.base, `${label}.base`);
...
function loadRepoProposalSet(repositoryPath, worldState) {
  if (!worldState?.head || !worldState?.world) {
    fail("MH_REPO_PROPOSAL_WORLD", "loading repo proposals requires the authoritative current World state");
  }
...
function prepareRepoProposal(repositoryPath, loadedProposalSet, proposal, { outcome = null } = {}) {
  ...
  return Object.freeze({ proposal: value, outcome: preparedOutcome, productProofSpec });
}
