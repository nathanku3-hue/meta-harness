...
function prepareRepoProposal(repositoryPath, loadedProposalSet, proposal, { outcome = null } = {}) {
  ...
  const semanticAuthority = compileSemanticAuthority({ productDirection: loadedProposalSet.direction });
  ...
  return Object.freeze({ proposal: value, outcome: preparedOutcome, productProofSpec, base, semanticAuthority });
}
...
function admitPreparedRepoProposal(repositoryPath, loadedProposalSet, prepared, { now = new Date() } = {}) {
  ...
  return acquireOutcomeClaimSession({
    ...
    buildSession: (claim) => sealWorkSession({
      ...
      semanticProjection: semanticProjection(prepared.semanticAuthority),
      endgameProjection: endgameProjection(prepared.semanticAuthority),
      origin: {
        type: "REPO_OUTCOME",
        outcomeDigest: prepared.outcome.outcomeDigest,
