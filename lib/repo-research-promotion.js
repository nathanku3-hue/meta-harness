...
const { runEphemeralStructuredModel, throwIfDrainRequested } = require("./ephemeral-structured-model");
  ...
  properties: {
    ...
    findings: {
      ...
      items: {
        ...
        properties: {
          kind: { enum: RESEARCH_FINDING_KINDS },
...
async function promoteResearchSource({
  ...
  const parent = fs.mkdtempSync(path.join(os.tmpdir(), "meta-harness-research-"));
...
async function ensureCurrentResearchPromotions({
  ...
  const sources = enumerateResearchSourceOccurrences({ repositoryPath, productCommit: current.head.productCommit });
...
function projectCurrentPromotedResearch({ repositoryPath, productCommit }) {
  const sources = enumerateResearchSourceOccurrences({ repositoryPath, productCommit });
  const projected = [];
  for (const source of sources) {
    const promotion = readResearchPromotion(repositoryPath, source.contentDigest, { optional: true });
    if (!promotion) {
      fail("MH_RESEARCH_PROMOTION_MISSING", "current committed research source has no canonical promotion", {
        sourcePath: source.path,
        contentDigest: source.contentDigest,
      });
    }
  ...
  projected.sort((left, right) => (
    left.sourcePath.localeCompare(right.sourcePath) || left.findingDigest.localeCompare(right.findingDigest)
  ));
