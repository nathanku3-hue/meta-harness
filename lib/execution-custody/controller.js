...
function createExecutionCustodyController(config) {
  ...
  const requiredWorkspaceRoot = path.join(repositoryPath, ".worktrees");
  fs.mkdirSync(stateRootInput, { recursive: true });
  ensureLocalWorktreesExclude(repositoryPath);
  fs.mkdirSync(requiredWorkspaceRoot, { recursive: true });

  const stateRoot = canonicalExistingRoot(stateRootInput, "stateRoot");
  const workspaceRoot = canonicalExistingRoot(requiredWorkspaceRoot, "workspaceRoot");
  let suppliedWorkspaceRoot;
  try {
    suppliedWorkspaceRoot = canonicalExistingRoot(workspaceRootInput, "workspaceRoot");
  } catch {
    throw codedError(
      "CUSTODY_WORKSPACE_ROOT",
      "workspaceRoot must equal trustedRepository.repositoryPath/.worktrees",
    );
  }
  if (!pathsEqual(suppliedWorkspaceRoot, workspaceRoot)) {
    throw codedError(
      "CUSTODY_WORKSPACE_ROOT",
      "workspaceRoot must equal trustedRepository.repositoryPath/.worktrees",
    );
