...
function revalidateAgentIdentity(bound) {
  ...
  const nodeLstat = fs.lstatSync(nodeReal);
  if (nodeLstat.isSymbolicLink() || !nodeLstat.isFile()) {
    throw codedError("CUSTODY_AGENT_NODE_FILE", "node executable must be a regular non-symlink file");
  }
  const nodeSha = sha256File(nodeReal);
  if (nodeSha !== bound.expectedNodeSha256) {
    throw codedError("CUSTODY_AGENT_NODE_HASH", "node executable sha256 mismatch");
  }
...
function bindAgentProgram(codexProgram, { sourceEnv } = {}) {
  ...
  if (!isNonEmptyString(codexProgram.expectedNodeSha256)
    || !/^[a-f0-9]{64}$/.test(codexProgram.expectedNodeSha256)) {
    throw codedError("CUSTODY_AGENT_NODE_HASH_SHAPE", "expectedNodeSha256 must be 64 hex chars");
  }
  ...
  const nodeLstat = fs.lstatSync(nodeRealPath);
  if (nodeLstat.isSymbolicLink() || !nodeLstat.isFile()) {
    throw codedError("CUSTODY_AGENT_NODE_FILE", "node executable must be a regular non-symlink file");
  }
  const nodeSha = sha256File(nodeRealPath);
  if (nodeSha !== codexProgram.expectedNodeSha256) {
    throw codedError(
      "CUSTODY_AGENT_NODE_HASH_MISMATCH",
      "node sha256 does not match expectedNodeSha256",
      { expected: codexProgram.expectedNodeSha256, actual: nodeSha },
    );
  }
  ...
  return {
    ...
    nodeRealPath,
    expectedNodeSha256: codexProgram.expectedNodeSha256,
