...
/** Attempt preparation, receipt-first replay, and immutable claim publication. */
...
} = require("../contracts/execution-readiness-facts");
const { authorizeAttempt } = require("../contracts/authorize");
const {
  computeWorkspacePolicyDigest,
} = require("../contracts/attempt-authorization");
const {
  sealWorkspaceAttestation,
  isPathUnderRoot,
} = require("../contracts/workspace-attestation");
const { evaluateWorkspaceStart } = require("../contracts/workspace-start");
...
const { implementAfterClaim } = require("./implement");
const {
  lookupStoredReceipt,
...
const {
  ...
  gitPath,
} = require("../worktree-custody");

const CLAIM_SCHEMA = "execution-custody-claim/v1";
...
function inspectExistingAttempt(ctx, receipt, authReqHex) {
  ...
  if (!isPlainObject(claim)
    ...
    throw codedError("CUSTODY_STATE_CORRUPT", "claim bindings invalid for this request");
  }
  assertClaimDigest(claim);

  if (!journalRead.exists) {
    throw codedError(
      "CUSTODY_CLAIM_WITHOUT_JOURNAL",
  ...
  if (!isPlainObject(journal)
    ...
    throw codedError("CUSTODY_STATE_CORRUPT", "journal bindings invalid for claim");
  }
  ...
  if (journal.terminal === true) {
    return {
      ...
      terminal: true,
      restart: false,
      code: "CUSTODY_PRIOR_TERMINAL_FAILURE",
  ...
  return {
    ...
    terminal: false,
    restart: false,
    code: withinWindow ? null : "CUSTODY_INCOMPLETE_RECEIPT_EXPIRED",
    authorizationRequestDigest: receipt.authorizationRequestDigest,
    authorizationReceiptDigest: receipt.receiptDigest,
    runSpecDigest: receipt.runSpecDigest,
...
function replayVerified(ctx, receipt, claim, journal, authReqHex) {
  ...
  if (!isNonEmptyString(journal.verifiedHeadRevision)
    ...
    throw codedError("CUSTODY_STATE_CORRUPT", "verified journal missing required terminal fields");
  }
...
async function executeAttempt(ctx, request) {
  ...
  try {
    ...
    if (runSpec.repository.repositoryId !== repositoryId) {
      throw codedError(
        "CUSTODY_REPOSITORY_ID_MISMATCH",
    ...
    if (storedReceipt.exists) {
      ...
      throw codedError(
        "CUSTODY_RECEIPT_WITHOUT_ATTEMPT",
    ...
    const { boundAgent, boundValidation } = ctx.bindExecutionTools();
    const executionCtx = { ...ctx, boundAgent, boundValidation };
    assertExactValidationCommandBinding(runSpec, boundValidation.expectedCommands);
    ...
    if (live.objectFormat !== runSpec.repository.objectFormat) {
      throw codedError("CUSTODY_OBJECT_FORMAT", "live objectFormat does not match runSpec");
    }
    if (live.head !== runSpec.repository.expectedBaseRevision) {
      throw codedError(
        "CUSTODY_BASE_REVISION",
        "live HEAD must equal runSpec.repository.expectedBaseRevision",
      );
    }
    if (!live.clean) {
      throw codedError("CUSTODY_REPOSITORY_DIRTY", "trusted repository primary worktree must be clean");
    }
    ...
    runGit(
      ...
      [
        ...
        "add",
        "--detach",
        gitPath(worktreePath, gitExecutablePath),
    ...
    if (wtHead !== runSpec.repository.expectedBaseRevision) {
      throw codedError("CUSTODY_WORKTREE_HEAD", "detached worktree HEAD must equal expected base");
    }
    const wtStatus = String(
      runGit(gitExecutablePath, worktreePath, ["status", "--porcelain", "-uall"], gitHome).stdout,
    );
    if (wtStatus.trim() !== "") {
      throw codedError("CUSTODY_WORKTREE_DIRTY", "worktree must be clean at base before start");
    }
