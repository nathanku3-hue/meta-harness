...
const RETIRED_PROOF_SUBMISSION_ACTION = ["CERTIFY", "CANDIDATE"].join("_");
...
function validateExecutionRequest(request) {
  ...
  if (!ACTIONS.has(request.action)) {
    if (request.action === RETIRED_PROOF_SUBMISSION_ACTION) {
      throw contractError(
        "EXECUTION_PAYLOAD_SHAPE",
        "retired proof-submission actions have no valid payload or execution path",
      );
    }
  ...
  try {
    ...
    if (request.action === "RECORD_PUBLICATION_OBSERVATION") {
      const recorded = recordPublicationObservation({
        ...
        publicationIntent: request.payload.publicationIntent,
