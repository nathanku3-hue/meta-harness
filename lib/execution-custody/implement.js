...
/**
 * Post-claim execution path: read-only agent artifact, controller materialization,
 * trusted validation, immutable terminal evidence, and create-only durable ref.
 */
...
} = require("../contracts/implementation-facts");
const {
  evaluateImplementationFacts,
} = require("../contracts/implementation-assessment");
const { commandIdFromSpecCommand } = require("../contracts/run-spec");
const { checkScope } = require("../contracts/scope");
...
const {
  ...
  summarizeEvents,
} = require("./change-artifact");
const { spawnAgentProcess } = require("./agent-process");
const { AGENT_TIMEOUT_SECONDS } = require("./constants");
const {
  captureWorktreeCustody,
  assertPostAoCleanCustody,
} = require("./agent-custody");
const {
...
const JOURNAL_SCHEMA = "execution-custody-journal/v1";
...
function writeTerminalFailure(journalPath, receipt, claim, startCheck, workspaceRef, invocationNonce, clock, err) {
  const code = (err && err.code) || "CUSTODY_CONTROLLER_FAILED";
...
function reattestAfterValidation(gitExecutablePath, worktreePath, repositoryPath, runSpec, commitHead, durableRef, gitHome) {
  ...
  if (status.trim() !== "") {
    throw codedError("CUSTODY_POST_DIRTY", "worktree must be clean after validation (including untracked)");
  }
  ...
  if (objectFormat !== runSpec.repository.objectFormat) {
    throw codedError("CUSTODY_OBJECT_FORMAT", "post-validation objectFormat mismatch");
  }
  ...
  if (mergeBase !== runSpec.repository.expectedBaseRevision) {
    throw codedError("CUSTODY_BASE_ANCESTOR", "expected base must be ancestor of commit");
  }
...
function persistAoMeta(artDir, processResult, eventSummary, spawnOrdinal) {
  const meta = {
    schemaVersion: "agent-process-meta/v1",
    spawnOrdinal,
...
async function implementAfterClaim(ctx, args) {
  ...
  try {
    ...
    if (custodyBefore.head !== runSpec.repository.expectedBaseRevision) {
      throw codedError(
        "CUSTODY_PRE_AGENT_HEAD",
        "pre-agent HEAD must equal expected base",
      );
    }
    if (!custodyBefore.detached || custodyBefore.status.trim() !== "") {
      throw codedError("CUSTODY_PRE_AGENT_CLEAN", "pre-agent worktree must be clean and detached");
    }

    const spawnOrdinal = typeof recordAgentSpawn === "function" ? recordAgentSpawn() : 1;
    const processResult = await spawnAgentProcess(boundAgent, {
      worktreePath,
      schemaPath,
      allowedPath,
      objective: runSpec.objective,
      timeoutSeconds: agentTimeoutSeconds || AGENT_TIMEOUT_SECONDS,
    });
    ...
    if (!processResult.ok) {
      ...
      if (processResult.meta.timedOut) {
        throw codedError("CUSTODY_AGENT_TIMEOUT", "agent process timed out; process tree terminated");
      }
      if (processResult.meta.capBreached) {
        throw codedError(
          "CUSTODY_AGENT_OUTPUT_CAP",
          `agent ${processResult.meta.capBreached} exceeded cap; process tree terminated`,
        );
      }
      throw codedError(
        "CUSTODY_AGENT_EXIT",
        `agent process exited ${processResult.meta.exitCode}`,
        { exitCode: processResult.meta.exitCode },
      );
    }

    const events = parseAgentJsonl(processResult.stdout);
    ...
    if (postResetHead !== runSpec.repository.expectedBaseRevision) {
      throw codedError("CUSTODY_RESET_HEAD", "after reset --mixed HEAD must equal expected base");
    }
    if (!isDetachedHead(gitExecutablePath, worktreePath, gitHome)) {
      throw codedError("CUSTODY_NOT_DETACHED", "worktree HEAD must be detached");
    }
    ...
    runGit(
      gitExecutablePath,
      worktreePath,
      ["commit", "-m", `verified bounded change ${authReqHex}`],
      gitHome,
    );
    ...
    const validationStdoutParts = [];
    const validationStderrParts = [];
    for (let commandIndex = 0; commandIndex < runSpec.validation.commands.length; commandIndex += 1) {
      const validationCmd = runSpec.validation.commands[commandIndex];
      const cwdRelative = validationCmd.cwdRelative === "." ? "" : validationCmd.cwdRelative;
      const commandCwd = path.resolve(worktreePath, ...cwdRelative.split("/").filter(Boolean));
      const relativeCwd = path.relative(worktreePath, commandCwd);
      if (relativeCwd === ".." || relativeCwd.startsWith(`..${path.sep}`) || path.isAbsolute(relativeCwd)) {
        throw codedError("CUSTODY_VALIDATION_CWD", "validation cwd escaped the worktree");
      }
      const stdoutName = `validation-${commandIndex + 1}.stdout`;
      const stderrName = `validation-${commandIndex + 1}.stderr`;
      const valStartedAt = clock();
      const valResult = spawnProgram(boundValidation, validationCmd, commandCwd, commandIndex);
      const valEndedAt = clock();
      const commandStdout = String(valResult.stdout || "");
      const commandStderr = String(valResult.stderr || "");
      writeTextFile(path.join(artDir, stdoutName), commandStdout);
      writeTextFile(path.join(artDir, stderrName), commandStderr);
      validationStdoutParts.push(`=== command ${commandIndex + 1} ===\n${commandStdout}`);
      validationStderrParts.push(`=== command ${commandIndex + 1} ===\n${commandStderr}`);
      if (valResult.error) {
        throw codedError("CUSTODY_VALIDATION_SPAWN_FAILED", valResult.error.message);
      }
      if (valResult.status !== 0) {
        throw codedError(
          "CUSTODY_VALIDATION_FAILED",
          `validation command ${commandIndex + 1} exited ${valResult.status}: ${String(valResult.stderr || valResult.stdout || "").trim()}`,
          { status: valResult.status, commandIndex },
        );
      }
      ...
      validationEvidence.push({
        commandId: commandIdFromSpecCommand(validationCmd),
        argv: validationCmd.argv.slice(),
        cwdRelative: validationCmd.cwdRelative,
        timeoutSeconds: validationCmd.timeoutSeconds,
        networkPolicy: validationCmd.networkPolicy,
        environmentPolicy: { allow: validationCmd.environmentPolicy.allow.slice() },
        ...
        stdoutArtifact: artifactDigest(stdoutName, String(valResult.stdout || "")),
        stderrArtifact: artifactDigest(stderrName, String(valResult.stderr || "")),
      });
    }
    writeTextFile(path.join(artDir, "validation.stdout"), validationStdoutParts.join("\n"));
    writeTextFile(path.join(artDir, "validation.stderr"), validationStderrParts.join("\n"));
    ...
    if (primaryStatus.trim() !== "") {
      throw codedError("CUSTODY_PRIMARY_DIRTY", "primary worktree must remain clean at base");
    }
    ...
    if (primaryHead !== runSpec.repository.expectedBaseRevision) {
      throw codedError("CUSTODY_PRIMARY_HEAD", "primary HEAD must remain at expected base");
    }
