...
const { gitPath, pathsEqual } = require("../worktree-custody");
...
function removeWorktreeVerified(gitExecutablePath, repositoryPath, worktreePath, gitHome) {
  try {
    runGit(
      gitExecutablePath,
      repositoryPath,
      ["worktree", "remove", "--force", gitPath(worktreePath, gitExecutablePath)],
      gitHome,
    );
  } catch (err) {
    try {
  ...
  for (const line of listed.split(/\r?\n/)) {
    ...
    try {
      ...
      stillRegistered = pathsEqual(registered, worktreePath);
    }
    if (stillRegistered) {
      throw codedError(
        "CUSTODY_WORKTREE_CLEANUP",
