...
function createDevSpaceProposalHost({ endpoint, token, fetchImpl = globalThis.fetch }) {
  ...
  return Object.freeze({
    ...
    async readTelemetry(binding, { signal } = {}) {
      const parsed = await post(new URL("telemetry", hostEndpoint), {
        schemaVersion: "meta-proposal-telemetry-read/v1",
        packetDigest: binding.packetDigest,
        taskId: binding.taskId,
        taskDigest: binding.taskDigest,
      }, "telemetry read", { signal });
      return validateHostProposalTelemetry(parsed, binding);
    },
...
async function captureProposalTelemetry(proposalHost, workspaceRegistryDir, intent, receipt, signal) {
  const existing = readProposalContextTelemetry({
    workspaceRegistryDir,
    workspaceId: intent.authority.workspaceId,
    generation: intent.authority.generation,
    optional: true,
  });
  if (existing) return Object.freeze({ telemetry: assertProposalContextTelemetryBinding(intent, receipt, existing), error: null });
  if (!proposalHost || typeof proposalHost.readTelemetry !== "function" || !receipt) {
    return Object.freeze({ telemetry: null, error: null });
  }
  try {
    const hostTelemetry = await proposalHost.readTelemetry({
      packetDigest: receipt.packetDigest,
      taskId: receipt.taskId,
      taskDigest: receipt.taskDigest,
    }, { signal });
    return Object.freeze({
      telemetry: persistProposalContextTelemetry({ workspaceRegistryDir, intent, receipt, hostTelemetry }),
      error: null,
    });
  } catch (error) {
    return Object.freeze({ telemetry: null, error });
  }
}
...
async function reconcileProposalDispatch({ proposalHost, workspaceRegistryDir, intent, signal }) {
  ...
  if (retainedResult) {
    ...
    const observed = await captureProposalTelemetry(proposalHost, workspaceRegistryDir, intent, receipt, signal);
  ...
  try {
    ...
    return Object.freeze({ established: false, state: "RUNNING", receipt: retainedReceipt, result: null, telemetry: null, error, telemetryError: null, releaseError: null });
  }
  const receipt = persistProposalDispatchReceipt({ workspaceRegistryDir, intent, hostResult });
  if (hostResult.state !== "RESULT_READY") {
    return Object.freeze({ established: true, state: "RUNNING", receipt, result: null, telemetry: null, error: null, telemetryError: null, releaseError: null });
  }
  ...
  try {
    ...
    return Object.freeze({ established: true, state: "RESULT_READY", receipt, result: null, telemetry: null, error, telemetryError: null, releaseError: null });
  }
  if (!durableResult) {
