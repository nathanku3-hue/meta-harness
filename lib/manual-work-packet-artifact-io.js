...
function readManualWorkPacketArtifact(context, rawPath, repos = []) {
...
function readManualWorkPacketArtifactForValidation(context, rawPath, repos = []) {
  const read = readManualWorkPacketArtifact(context, rawPath, repos);
  return validationResult(rawPath, read.readError, read.artifact);
}

function readManualWorkPacketArtifactForRollup(context, rawPath, repos = []) {
  const read = readManualWorkPacketArtifact(context, rawPath, repos);
  const validation = validationResult(rawPath, read.readError, read.artifact);
  return {
    validation,
    verifiedManualWorkPacket: validation.verdict === "pass" && read.artifact ? read.artifact.manual_work_packet : null,
  };
}
