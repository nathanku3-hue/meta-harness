...
const { readOutcomeClaim, releaseOutcomeClaimUnderAuthority } = require("./outcome-claim");
const {
  admissionForHead,
...
function outcomeClosureForTransition(repositoryPath, transition) {
  ...
  if (claim.outcomeDigest !== closure.origin.outcomeDigest
      || claim.originWorldHeadDigest !== transition.predecessorHeadDigest) {
    fail("MH_WORLD_TRANSITION_PREDECESSOR", "Outcome claim does not bind the transition predecessor WorldHead");
  }
  return canonical;
}
