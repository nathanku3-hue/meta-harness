...
function assertAttemptTransition(repositoryPath, transition, admission) {
  if (!admission) {
    if (transition.cause.type !== "REALITY_REFRESH") {
      fail("MH_WORLD_TRANSITION_CAUSE", "ATTEMPT_* transition requires an admitted Decision for the predecessor WorldHead");
    }
    return;
  }
  if (transition.cause.type === "REALITY_REFRESH") {
    fail("MH_WORLD_HEAD_FROZEN", "WorldHead cannot reality-refresh while a Repo Decision admitted against it remains unbanked", {
      decisionDigest: admission.decisionDigest,
    });
  }
  const decision = readImmutableJson(repositoryPath, "decisions", admission.decisionDigest);
