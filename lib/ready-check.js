...
const { scanRepositoryLayout } = require("./root-leak-check");
...
async function runReadyCheck({
  ...
  // 11. MH_STATE_ROOT_LEAK_001: Repository layout custody
  await runCheck("MH_STATE_ROOT_LEAK_001", "repository_layout", () => {
    const res = scanRepositoryLayout({ targetRoot });
    if (res.status === "SKIP") {
      return { status: "skip", reason: res.reason, applicable: false, details: res };
    }
    if (res.status === "PASS") {
      return { status: "pass", details: res };
    }
    const summary = [
      `physical-only=${res.counts.physical_only}`,
      `registered-only=${res.counts.registered_only}`,
      `broken-pointers=${res.counts.broken_pointers}`,
      `unexpected-root=${res.counts.unexpected_root_entries}`,
      `external-creator=${res.counts.external_creator_residue}`,
    ].join(" ");
    return {
      status: "fail",
      reason: `repository layout custody rejected: ${summary}`,
      next_action: "Preserve unique bytes, reconcile Git registration with physical managed-worktree paths, and relocate unexpected root artifacts; do not delete through readiness",
      details: res,
    };
  });
