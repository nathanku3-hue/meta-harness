...
const { checkContextGateArtifact, validateContextGateArtifactDoc } = require("./ready-context-gate");
const {
  SHIPGATE_CHECK_ID,
  ShipGateUnavailableError,
  classifyCurrentChangeSet,
} = require("./ship-gate");
const { REQUIRED_CHECK_IDS, getReadyCheckTimeoutMs } = require("./ready/check-constants");
const {
  readQualityContract,
  readQualityBaseline,
  analyzeQuality,
...
async function runReadyCheck({
  ...
  await runCheck("MH_CONTEXT_GATE_001", "context_gate", () => checkContextGateArtifact({ targetRoot }));
