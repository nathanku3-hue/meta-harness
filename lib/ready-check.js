...
const { validateContextGateArtifactDoc } = require("./ready-context-gate");
const { checkContextGateArtifact } = require("./ready-context-gate-evaluation");
const { checkContextGateGraphForReady } = require("./context-gate-graph");
const { SHIPGATE_CHECK_ID, ShipGateUnavailableError, classifyCurrentChangeSet } = require("./ship-gate");
const { INCLUDED_CHECK_IDS, REQUIRED_CHECK_IDS, getReadyCheckTimeoutMs } = require("./ready/check-constants");
const { readQualityContract, readQualityBaseline, analyzeQuality, compareQualityToBaseline, qualityContractPath, qualityBaselinePath } = require("./quality");
const { resolvePackageRoot, isNodeRangeSupported } = require("./package-root");
const { inspectLifecycleScripts } = require("./security-package-check");
const { truthCheck } = require("./truth-reconciler");
...
function computeReadyStateHash(targetRoot, packageResolution = null) {
  ...
  const packageRoot = packageResolution && typeof packageResolution === "object"
    ? packageResolution.packageRoot
    : (typeof packageResolution === "string" ? packageResolution : null);
  const packageRootRelative = packageResolution && typeof packageResolution === "object"
    ? packageResolution.packageRootRelative
    : null;
  if (packageRoot && packageRootRelative && packageRootRelative !== ".") {
    hasher.update(`package-root:${packageRootRelative}\n`);
    for (const file of ["package.json", "package-lock.json", ".npmignore", ".gitignore"]) {
      addFileToHash(hasher, packageRoot, file);
    }
  }
...
function validatePregeneratedReady(doc, targetRoot, noExec, packageResolution = null) {
  ...
  if (doc.package_root !== undefined) {
    const expectedPackageRoot = packageResolution?.packageRootRelative || null;
    if (doc.package_root !== expectedPackageRoot) {
      return { ok: false, reason: `package_root mismatch: ready.json has ${doc.package_root} but resolved package root is ${expectedPackageRoot || "none"}` };
    }
  }
  if (doc.package_discovery !== undefined) {
    const expectedDiscovery = packageResolution?.packageDiscovery || "none";
    if (doc.package_discovery !== expectedDiscovery) {
      return { ok: false, reason: `package_discovery mismatch: ready.json has ${doc.package_discovery} but resolution is ${expectedDiscovery}` };
    }
  }
  ...
  for (const id of INCLUDED_CHECK_IDS) {
    if (!checkIds.has(id) && !notApplicable.has(id)) {
      return { ok: false, reason: `missing included ready check result for: ${id}` };
    }
  }
  ...
  const currentStateHash = computeReadyStateHash(targetRoot, packageResolution);
...
async function runReadyCheck({
  ...
  const packageResolution = resolvePackageRoot(targetRoot, packageRootOption);
  const resolvedPackageRoot = packageResolution.packageRoot;
  const checks = [];
  ...
  if (fs.existsSync(foundPath)) {
    try {
      ...
      const validation = validatePregeneratedReady(doc, targetRoot, effectiveNoExec, packageResolution);
      if (validation.ok) {
  ...
  async function runCheck(checkId, name, checkFn) {
    ...
    const checkResult = { id: checkId, name, status: res.status, reason: res.reason || "",
      next_action: res.next_action || "", applicable: res.applicable !== false, duration_ms: durationMs };
    for (const field of ["details", "diagnostic", "explanation", "provenance", "remediation"]) {
      if (res[field] !== undefined) checkResult[field] = res[field];
    }
  ...
  await runCheck("MH_CONTRACT_001", "contract", () => {
    ...
    return {
      status: "fail",
      reason: `conflicting report contracts or active guidance: ${failed}`,
      next_action: "Separate worker-report artifacts from concise final chat guidance"
    };
  });
  ...
  await runCheck("MH_TRUTH_001", "truth", () => truthCheck({ targetRoot }));
  await runCheck("MH_CONTEXT_GATE_001", "context_gate", () => checkContextGateArtifact({ targetRoot, explain }));

  await runCheck("MH_TRANSITION_GRAPH_001", "transition_graph", () => checkContextGateGraphForReady({ sourceRoot }));
  ...
  await runCheck("MH_QUALITY_001", "quality", () => {
    ...
    if (!hasContract && !hasBaseline) {
      return {
        status: "skip",
        reason: "quality policy not adopted",
        applicable: false
      };
    }
    if (!hasContract || !hasBaseline) {
      return {
        status: "fail",
        reason: `quality policy adoption incomplete: ${hasContract ? "baseline missing" : "contract missing"}`,
        next_action: "Adopt both .meta-harness/clean-code-contract.json and .meta-harness/baseline/quality-baseline.json"
      };
    }
  ...
  await runCheck("MH_NPM_SCRIPTS_001", "npm_scripts", () => {
    if (packageResolution.error) {
      return { status: "fail", reason: packageResolution.error, next_action: "Pass --package-root <relative-path> to select one package" };
    }
    if (!resolvedPackageRoot) {
      return { status: "skip", reason: "no Node package found", applicable: false };
    }
    const { failedHooks, packHooks, warnedHooks } = inspectLifecycleScripts(resolvedPackageRoot);
  ...
  await runCheck("MH_REPRO_001", "reproducibility", () => {
    ...
    const pkgPath = path.join(resolvedPackageRoot, "package.json");
    const pkg = JSON.parse(fs.readFileSync(pkgPath, "utf8"));
    const enginesNode = pkg.engines && pkg.engines.node;
    const packageManager = pkg.packageManager;

    const gitignorePath = path.join(resolvedPackageRoot, ".gitignore");
    ...
    const lockPath = path.join(resolvedPackageRoot, "package-lock.json");
    const missingLock = !fs.existsSync(lockPath);
    const lockIgnored = isLockIgnored;
    const invalidEngines = !enginesNode || !isNodeRangeSupported(enginesNode);
    ...
    if (missingLock || lockIgnored || invalidEngines || invalidPackageManager || invalidLockfile) {
      ...
      if (invalidEngines) {
        if (!enginesNode) {
          ...
          reasons.push(`engines.node must specify a supported Node range (found ${enginesNode})`);
        }
      }
      ...
      return {
        status: "fail",
        reason: reasons.join(", "),
        next_action: "Ensure package-lock.json (with lockfileVersion), a supported engines.node range, and packageManager (npm) are defined and not ignored"
      };
    }
  ...
  await runCheck("MH_TEST_001", "test", () => {
    ...
    try {
      execSync("npm test", { cwd: resolvedPackageRoot, stdio: "ignore", timeout: getReadyCheckTimeoutMs("MH_TEST_001") });
      return { status: "pass" };
    } catch (err) {
      if (err.code === "ETIMEDOUT" || err.signal === "SIGTERM" || err.status === 124) {
  ...
  const state_hash = computeReadyStateHash(targetRoot, packageResolution);
  ...
  return {
    ...
    mode,
    package_root: packageResolution.packageRootRelative,
    package_discovery: packageResolution.packageDiscovery,
    ...(packageResolution.candidates.length > 0 ? { package_root_candidates: packageResolution.candidates } : {}),
    ...(packageResolution.error ? { package_root_error: packageResolution.error } : {}),
...
module.exports = { checkContextGateArtifact, validateContextGateArtifactDoc, runReadyCheck, computeReadyStateHash, getReadyCheckTimeoutMs };
