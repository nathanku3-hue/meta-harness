...
const { checkWorktreeCustody } = require("./worktree-custody");
...
async function runReadyCheck({
  ...
  // 11b. MH_WORKTREE_001: Repository-local worktree custody confinement
  await runCheck("MH_WORKTREE_001", "worktree_custody", () => {
    if (effectiveNoExec) {
      return getPregeneratedOrSkip("MH_WORKTREE_001", "worktree_custody", "worktree custody skipped in no-exec mode");
    }
    if (!hasGitMetadata(targetRoot)) {
      return { status: "skip", reason: "not a git repository", applicable: false };
    }
    const res = checkWorktreeCustody({ targetRoot, mode });
    return {
      status: res.status,
      reason: res.reason || "",
