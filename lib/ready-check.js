...
async function runReadyCheck({
  ...
  await runCheck("MH_QUALITY_001", "quality", () => {
    ...
    return {
      status: "fail",
      reason,
      next_action: "Fix quality gate violations or update quality baseline with --force"
    };
  });

  // 8. MH_SECURITY_001: Security hygiene
  await runCheck("MH_SECURITY_001", "security", () => {
    const securityMdExists = fs.existsSync(path.join(targetRoot, "SECURITY.md"));
    const gitattributesExists = fs.existsSync(path.join(targetRoot, ".gitattributes"));
    const gitignorePath = path.join(targetRoot, ".gitignore");
    let gitignoreOk = true;
    let gitignoreMissingPatterns = [];

    if (!fs.existsSync(gitignorePath)) {
      return {
        status: "warn",
        reason: ".gitignore missing",
        next_action: "Create .gitignore with secret and local-state patterns"
      };
    }
    ...
    gitignoreMissingPatterns = requiredPatterns.filter(p => !lines.includes(p));
    if (gitignoreMissingPatterns.length > 0) {
      gitignoreOk = false;
    }
    ...
    if (!gitignoreOk) {
      return {
        status: "fail",
        reason: `.gitignore lacks patterns: ${gitignoreMissingPatterns.join(", ")}`,
        next_action: "Add secret and key ignore patterns to .gitignore"
      };
    }
    if (!securityMdExists) {
      return {
        status: "skip",
        reason: "SECURITY.md not yet required"
      };
    }
    if (!gitattributesExists) {
      return {
        status: "warn",
        reason: ".gitattributes missing",
        next_action: "Create .gitattributes to normalize line endings"
      };
    }
    return { status: "pass" };
  });
  ...
  // 11. MH_GITCHECK_001: Git whitespace
  await runCheck("MH_GITCHECK_001", "git", () => {
    ...
    try {
      execSync("git diff --check", { cwd: targetRoot, stdio: "ignore", timeout: 30000 });
      return { status: "pass" };
    } catch (err) {
  ...
  // 12. MH_PACKAGE_001: Package dry-run
  ...
  // 13. MH_GITHUB_SETTINGS_001: GitHub repository settings
  ...
  await runCheck("MH_TEST_001", "test", () => {
    ...
    try {
      execSync("npm test", { cwd: targetRoot, stdio: "ignore", timeout: 30000 });
      return { status: "pass" };
    } catch (err) {
      if (err.code === "ETIMEDOUT" || err.signal === "SIGTERM" || err.status === 124) {
