...
function optionalString(decision, index, field, problems) {
  if (decision[field] === undefined || decision[field] === null) {
    return undefined;
  }
  const value = trimmedString(decision[field]);
  if (!value) {
    problems.push(item("REJECTED", fieldPath(index, field), `${field} must be a non-empty string when present`));
    return undefined;
  }
  return value;
}
...
function validateDecision(decision, index, seen, problems) {
  ...
  requiredString(decision, index, "state_hash", problems);

  requiredString(decision, index, "assumption_hash", problems);

  const identityHash = optionalString(decision, index, "identity_hash", problems);
  checkDuplicate(identityHash, index, "identity_hash", seen.identityHashes, problems);
...
function validateInbox(value) {
  ...
  const problems = [];
  const seen = { ids: new Map(), identityHashes: new Map() };
