...
function writeCompleteLedgerAtomic(eventsPath, events, { targetRoot } = {}) {
  ...
  try {
    ...
    // One direct rename only. Supported Linux and Windows Node runtimes replace the
    // existing destination. Never move the live ledger aside — a failed install must
    // leave the previous truth readable at the canonical path.
