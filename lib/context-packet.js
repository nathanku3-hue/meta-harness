...
const { hasAdoptionContract } = require("./context-gate-adoption");
const {
  PACKET_TARGETS,
  validatePacketInputs,
  validationOptionsForPacketAudience,
} = require("./context-packet-policy");
...
function collectPacketState(cwd, artifactPath, artifact) {
  ...
  const sources = [];
  if (fs.existsSync(path.join(cwd, "PRODUCT.md"))) sources.push("PRODUCT.md");
  if (fs.existsSync(path.join(cwd, HARNESS_DIR, "status.md"))) sources.push(".meta-harness/status.md");
  if (fs.existsSync(path.join(cwd, HARNESS_DIR, "events.jsonl"))) sources.push(".meta-harness/events.jsonl");
  sources.push(slashPath(path.relative(cwd, artifactPath)));
  if (
    hasAdoptionContract(cwd)
    && fs.existsSync(path.join(cwd, HARNESS_DIR, "phase-map.md"))
  ) {
    sources.push(".meta-harness/phase-map.md (legacy compatibility evidence)");
  }
...
function renderContextPacket(artifact, state, target, warnings = []) {
...
function buildContextPacket(options = {}) {
  ...
  const target = String(options.for || options.audience || options.target || "planning").trim();
  const selected = readGateArtifact(cwd, options.roundId, validationOptionsForPacketAudience(target, options.validation || {}));
  const artifact = selected.artifact;

  const warnings = validatePacketInputs({
    cwd,
    artifact,
  ...
  const packetMarkdown = renderContextPacket(artifact, state, target, warnings);
