...
function assertContainedPath(targetRoot, targetPath, {
  ...
  for (let index = 0; index < parts.length; index += 1) {
    ...
    // Canonical authority, ledger, status, and lock files must be uniquely named inodes.
    // Hard links pass symlink/realpath checks but alias mutation outside the repository.
    if (isLeaf && leafType === "file" && stats.isFile() && stats.nlink !== 1) {
      pathError(`${label} must not be a hard-linked or multiply linked file`, {
        path: current,
        nlink: stats.nlink,
      });
    }
