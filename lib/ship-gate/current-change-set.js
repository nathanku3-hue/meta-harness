...
function readOwners(targetRoot, unavailableError) {
  ...
  try {
    ...
    if (Array.isArray(owners.modules)) {
      return owners.modules.flatMap((mod) => [
        ...(Array.isArray(mod.owned_paths) ? mod.owned_paths : []),
        ...(Array.isArray(mod.ownedPaths) ? mod.ownedPaths : []),
        ...(Array.isArray(mod.paths) ? mod.paths : []),
        ...(typeof mod.path === "string" ? [mod.path] : []),
      ]);
    }
  } catch (error) {
