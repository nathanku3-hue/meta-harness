...
const { commandSummary, runNpm } = require("./npm-command");
...
const EXCLUDED_SCAN_DIRS = new Set([".git", "node_modules", "docs", "tests"]);
const CODE_SCAN_EXTENSIONS = new Set([".cjs", ".js", ".jsx", ".mjs", ".ts", ".tsx", ".py", ".rb", ".go", ".rs", ".java", ".cs", ".php"]);
...
function checkHelperBudget(context) {
  const helpers = [];
  for (const file of context.changedFiles.filter((item) => item.endsWith(".js") && !item.startsWith("tests/") && !item.startsWith("scripts/"))) {
    const added = diffAddedLines(context.targetRoot, context.baseSha, file, context.untrackedFiles.includes(file));
...
function checkOldSymbolResidue(context) {
  ...
  walkFiles(context.targetRoot, (relativePath, filePath, stat) => {
    ...
    if (!CODE_SCAN_EXTENSIONS.has(path.extname(relativePath))) return;
