...
function certifyCandidate({
  ...
  let context = validateActiveContext({ repositoryPath, sliceAuthorization, controllerInstanceId, packageRoot });
  if (context.currentState.stage !== "ACTIVE") {
    throw contractError("CERTIFICATION_STAGE", "certification may be produced only from ACTIVE state");
  }
  if (context.authorization.sliceMode !== "CERTIFICATION") {
    throw contractError("CERTIFICATION_MODE_REQUIRED", "certification requires a CERTIFICATION authorization");
  }
  ...
  const produced = produceCertificationEvidence({
    repositoryPath,
    sliceAuthorization: context.authorization,
  ...
  const evidence = [
    ...
    ["CERTIFICATION_PREPARE", "certification-candidate/v1", produced.certificationCandidate.certificationCandidateDigest, `certification/candidate-generation-${integrated.generation}.json`, produced.certificationCandidate],
    ["PROOF_EXECUTE", "certification-proof/v1", produced.proof.proofDigest, `certification/proof-generation-${integrated.generation}.json`, produced.proof],
    ...produced.reviews.map((review) => [
      "REVIEW_ORCHESTRATE",
      "certification-reviewer-assessment/v1",
      review.assessmentDigest,
      `certification/reviews/${review.role.toLowerCase()}-generation-${integrated.generation}.json`,
      review,
    ]),
    ["CERTIFICATION_ASSESS", "certification-assessment/v1", produced.assessment.assessmentDigest, `certification/assessment-generation-${integrated.generation}.json`, produced.assessment],
  ];
  const operations = [];
  ...
  context = validateActiveContext({ repositoryPath, sliceAuthorization, controllerInstanceId, packageRoot });
  const state = nextSliceState(context.currentState, "CERTIFICATION_VERIFIED", {
    certificationCandidateDigest: produced.certificationCandidate.certificationCandidateDigest,
    terminalAssessmentDigest: produced.assessment.assessmentDigest,
  });
  const transition = transitionSliceState({
    ...
    nextState: state,
    controllerInstanceId,
    capability: "CERTIFICATION_ASSESS",
    packageRoot,
  });
  return Object.freeze({ integrated, ...produced, operations, transition });
}
