...
const EXPLICIT_OWNER_AUTHORIZATION = "EXPLICIT_OWNER_AUTHORIZATION";
...
const SLICE_MODES = Object.freeze(new Set(["DELIVERY"]));
const AUTHORITY_EXECUTION_PLATFORMS = Object.freeze(new Set(["linux"]));
const RETIRED_AUTHORIZATION_CAPABILITIES = Object.freeze(new Set([
  ["CERTIFICATION", "PREPARE"].join("_"),
  ["CERTIFICATION", "ASSESS"].join("_"),
]));
...
function validateControllerBinding(value) {
  ...
  for (const capability of capabilities) {
    if (!CONTROLLER_CAPABILITIES.has(capability) && !RETIRED_AUTHORIZATION_CAPABILITIES.has(capability)) {
...
function validatePublicationPolicy(value) {
  ...
  requireExactKeys(value.trustedPublisher, ["provider", "repository", "workflowFilename"], "publicationPolicy.trustedPublisher");
  if (value.trustedPublisher.provider !== "github-actions") {
    throw contractError("SLICE_PUBLICATION_PROVIDER", "trusted publisher provider must be github-actions");
  }
  requireNonEmptyString(value.trustedPublisher.repository, "publicationPolicy.trustedPublisher.repository");
  if (!/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(value.trustedPublisher.repository)) {
    throw contractError("SLICE_PUBLICATION_REPOSITORY", "trusted publisher repository must be owner/name");
  }
  requireNonEmptyString(value.trustedPublisher.workflowFilename, "publicationPolicy.trustedPublisher.workflowFilename");
  if (value.trustedPublisher.workflowFilename.includes("/")
    || value.trustedPublisher.workflowFilename.includes("\\")
    || !/\.ya?ml$/i.test(value.trustedPublisher.workflowFilename)) {
    throw contractError("SLICE_PUBLICATION_WORKFLOW", "trusted publisher workflow must be one .yml or .yaml filename");
  }
...
function validateSliceAuthorization(value, ownerPin, options = {}) {
  ...
  if (!SLICE_MODES.has(value.sliceMode)) {
    throw contractError("SLICE_MODE_INVALID", "SliceAuthorization.sliceMode must be DELIVERY");
  }
  ...
  if (value.ownerSignature !== EXPLICIT_OWNER_AUTHORIZATION) {
    throw contractError(
      "SLICE_AUTHORIZATION_SIGNING_RETIRED",
      `SliceAuthorization.ownerSignature must be ${EXPLICIT_OWNER_AUTHORIZATION}`,
    );
  }
  assertSealedDigest(
  ...
  if (value.ownerKeyId !== ownerPin.ownerKeyId) {
    throw contractError("SLICE_AUTHORIZATION_OWNER_MISMATCH", "SliceAuthorization owner identity does not match external owner pin");
  }
