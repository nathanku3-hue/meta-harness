...
function validateArtifactFields(value) {
  ...
  if (STAGE_ORDER.get(value.stage) >= STAGE_ORDER.get("TERMINAL_CANDIDATE") && value.stage !== "ABORTED") {
    requirePresent("terminalCandidateDigest");
    requirePresent("releaseCandidateDigest");
  }
  if (STAGE_ORDER.get(value.stage) >= STAGE_ORDER.get("TERMINAL_VERIFIED") && value.stage !== "ABORTED") {
    requirePresent("terminalAssessmentDigest");
  }
  if (STAGE_ORDER.get(value.stage) >= STAGE_ORDER.get("PUBLISHED") && value.stage !== "ABORTED") {
