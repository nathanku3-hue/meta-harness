...
function validateArtifactFields(value) {
  ...
  const deliveryStages = new Set(["TERMINAL_CANDIDATE", "TERMINAL_VERIFIED", "PUBLICATION_AUTHORIZED", "PUBLISHED", "CLOSED"]);
  if (deliveryStages.has(value.stage)) {
    requirePresent("terminalCandidateDigest");
    requirePresent("releaseCandidateDigest");
  }
  if (new Set(["TERMINAL_VERIFIED", "PUBLICATION_AUTHORIZED", "PUBLISHED", "CLOSED"]).has(value.stage)) {
    requirePresent("terminalAssessmentDigest");
  }
  if (new Set(["PUBLICATION_AUTHORIZED", "PUBLISHED", "CLOSED"]).has(value.stage)) {
    requirePresent("publicationIntentDigest");
  }
...
function validateSliceStateTransition(priorValue, nextValue) {
  ...
  if (next.terminalCandidateDigest !== null
    ...
    || next.publicationIntentDigest !== null
