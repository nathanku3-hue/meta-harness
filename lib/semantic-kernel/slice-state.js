...
const TERMINAL_STAGES = Object.freeze(new Set(["CLOSED", "CERTIFICATION_VERIFIED", "ABORTED"]));
...
function validateArtifactFields(value) {
  ...
  if (new Set(["TERMINAL_VERIFIED", "PUBLISHED", "CLOSED", "CERTIFICATION_VERIFIED"]).has(value.stage)) {
  ...
  if (value.stage === "CERTIFICATION_VERIFIED") {
    requirePresent("certificationCandidateDigest");
    if (value.terminalCandidateDigest !== null
      || value.releaseCandidateDigest !== null
      || value.publicationObservationDigest !== null) {
      throw contractError("SLICE_STATE_CERTIFICATION_DELIVERY_EVIDENCE", "certification state cannot contain delivery or publication evidence");
    }
  }
}
