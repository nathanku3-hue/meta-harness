...
const PUBLICATION_INTENT_DRAFT_SCHEMA = "publication-intent-draft/v1";
const PUBLICATION_INTENT_SCHEMA = "publication-intent/v1";
const PUBLICATION_INTENT_DOMAIN = "meta-harness-publication-intent/v1";
const PUBLICATION_INTENT_SIGNATURE_DOMAIN = "meta-harness-publication-intent-signature/v1";
...
const DRAFT_KEYS = Object.freeze(["schemaVersion", ...COMMON_KEYS]);
...
function publicationIntentSigningBody(value) {
  return bodyWithout(value, ["ownerSignature"]);
}

function computePublicationIntentDigest(value) {
  return sealDigest(PUBLICATION_INTENT_DOMAIN, value, "intentDigest", ["ownerSignature"]);
}
...
function expectedBindings(bindings) {
  const policy = bindings.sliceAuthorization?.publicationPolicy;
  return {
    repositoryId: bindings.sliceAuthorization?.repositoryId,
    sliceId: bindings.sliceAuthorization?.sliceId,
    generation: bindings.releaseCandidate?.generation,
    sliceAuthorizationDigest: bindings.sliceAuthorization?.authorizationDigest,
    terminalAssessmentDigest: bindings.terminalAssessment?.terminalAssessmentDigest,
    releaseCandidateDigest: bindings.releaseCandidate?.releaseCandidateDigest,
    terminalStateDigest: bindings.terminalStateDigest,
    terminalOperationEventHead: bindings.terminalOperationEventHead,
    publicationAttemptOrdinal: bindings.publicationAttemptOrdinal,
    transport: policy?.trustedPublisher,
    gitTag: bindings.releaseCandidate?.gitTag,
    gitTagTargetRevision: bindings.releaseCandidate?.gitTagTargetRevision,
    tarballDigest: bindings.releaseCandidate?.tarballDigest,
    tarballIntegrity: bindings.releaseCandidate?.tarballIntegrity,
    tarballByteLength: bindings.packageCandidate?.tarballByteLength,
    packageName: bindings.releaseCandidate?.packageName,
    version: bindings.releaseCandidate?.version,
    registry: bindings.releaseCandidate?.registry,
    access: bindings.releaseCandidate?.access,
    distTag: bindings.releaseCandidate?.distTag,
    provenanceRequired: bindings.releaseCandidate?.provenanceRequired,
    publishBy: policy?.publishBy,
  };
}

function validateCommon(value, bindings = {}) {
  for (const field of [
    "repositoryId",
    "sliceAuthorizationDigest",
    "terminalAssessmentDigest",
  ...
  const expected = expectedBindings(bindings);
  for (const [field, expectedValue] of Object.entries(expected)) {
    if (expectedValue === undefined) continue;
    const actualNormalized = typeof expectedValue === "object" ? JSON.stringify(value[field]) : value[field];
    const expectedNormalized = typeof expectedValue === "object" ? JSON.stringify(expectedValue) : expectedValue;
    if (actualNormalized !== expectedNormalized) {
      throw contractError("PUBLICATION_INTENT_BINDING", `PublicationIntent.${field} differs from terminal release authority`);
    }
  }
  if (bindings.releaseCandidate && bindings.packageCandidate) {
    const tarballAsset = assets.find((asset) => asset.role === "tarball");
    if (tarballAsset.sha256 !== bindings.releaseCandidate.tarballDigest
      || tarballAsset.bytes !== bindings.packageCandidate.tarballByteLength) {
...
function createPublicationIntentDraft({
  sliceAuthorization,
  packageCandidate,
  releaseCandidate,
  terminalAssessment,
  ...
  const draft = {
    schemaVersion: PUBLICATION_INTENT_DRAFT_SCHEMA,
  ...
  return validatePublicationIntentDraft(draft, {
    sliceAuthorization,
    packageCandidate,
    releaseCandidate,
    terminalAssessment,
...
function validatePublicationIntentDraft(value, bindings = {}) {
  requireExactKeys(value, DRAFT_KEYS, "PublicationIntentDraft");
  if (value.schemaVersion !== PUBLICATION_INTENT_DRAFT_SCHEMA) {
    throw contractError("UNSUPPORTED_SCHEMA", `PublicationIntentDraft schema must be ${PUBLICATION_INTENT_DRAFT_SCHEMA}`);
  }
  validateCommon(value, bindings);
  return immutable(value);
}

function validatePublicationIntent(value, bindings = {}) {
  requireExactKeys(value, INTENT_KEYS, "PublicationIntent");
  ...
  validateCommon(value, bindings);
  requireDigest(value.ownerKeyId, "PublicationIntent.ownerKeyId");
  requireDigest(value.intentDigest, "PublicationIntent.intentDigest");
  requireNonEmptyString(value.ownerSignature, "PublicationIntent.ownerSignature");
  assertSealedDigest(
  ...
  const ownerPin = bindings.ownerPin;
  if (!ownerPin
    || value.repositoryId !== ownerPin.repositoryId
    || value.ownerKeyId !== ownerPin.ownerKeyId) {
    throw contractError("PUBLICATION_INTENT_OWNER_MISMATCH", "PublicationIntent does not match the external owner pin");
  }
  if (bindings.expectedOwnerKeyId !== undefined && value.ownerKeyId !== bindings.expectedOwnerKeyId) {
    throw contractError("PUBLICATION_INTENT_OWNER_ANCHOR", "PublicationIntent owner key differs from trusted publisher anchor");
  }
  verifyEd25519Signature({
    domain: PUBLICATION_INTENT_SIGNATURE_DOMAIN,
    body: publicationIntentSigningBody(value),
    signature: value.ownerSignature,
    publicKeyJwk: ownerPin.ownerPublicKey,
    label: "PublicationIntent",
  });
  return immutable(value);
}
...
module.exports = {
  ...
  PUBLICATION_INTENT_DOMAIN,
  PUBLICATION_INTENT_DRAFT_SCHEMA,
