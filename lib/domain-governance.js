...
const { checkDomainRuleEvidence, hasDomainRuleSurface } = require("./domain-rule-check");
...
function checkDomainGovernance({ targetRoot }) {
  ...
  const ruleResult = checkDomainRuleEvidence({ targetRoot, activation: activationResult.activation });
  const checks = [...activationResult.checks, ...pilotResult.checks, ...ruleResult.checks];
...
function hasDomainGovernanceSurface(targetRoot) {
  const governanceRoot = path.join(targetRoot, ...GOVERNANCE_DIR);
  return fs.existsSync(path.join(governanceRoot, ACTIVATION_FILE)) || fs.existsSync(path.join(governanceRoot, PILOT_CHAIN_FILE)) || hasDomainRuleSurface(targetRoot);
}

module.exports = { checkDomainGovernance, hasDomainGovernanceSurface };
