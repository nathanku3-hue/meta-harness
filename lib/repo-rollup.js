...
function emptySummary() {
  return { total: 0, ready: 0, warned: 0, failed: 0, stale: 0, unknown: 0, missing: 0, invalid: 0, drift_warnings: 0 };
}
...
function attachDrift(root, repo, paths, repoResult, fsApi) {
  return {
    ...repoResult,
...
function summarizeRepo(root, repo, nowMs, fsApi = fs) {
  ...
  const done = (repoResult) => attachDrift(root, repo, paths, repoResult, fsApi);
...
function renderRepoRollupMarkdown(rollup) {
  const lines = [
    ...
    "",
    `ready=${rollup.summary.ready} warned=${rollup.summary.warned} failed=${rollup.summary.failed} stale=${rollup.summary.stale} unknown=${rollup.summary.unknown} missing=${rollup.summary.missing} invalid=${rollup.summary.invalid} drift_warnings=${rollup.summary.drift_warnings}`,
    "",
    "## Child Repos",
    "",
  ];
