...
function emptySummary() {
  return { total: 0, ready: 0, warned: 0, failed: 0, stale: 0, unknown: 0, missing: 0, invalid: 0 };
}
...
function summarizeRepo(root, repo, nowMs, fsApi = fs) {
  ...
  if (!repo.path || !dirExists(paths.absolutePath, fsApi)) {
    return { ...result, state: "missing", reason: repo.path ? "configured child repo path does not exist" : "configured child repo path is empty" };
  }
  if (fileExists(paths.readyPath, fsApi)) {
    const parsed = safeReadJson(paths.readyPath, fsApi);
    if (!parsed.ok) {
      return { ...result, state: "invalid", source: `${HARNESS_DIR}/ready.json`, reason: "ready.json could not be parsed" };
    }
    return summarizeReadyRepo(result, parsed.value, classifyReadyJson(parsed.value, nowMs));
  }
  ...
  return { ...result, state: "unknown", reason: "no ready/status/poll artifact found" };
}
...
function renderRepoRollupMarkdown(rollup) {
  const lines = [
    ...
    "",
    `ready=${rollup.summary.ready} warned=${rollup.summary.warned} failed=${rollup.summary.failed} stale=${rollup.summary.stale} unknown=${rollup.summary.unknown} missing=${rollup.summary.missing} invalid=${rollup.summary.invalid}`,
    "",
    "## Child Repos",
    "",
  ];
