...
const autonomyPlan = require("./repo-rollup-autonomy-plan");
const autonomyApprovalReceiptValidation = require("./repo-rollup-autonomy-approval-receipt-validation");
const manualWorkPacket = require("./repo-rollup-manual-work-packet"), packetArtifactValidation = require("./repo-rollup-manual-work-packet-artifact-validation");
const operatorExecutionPlanBuilder = require("./repo-rollup-operator-execution-plan");
const opPlanArtifactValidation = require("./repo-rollup-operator-execution-plan-artifact-validation");
const executionReadiness = require("./execution-readiness");
const entryAuthority = require("./entry-authority");
const workerEntryGate = require("./worker-entry-gate");
const proposalReviewGate = require("./repo-rollup-proposal-review-gate");
const proposalReviewPacket = require("./repo-rollup-proposal-review-packet");
const proposalReviewOptions = require("./repo-rollup-proposal-review-options");
const proposalReviewReceiptTemplate = require("./repo-rollup-proposal-review-receipt-template"), proposalReviewReceiptValidation = require("./repo-rollup-proposal-review-" + "receipt-validation"), proposalReviewCopyBlock = require("./repo-rollup-proposal-review-copy-block"), proposalReviewCopyBlockValidation = require("./repo-rollup-proposal-review-copy-block-validation"), proposalReviewExportIntent = require("./repo-rollup-proposal-review-export-intent"), proposalReviewExportSafetyGate = require("./repo-rollup-proposal-review-export-safety-gate");
const { buildResponseHandoff } = require("./repo-rollup-handoff");
const HARNESS_DIR = ".meta-harness", SCHEMA_VERSION = "1.0.0";
const NOT_CHANGED = Object.freeze(["child_repos", "child_status", "child_events", "parent_status", "parent_events"]);
const REQUIRED_READY_FIELDS = Object.freeze(["schema_version", "generated_at", "target", "ok", "redacted", "expires_after", "checks"]);
function emptySummary() {
  return { total: 0, ready: 0, warned: 0, failed: 0, stale: 0, unknown: 0, missing: 0, invalid: 0, drift_warnings: 0, next_action_candidates: 0 };
}
function fileExists(filePath, fsApi = fs) {
  try { return fsApi.existsSync(filePath) && fsApi.statSync(filePath).isFile(); } catch (_) { return false; }
}
function dirExists(dirPath, fsApi = fs) {
  try { return fsApi.existsSync(dirPath) && fsApi.statSync(dirPath).isDirectory(); } catch (_) { return false; }
}
...
function normalizeRepo(rawRepo, index) {
  return { name: rawRepo?.name ? String(rawRepo.name) : `repo-${index + 1}`, path: rawRepo?.path ? String(rawRepo.path) : "", role: rawRepo?.role ? String(rawRepo.role) : "child" };
}
...
function buildRepoRollup(root, options = {}) {
  ...
  const reviewGate = proposalReviewGate.buildProposalReviewGate({ proposalDraft, proposalValidation: validation });
  const reviewPacket = proposalReviewPacket.buildProposalReviewPacket({ proposalDraft, proposalValidation: validation, proposalReviewGate: reviewGate });
  const reviewOptions = proposalReviewOptions.buildProposalReviewOptions({ proposalReviewPacket: reviewPacket });
  const receiptTemplate = proposalReviewReceiptTemplate.buildProposalReviewReceiptTemplate({ proposalReviewOptions: reviewOptions });
  const receiptValidation = proposalReviewReceiptValidation.buildProposalReviewReceiptValidation({ proposalReviewOptions: reviewOptions, proposalReviewReceiptTemplate: receiptTemplate, rollup: { summary, repos } });
  const reviewCopyBlock = proposalReviewCopyBlock.buildProposalReviewCopyBlock({ proposalReviewPacket: reviewPacket, proposalReviewOptions: reviewOptions, proposalReviewReceiptTemplate: receiptTemplate, proposalReviewReceiptValidation: receiptValidation });
  const reviewCopyBlockValidation = proposalReviewCopyBlockValidation.buildProposalReviewCopyBlockValidation({ proposalReviewPacket: reviewPacket, proposalReviewOptions: reviewOptions, proposalReviewReceiptTemplate: receiptTemplate, proposalReviewReceiptValidation: receiptValidation, proposalReviewCopyBlock: reviewCopyBlock, rollup: { summary, repos } });
  const reviewExportIntent = proposalReviewExportIntent.buildProposalReviewExportIntent({ proposalReviewPacket: reviewPacket, proposalReviewOptions: reviewOptions, proposalReviewReceiptTemplate: receiptTemplate, proposalReviewReceiptValidation: receiptValidation, proposalReviewCopyBlock: reviewCopyBlock, proposalReviewCopyBlockValidation: reviewCopyBlockValidation, rollup: { summary, repos } });
  const reviewExportSafetyGate = proposalReviewExportSafetyGate.buildProposalReviewExportSafetyGate({ proposalReviewPacket: reviewPacket, proposalReviewOptions: reviewOptions, proposalReviewReceiptTemplate: receiptTemplate, proposalReviewReceiptValidation: receiptValidation, proposalReviewCopyBlock: reviewCopyBlock, proposalReviewCopyBlockValidation: reviewCopyBlockValidation, proposalReviewExportIntent: reviewExportIntent, rollup: { summary, repos } });
  const controlledAutonomyPlan = autonomyPlan.buildAutonomyPlan({ proposalReviewPacket: reviewPacket, proposalReviewOptions: reviewOptions, proposalReviewReceiptTemplate: receiptTemplate, proposalReviewReceiptValidation: receiptValidation, proposalReviewCopyBlockValidation: reviewCopyBlockValidation, proposalReviewExportIntent: reviewExportIntent, proposalReviewExportSafetyGate: reviewExportSafetyGate, rollup: { summary, repos } });
  const controlledAutonomyApprovalReceiptValidation = autonomyApprovalReceiptValidation.buildAutonomyApprovalReceiptValidation({ autonomyPlan: controlledAutonomyPlan, autonomyApprovalReceipt: options.autonomyApprovalReceipt, rollup: { summary, repos } });
  const approvedManualWorkPacket = manualWorkPacket.buildManualWorkPacket({ nextActionBrief, proposalReviewPacket: reviewPacket, autonomyPlan: controlledAutonomyPlan, autonomyApprovalReceiptValidation: controlledAutonomyApprovalReceiptValidation });
  const manualWorkPacketArtifactValidation = options.manualWorkPacketArtifactValidation || packetArtifactValidation.defaultManualWorkPacketArtifactValidation();
  const operatorExecutionPlan = operatorExecutionPlanBuilder.buildOperatorExecutionPlan({
    manualWorkPacketArtifactValidation,
    verifiedManualWorkPacket: options.verifiedManualWorkPacket || null,
  });
  return {
    ...
    proposal_review_gate: reviewGate,
    proposal_review_packet: reviewPacket,
    proposal_review_options: reviewOptions,
    proposal_review_receipt_template: receiptTemplate,
    ["proposal_review_" + "receipt_validation"]: receiptValidation,
    proposal_review_copy_block: reviewCopyBlock,
    proposal_review_copy_block_validation: reviewCopyBlockValidation,
    proposal_review_export_intent: reviewExportIntent,
    proposal_review_export_safety_gate: reviewExportSafetyGate,
    autonomy_plan: controlledAutonomyPlan,
    autonomy_approval_receipt_validation: controlledAutonomyApprovalReceiptValidation,
    manual_work_packet: approvedManualWorkPacket,
    manual_work_packet_artifact_validation: manualWorkPacketArtifactValidation,
...
function renderDriftLine(warning) { return `  - DRIFT ${warning.id} ${warning.kind} — ${warning.reason}`; }
function renderActionLine(action) { return `  - ACTION ${action.priority} ${action.id} ${action.kind} — ${action.reason}`; }
...
function renderRepoRollupMarkdown(rollup) {
  ...
  lines.push(
    ...proposalValidation.renderProposalValidationMarkdown(rollup.proposal_validation),
    ...proposalReviewGate.renderProposalReviewGateMarkdown(rollup.proposal_review_gate),
    ...proposalReviewPacket.renderProposalReviewPacketMarkdown(rollup.proposal_review_packet),
    ...proposalReviewOptions.renderProposalReviewOptionsMarkdown(rollup.proposal_review_options),
    ...proposalReviewReceiptTemplate.renderProposalReviewReceiptTemplateMarkdown(rollup.proposal_review_receipt_template),
    ...proposalReviewReceiptValidation.renderProposalReviewReceiptValidationMarkdown(rollup["proposal_review_" + "receipt_validation"]),
    ...proposalReviewCopyBlock.renderProposalReviewCopyBlockMarkdown(rollup.proposal_review_copy_block),
    ...proposalReviewCopyBlockValidation.renderProposalReviewCopyBlockValidationMarkdown(rollup.proposal_review_copy_block_validation),
    ...proposalReviewExportIntent.renderProposalReviewExportIntentMarkdown(rollup.proposal_review_export_intent),
    ...proposalReviewExportSafetyGate.renderProposalReviewExportSafetyGateMarkdown(rollup.proposal_review_export_safety_gate),
    ...autonomyPlan.renderAutonomyPlanMarkdown(rollup.autonomy_plan),
    ...autonomyApprovalReceiptValidation.renderAutonomyApprovalReceiptValidationMarkdown(rollup.autonomy_approval_receipt_validation),
    ...manualWorkPacket.renderManualWorkPacketMarkdown(rollup.manual_work_packet),
    ...operatorExecutionPlanBuilder.renderOperatorExecutionPlanMarkdown(rollup.operator_execution_plan),
    ...(rollup.operator_execution_plan_artifact_validation
      ? opPlanArtifactValidation.renderOperatorExecutionPlanArtifactValidationMarkdown(rollup.operator_execution_plan_artifact_validation)
      : []),
    ...(rollup.execution_readiness
      ? executionReadiness.renderExecutionReadinessMarkdown(rollup.execution_readiness)
      : []),
    ...(rollup.entry_authority
      ? entryAuthority.renderEntryAuthorityMarkdown(rollup.entry_authority)
      : []),
    ...(rollup.worker_entry_gate
      ? workerEntryGate.renderWorkerEntryGateMarkdown(rollup.worker_entry_gate)
      : [])
  );
  return `${lines.join("\n")}\n`;
}
