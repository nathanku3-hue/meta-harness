...
const { buildPatchProposals } = require("./repo-rollup-patches");
...
function attachRollupDerived(root, repo, paths, repoResult, fsApi) {
  ...
  const withActions = {
    ...withDrift,
    action_candidates: buildActionCandidates(withDrift),
  };
  return {
    ...withActions,
    patch_proposals: buildPatchProposals(withActions),
  };
}
...
function buildRepoRollup(root, options = {}) {
  ...
  for (const repo of repos) {
    ...
    summary.action_candidates += Array.isArray(repo.action_candidates) ? repo.action_candidates.length : 0;
    summary.patch_proposals += Array.isArray(repo.patch_proposals) ? repo.patch_proposals.length : 0;
  }
  return {
    ...
    summary,
    response_handoff: buildResponseHandoff(repos),
    repos,
...
function renderActionLine(action) {
  return `  - ACTION ${action.id} ${action.kind} — ${action.reason}`;
}

function renderPatchProposalLine(proposal) {
  return `  - PATCH ${proposal.id} ${proposal.kind} — ${proposal.reason}`;
}
...
function renderRepoRollupMarkdown(rollup) {
  const lines = [
    ...
    "",
    `ready=${rollup.summary.ready} warned=${rollup.summary.warned} failed=${rollup.summary.failed} stale=${rollup.summary.stale} unknown=${rollup.summary.unknown} missing=${rollup.summary.missing} invalid=${rollup.summary.invalid} drift_warnings=${rollup.summary.drift_warnings} action_candidates=${rollup.summary.action_candidates} patch_proposals=${rollup.summary.patch_proposals}`,
    "",
    "## Child Repos",
    "",
  ];
  if (rollup.repos.length === 0) lines.push("- none");
    ...
    for (const action of repo.action_candidates || []) lines.push(renderActionLine(action));
    for (const proposal of repo.patch_proposals || []) lines.push(renderPatchProposalLine(proposal));
  }
