...
/**
 * Privacy-safe portable execution-custody export.
...
const EXPORT_SCHEMA = "portable-execution-custody-export/v1";
...
function parseJson(filePath, label) {
  ...
  try {
    ...
    throw codedError("CUSTODY_EXPORT_SOURCE", `${label} unreadable: ${err.message}`);
  }
  try {
    return { bytes, value: JSON.parse(bytes.toString("utf8")) };
  } catch (err) {
    throw codedError("CUSTODY_EXPORT_SOURCE", `${label} invalid JSON: ${err.message}`);
  }
}
...
function buildApprovalProjection(approval) {
  return {
    schemaVersion: "run-spec-approval-projection/v1",
...
function buildAttestationProjection(attestation) {
  return {
    schemaVersion: "workspace-attestation-projection/v1",
...
function buildAoProjection(meta) {
  return {
    schemaVersion: "agent-process-meta-projection/v1",
    ...
    redactions: ["identity executable paths", "argv", "environment", "raw agent streams"],
  };
}

function buildFactsProjection(facts) {
  return {
    schemaVersion: "implementation-facts-projection/v1",
...
function publishExportDirectoryNoReplace(stageDir, finalDir) {
  ...
  function visit(directory, relative = "") {
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
      ...
      else throw codedError("CUSTODY_EXPORT_PUBLISH", `unsupported staged entry: ${childRelative}`);
    }
  }
  visit(stageDir);
  if (!files.includes(manifestName)) {
    throw codedError("CUSTODY_EXPORT_PUBLISH", "portable manifest missing from staging");
  }
  try {
    fs.mkdirSync(finalDir, { recursive: false });
  } catch (err) {
    throw codedError(
      "CUSTODY_EXPORT_EXISTS",
...
function exportPortableCustody(options) {
  ...
  for (const [label, value] of Object.entries({
    ...
    if (typeof value !== "string" || value.length === 0) {
      throw codedError("CUSTODY_EXPORT_OPTIONS", `${label} required`);
    }
  }
  ...
  if (!isPlainObject(localManifest)
    ...
    throw codedError("CUSTODY_EXPORT_BINDING", "local terminal manifest does not match export request");
  }
  ...
  if (verifiedLocal.terminalManifestDigest !== terminalManifestDigest) {
    throw codedError("CUSTODY_EXPORT_BINDING", "verified local evidence digest does not match export request");
  }

  const exportName = `custody-${verifiedHeadRevision.slice(0, 12)}-${authReqHex.slice(0, 12)}`;
  fs.mkdirSync(exportsRoot, { recursive: true });
  const finalDir = path.join(exportsRoot, exportName);
  if (fs.existsSync(finalDir)) {
    throw codedError("CUSTODY_EXPORT_EXISTS", `portable export already exists: ${exportName}`);
  }
