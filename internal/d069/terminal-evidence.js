...
/** D072 immutable terminal evidence preparation, publication, and replay verification. */
...
const MANIFEST_SCHEMA = "d072-custody-manifest/v1";
...
function parseJsonFile(filePath, label) {
  ...
  try {
    ...
    throw codedError("D072_TERMINAL_EVIDENCE_CORRUPT", `${label} unreadable: ${err.message}`);
  }
  try {
    return { bytes, value: JSON.parse(bytes.toString("utf8")) };
  } catch (err) {
    throw codedError("D072_TERMINAL_EVIDENCE_CORRUPT", `${label} invalid JSON: ${err.message}`);
  }
}
...
function verifyEvidenceDirectory({ directory, stateRoot, authReqHex, receipt, claim, journal }) {
  ...
  if (!isPlainObject(manifest) || manifest.schemaVersion !== MANIFEST_SCHEMA) {
    throw codedError("D072_TERMINAL_EVIDENCE_CORRUPT", "custody manifest schema invalid");
  }
  const body = { ...manifest };
  ...
  if (!isNonEmptyString(manifest.terminalManifestDigest)
    ...
    throw codedError("D072_TERMINAL_EVIDENCE_CORRUPT", "custody manifest digest mismatch");
  }
  if (manifest.authorizationRequestDigest !== receipt.authorizationRequestDigest
    ...
    throw codedError("D072_TERMINAL_EVIDENCE_CORRUPT", "custody manifest terminal bindings mismatch");
  }

  if (!Array.isArray(manifest.files)) {
    throw codedError("D072_TERMINAL_EVIDENCE_CORRUPT", "custody manifest files must be an array");
  }
  ...
  for (const entry of manifest.files) {
    if (!isPlainObject(entry)
      ...
      throw codedError("D072_TERMINAL_EVIDENCE_CORRUPT", "custody manifest file entry invalid");
    }
  ...
  for (const name of REQUIRED_FILES) {
    ...
    if (!entry) {
      throw codedError("D072_TERMINAL_EVIDENCE_CORRUPT", `custody manifest missing ${name}`);
    }
    const filePath = path.join(directory, name);
    let stat;
    try { stat = fs.statSync(filePath); } catch (err) {
      throw codedError("D072_TERMINAL_EVIDENCE_CORRUPT", `${name} missing: ${err.message}`);
    }
    if (!stat.isFile() || stat.size !== entry.bytes || sha256File(filePath) !== entry.sha256) {
      throw codedError("D072_TERMINAL_EVIDENCE_CORRUPT", `${name} digest or size mismatch`);
    }
  }
  ...
  if (!approvalCheck.ok || approval.runSpecDigest !== receipt.runSpecDigest) {
    throw codedError("D072_TERMINAL_EVIDENCE_CORRUPT", "stored RunSpecApproval invalid or conflicting");
  }
  ...
  if (!receiptCheck.ok || storedReceiptRead.value.receiptDigest !== receipt.receiptDigest) {
    throw codedError("D072_TERMINAL_EVIDENCE_CORRUPT", "stored receipt invalid or conflicting");
  }
  ...
  if (!canonicalBytes.equals(storedReceiptRead.bytes)) {
    throw codedError("D072_RECEIPT_COPY_CONFLICT", "evidence receipt differs from canonical index");
  }
  ...
  if (storedClaim.claimDigest !== claim.claimDigest) {
    throw codedError("D072_TERMINAL_EVIDENCE_CORRUPT", "claim digest differs from manifest binding");
  }
  ...
  if (storedJournal.journalDigest !== journal.journalDigest) {
    throw codedError("D072_TERMINAL_EVIDENCE_CORRUPT", "terminal journal differs from binding");
  }
  ...
  if (assessment.implementationAssessmentDigest !== journal.implementationAssessmentDigest) {
    throw codedError("D072_TERMINAL_EVIDENCE_CORRUPT", "assessment binding mismatch");
  }
...
function prepareTerminalEvidence(options) {
  ...
  if (fs.existsSync(evidenceDir)) {
    throw codedError("D072_TERMINAL_EVIDENCE_EXISTS", "terminal evidence directory already exists");
  }
