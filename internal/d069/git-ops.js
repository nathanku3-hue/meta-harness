...
/**
 * D069 private Git runtime helpers (not packaged).
 */
...
function ensureIsolatedGitHome(stateRoot) {
  const home = path.join(stateRoot, ".d069-git-home");
...
function resolveGitExecutable(gitHome) {
  ...
  throw codedError("D069_GIT_UNRESOLVED", "failed to resolve absolute git executable");
}
...
function runGit(gitExecutablePath, repoCwd, args, gitHome, options = {}) {
  ...
  if (result.error) {
    throw codedError("D069_GIT_SPAWN_FAILED", result.error.message);
  }
  if (result.status !== 0) {
    throw codedError(
      "D069_GIT_FAILED",
...
function createOnlyRef(gitExecutablePath, repositoryPath, refName, newOid, objectFormat, gitHome) {
  if (refExists(gitExecutablePath, repositoryPath, refName, gitHome)) {
    throw codedError("D069_REF_EXISTS", `durable ref already exists: ${refName}`);
  }
  runGit(
    gitExecutablePath,
    repositoryPath,
