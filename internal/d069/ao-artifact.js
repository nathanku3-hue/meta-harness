...
/**
 * D070-A1 schema-bound change artifact: parse, extract, validate, materialize.
 * Content is NOT sealed by RunSpec; path is constrained by single-literal scope.allow.
 */
...
/**
 * A1 scope: exactly one literal allow path, empty deny, no globs.
 * @returns {string} the single allowed relative path
 */
function requireSingleLiteralScopePath(scope) {
  if (!isPlainObject(scope)) {
    throw codedError("D070_SCOPE_REQUIRED", "runSpec.scope is required");
  }
  ...
  if (!isNonEmptyString(allowed)) {
    throw codedError("D070_SCOPE_PATH", "scope.allow[0] must be a non-empty string");
  }
  if (path.isAbsolute(allowed) || allowed.includes("\0")) {
    throw codedError("D070_SCOPE_PATH", "scope.allow[0] must be a relative non-NUL path");
  }
  if (allowed.includes("\\")) {
    throw codedError(
      "D070_SCOPE_PATH",
  ...
  if (GLOB_CHARS_RE.test(allowed)) {
    throw codedError("D070_SCOPE_GLOB", "A1 rejects glob characters in scope.allow");
  }
  ...
  return {
    ...
    properties: {
      ...
      content: {
        type: "string",
        minLength: 1,
        maxLength: AO_CONTENT_MAX_BYTES,
      },
    },
  };
}

function parseCodexJsonl(stdoutText) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (line === "") {
      ...
      throw codedError("D070_JSONL_BLANK", `blank stdout line at index ${i}`);
    }
    let event;
    try {
    ...
    if (!isPlainObject(event)) {
      throw codedError("D070_JSONL_EVENT", `stdout line ${i} is not a JSON object`);
    }
    events.push(event);
  }
  if (events.length === 0) {
    throw codedError("D070_JSONL_EMPTY", "stdout produced no JSONL events");
  }
  return events;
}
...
function extractChangeArtifact(events) {
  if (!Array.isArray(events) || events.length === 0) {
    throw codedError("D070_EVENTS_REQUIRED", "events array required");
  }
  ...
  for (const event of events) {
    ...
    if (t === "turn.failed" || t === "error" || t === "thread.failed" || t.endsWith(".failed")) {
      throw codedError("D070_TERMINAL_ERROR", `failed event type: ${t}`);
    }
    if (t === "item.completed" && event.item && event.item.type === "error") {
      throw codedError("D070_TERMINAL_ERROR", "item.completed error item");
    }
  }
  ...
  for (let i = 0; i < lastCompletedIdx; i += 1) {
    ...
    if (typeof event.item.text !== "string") {
      throw codedError("D070_AGENT_MESSAGE", "agent_message.text must be a string");
    }
    agentMessages.push(event.item.text);
  }
  if (agentMessages.length === 0) {
    throw codedError(
      "D070_AGENT_MESSAGE_MISSING",
...
function parseExactJsonObject(text) {
  if (typeof text !== "string") {
    throw codedError("D070_ARTIFACT_TEXT", "artifact message must be a string");
  }
  ...
  if (!isPlainObject(value) || Array.isArray(value)) {
    throw codedError("D070_ARTIFACT_OBJECT", "artifact must be a JSON object");
  }
  return value;
}
...
function validateChangeArtifact(artifact, allowedPath) {
  if (!isPlainObject(artifact)) {
    throw codedError("D070_ARTIFACT_OBJECT", "artifact must be a plain object");
  }
  ...
  if (typeof artifact.path !== "string" || typeof artifact.content !== "string") {
    throw codedError("D070_ARTIFACT_TYPES", "path and content must be strings");
  }
  ...
  if (artifact.content.length === 0) {
    throw codedError("D070_ARTIFACT_EMPTY", "content must be non-empty");
  }
  if (artifact.content.includes("\0")) {
    throw codedError("D070_ARTIFACT_NUL", "content must not contain NUL");
  }
  const byteLen = Buffer.byteLength(artifact.content, "utf8");
  if (byteLen > AO_CONTENT_MAX_BYTES) {
    throw codedError(
      "D070_ARTIFACT_SIZE",
      `content ${byteLen} bytes exceeds ceiling ${AO_CONTENT_MAX_BYTES}`,
    );
  }
  return {
    path: artifact.path,
...
function materializeChangeArtifact(worktreePath, artifact) {
  if (!isNonEmptyString(worktreePath)) {
    throw codedError("D070_WORKTREE", "worktreePath required");
  }
  const rel = artifact.path;
  if (!isNonEmptyString(rel) || path.isAbsolute(rel) || rel.includes("\0")) {
    throw codedError("D070_MATERIALIZE_PATH", "artifact.path invalid");
  }
  ...
  try {
    ...
    throw codedError("D070_TARGET_MISSING", `target missing: ${err.message}`);
  }
  if (st.isSymbolicLink() || !st.isFile()) {
    throw codedError(
      "D070_TARGET_NOT_REGULAR",
