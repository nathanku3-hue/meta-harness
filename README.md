...
Planner state is disposable: before Claim visibility it may be recomputed, while after Claim visibility the Claim/session owns continuity. The planner is invoked only to fill unused initial local capacity and is not repeatedly awakened after the first landing; continuous refill is deliberately absent. `repo-decision/v3` remains historical-only. Unsupported owner-authority assertions cannot manufacture human authority; proof-backed Phase-4 `OWNER_REQUIRED` remains the only model-mediated owner-routing path.
