...
For bounded external delegation, `meta-harness-delegation-round2/v1` projects the exact PRODUCT Endgame, authoritative current World/WorldHead provenance, one explicit current gate, and immutable Outcome evidence references into DevSpace-compatible zero-history memory. Each lane's acceptance criteria derive mechanically from `Outcome.desiredState` and `Outcome.evidenceRequirement`. A lane always boots in a fresh ChatGPT conversation: DevSpace injects the deterministic Endgame → World → gate → lane brief → evidence index packet, then the child reattaches mechanical project continuity through `open_workspace(taskId=<exact retained taskId>)`. `lane-result-card/v1` fan-in is reduced to a compact acceptance matrix without child transcripts; disputed evidence is fetched only by refs already named by the compact card. Advisory lane `worldDelta` is never applied by this contract. See [Delegation Fresh-Session User Flow](docs/product/delegation-fresh-session-flow.md) for the complete ground-up journey.

Round 3 adds one bounded event-driven **evidence-lifecycle** orchestrator over that frozen seam. Result-bearing evidence lanes close and their compact cards become immutable `meta-harness-delegation-learning/v1`; the repository interpreter re-evaluates that learning against **current** World and commits a no-code `DELEGATION_LEARNING` World transition with replay protection. Still-live evidence lanes receive exactly one challenged `CONTINUE | HOLD | OBSOLETE` decision. HOLD persists a compact task/workspace checkpoint before exact `cancel_lane {delegationId,laneKey}`; obsolete lanes cancel directly; interrupted useful delegations use `resume_delegation`. `newOutcomes` is mechanically empty: Round 3 never spawns coding work. Claim-history Outcomes and legacy writable `r3-*` lanes fail closed back to the sole logical-planner → Outcome → Claim → controller-integration coding path. After planner + Grill + World recheck, a fresh compact host snapshot must still match task/workspace/status/result identity before lifecycle action. Only a changed semantic frontier can surface a new forward/owner gate, and durable surfaced-frontier evidence prevents restart repetition.

Round 4 makes that journey single-action without adding another authority plane. While retained evidence lanes are pending, the active repo-work invocation waits only on compact DevSpace lane state and WorldHead; unchanged polling does not rerun semantic models, and a real change wakes the same Round-3 reconciliation. Round 5 then breaks the failed shared-context assumption: the persistent authenticated profile remains the explicit human `web_launch` surface, while machine-owned ChatGPT jobs share one separate managed Chrome process and receive independent BrowserContexts seeded from copied authenticated storage state. Result, cancellation, failure, or expiry releases only that lane context. The delegation programme is closed against the accepted DevSpace `product/autopilot-alpha` browser-host candidate `94f8ea8`; later dirty browser edits in that worktree are separate owner state, not part of this closure. No queue, daemon, scheduler, provider router, browser-pool framework, swarm, persistent idle worker, or second coding dispatcher is introduced.
