...
**D076 release preparation is active without feature expansion:** exact closure commit `6893280` passed all 115 native Windows files and produced a 234-entry tarball with SHA-256 `7a28690d7227d669178f939eb87f1de0754f2d70e450a490873f6b528d4bd9d0`. Registry lookup returned E404 with no public record observed, so release version `0.2.0` is selected. Release mechanics commit `eaf7ed9` synchronizes package/MCP metadata and gives the proven suite one shared 300-second budget. GitHub secret scanning and push protection are enabled. Failed immutable candidates `2a190dd`, `8676afd`, pushed `be6eb58`, pushed `a05fcc5`, and pushed `3482db0` remain preserved. `3482db0` passed all 116 local native files in 224.9 seconds, exact 234-entry package proof, isolated installation, remote Linux Node tests, Semgrep, CodeQL, and hosted-Windows installed execution custody. Hosted Windows reached production judge Git discovery but failed only `judge.test.js`: the containment check treated the long `runneradmin` Git root and equivalent 8.3 `RUNNER~1` target as unrelated. The bounded repair keeps lexical checks for untrusted relative inputs while using bigint filesystem `dev` and `ino` across existing-directory ancestors for repository containment. It passes 9 focused tests on native Windows and Linux plus the complete 116-file native Windows worktree suite in 212.5 seconds. Branch protection, exact release evidence, tag, and publication remain pending.
...
- Pushed candidate `3482db00186e85fa9508668d19a27888b53a3c4b` / tree `a7db207cd0ca972b64df4b804b21d5310cc8b680` passed 116 local native files in 224.9 seconds and exact 234-entry package proof with tarball SHA-256 `acdb0577c114045623845ab1ab6f7fb717c47041558c2474fbc19035bf7ac9f3`, 481252 bytes. Remote Semgrep run `29393721755`, CodeQL run `29393721231`, Linux Node job `87282458156`, and hosted-Windows installed execution custody passed. Windows job `87282458170` failed only `judge.test.js` after Git discovery succeeded because alias-sensitive repository containment returned `JUDGE_INPUT_TARGET_NOT_GIT` for equivalent long and 8.3 directory spellings. Candidate `3482db0` remains immutable and untagged/unpublished. The alias-safe existing-directory containment repair passes 9 focused tests on native Windows, 9 on Linux, and the complete native Windows worktree suite across 116 files with zero failures in 212.5 seconds.
