...
Active streams:
- coding: D075 implementation is audit-accepted; the active gate is one immutable candidate, exact suite, then two distinct private operations through the same production-owned seam.
- research: closed; direct write remains NO-GO and controller-materialized artifact execution remains the selected seam.
- writing: D075 private request/receipt boundary, candidate/live gate, and friction record; D074 closure and failed-candidate history remain preserved.
- review: offline operator, shared live adapters, exact request binding, candidate clean-before/after checks, shallow authority, replay, verifier, and leakage paths pass; authenticated DevSpace and Fluxara operator receipts are still absent for D075.
...
- D075: **implementation audit accepted; immutable candidate and repeated operator use pending — Private Operator Use Gate**. One unregistered local request entrypoint now invokes a production-owned orchestration path; existing live tests use that same path. Public CLI/package/contracts/examples remain frozen. Closure requires one DevSpace/Node and one Fluxara/Python operation on the exact candidate, each with a distinct create-only root and retained operator receipt.
...
- Roadmap honesty improves because D074 closes only from retained live evidence and D075 OPERATE remains the next gate before public-surface decisions or deletion.
...
- The private seam now exists outside tests, but repeated real operator usability is unproved. D075 remains blocked on an immutable candidate plus two retained operations and measured setup friction before DECIDE or broad DELETE.
...
- D075 focused operator/custody coverage passes 17, skips only the DevSpace and Fluxara authenticated gates, and fails zero. A baseline native Windows Node `v25.2.1` suite passes 113 files, zero failures, exit 0 in 186.1 seconds with unchanged path status. Because the candidate-after-operation identity recheck landed while that suite was in flight, it is baseline evidence only; final truth-aligned and exact-candidate suites are still required.
...
Next action:
Complete D075 truth alignment and the final native 113-file pre-candidate suite, then create one immutable candidate without amend. Run the exact candidate suite with all live flags unset. Next invoke only `scripts/operate-execution-custody.js` with ignored absolute local requests for `d075-devspace-01` and `d075-fluxara-01`, each using a distinct create-only root. Require exact candidate identity and clean-before/after, exact shallow pinned authority, one authenticated spawn → VERIFIED, normal close, process 2 at retained receipt expiry +60 seconds with unusable tools → REPLAY and zero spawns, independent child validation, leakage PASS, and create-only operator receipt. Preserve every root on failure and never reuse an operation identity/root. Record actual setup friction and close D075 in a separate commit only after both operations pass. Do not add a public CLI, provider registry, compatibility path, delivery semantics, concurrency framework, or broad deletion. DECIDE and DELETE remain blocked.
