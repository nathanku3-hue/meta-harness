...
function initialReceipt(authority, options = {}) {
  ...
  return signTruthReceipt({
    ...
    proposal: {
      ...
      rejected_event_digests: operation === "reconcile"
        ? [`sha256:${"1".repeat(64)}`]
        : [],
    },
  });
...
test("init consumes external authority and stores verifier material only", () => {
  ...
  const privatePath = path.join(cwd, ".meta-harness", "local", "truth-authority-private.json");
...
test("invalid bootstrap contracts and receipts leave the target byte-identical", () => {
  const cases = [
    ["invalid signature", (authority) => {
      const receipt = initialReceipt(authority);
      receipt.signature = `${receipt.signature.slice(0, -2)}aa`;
      return [authority.publicDocument, receipt, /signature is invalid/i];
    }],
    ["expired", (authority) => [
      authority.publicDocument,
      initialReceipt(authority, { issuedAt: "2026-01-01T00:00:00.000Z", ttlSeconds: 1 }),
      /outside its validity window/i,
    ]],
    ...
    ["wrong repository", (authority) => [
      authority.publicDocument,
      initialReceipt(authority, { signingAuthority: { ...authority, repositoryId: "test-repository:other" } }),
      /different repository identity/i,
    ]],
    ["wrong prior", (authority) => [
      authority.publicDocument,
      initialReceipt(authority, { priorSnapshotDigest: `sha256:${"0".repeat(64)}` }),
      /not bound to the current canonical snapshot/i,
    ]],
    ["non-snapshot", (authority) => [authority.publicDocument, initialReceipt(authority, { operation: "reconcile" }), /must authorize a snapshot/i]],
    ["tampered proposal", (authority) => {
      const receipt = initialReceipt(authority);
      receipt.proposal.goal = "tampered";
      return [authority.publicDocument, receipt, /proposal_digest|signature/i];
    }],
    ["RSA authority", (authority) => {
      const rsa = crypto.generateKeyPairSync("rsa", { modulusLength: 2048 }).publicKey.export({ format: "jwk" });
      return [{ ...authority.publicDocument, public_key: rsa }, initialReceipt(authority), /OKP Ed25519/i];
    }],
  ];
...
test("failed bootstrap cannot pre-claim authority and a legitimate retry succeeds", () => {
  ...
  const installed = JSON.parse(fs.readFileSync(path.join(cwd, ".meta-harness", "contracts", "truth-authority-public.json"), "utf8"));
  assert.equal(installed.repository_id, "audit:legitimate");
});
...
test("concurrent bootstrap accepts exactly one authority", async () => {
  ...
  assert.match(failure.stderr, /bootstrap is already in progress|initial canonical state|different repository identity/i);
  const installed = JSON.parse(fs.readFileSync(path.join(cwd, ".meta-harness", "contracts", "truth-authority-public.json"), "utf8"));
  assert.equal([first.repositoryId, second.repositoryId].includes(installed.repository_id), true);
...
test("legacy authority presence returns an actionable migration error without mutation", () => {
  const cwd = tempDir("truth-bootstrap-legacy-present-");
  const authority = createExternalAuthority("audit:legacy");
  writeJson(cwd, ".meta-harness/contracts/truth-authority-public.json", authority.publicKey);
  writeJson(cwd, ".meta-harness/repos.json", { repos: [] });
  const before = treeSnapshot(cwd);

  assertCliError(
    runRaw(cwd, initArgs(authority.publicDocument, initialReceipt(authority))),
    "MH_TRUTH_AUTHORITY",
    /legacy.*reviewed structured authority migration/i,
  );
...
test("legacy Ed25519 history remains readable while non-Ed25519 legacy keys are rejected", () => {
  ...
  const proposal = createTruthProposal({
    operation: "snapshot", stream: "coding", phase: "verify", action: "verify legacy",
    result: "legacy history remains readable", goal: "preserve D078 evidence",
    next_action: "migrate only through a reviewed candidate", stop_criteria: "stop on verification loss",
    evidence: null, decision: null, occurred_at: issuedAt, rejected_event_digests: [],
  });
  const body = {
    schema_version: LEGACY_RECEIPT_SCHEMA,
    receipt_id: "legacy-readable",
    capability: CAPABILITY,
    prior_snapshot_digest: null,
    proposal,
    proposal_digest: domainDigest("truth-proposal/v1", proposal),
    signer_key_id: signerKeyId(authority.publicKey),
    issued_at: issuedAt,
    expires_at: new Date(Date.parse(issuedAt) + 300000).toISOString(),
  };
  const signature = crypto.sign(
    null,
    Buffer.from(`truth-authority-receipt/v1\u001e${canonicalize(body)}`, "utf8"),
