...
test("CI runs one complete suite per platform with phase-neutral identities", () => {
  const ci = read(".github/workflows/ci.yml");

  assert.match(ci, /runs-on:\s*ubuntu-latest[\s\S]*?run:\s*npm test/i);
  assert.match(
    ci,
    /name:\s*Windows complete suite[\s\S]*?runs-on:\s*windows-latest[\s\S]*?run:\s*npm test/i,
  );
  assert.doesNotMatch(ci, /d0(?:69|70|71|72)-windows|D0(?:69|70|71|72) Windows/i);
  assert.doesNotMatch(
    read("scripts/run-tests.js"),
    /runtime-d0(?:70|71|72)/i,
  );
});
...
test("historical audits remain while the active legacy runtime is deleted", () => {
  ...
  for (const active of [
    ...
    "internal/execution-custody/example.js",
    "tests/runtime-execution-custody.test.js",
    "tests/fixtures/execution-custody/known-good-message.js",
  ]) {
  ...
  const d072 = JSON.parse(
    read("docs/ops/audits/d072-exact-commit-live-gate-audit.json"),
  );
  assert.equal(d072.decision.id, "D073");
  assert.equal(d072.decision.status, "approved");
  assert.equal(d072.liveGate.verifiedDispositionProduced, false);
});
