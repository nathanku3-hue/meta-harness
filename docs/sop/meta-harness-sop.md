...
The accepted R2 comparison remains in `docs/ops/audits/d088-cross-repository-proof.json`. The R3 candidate proof is retained in `docs/ops/audits/d089-r3-entry-authority-proof.json`: the tracked runtime collector derives live Git identity and explicit product-path custody; Meta-Harness and Quant each redirect from the contaminated primary and pass at exact authority; checkout-local self-claim blocks; Leningrad requires custody because 30 Alpha 0 product files are absent from the named base authority. Six cases passed with measured elapsed time, 4,452 input-context bytes, and zero human questions; focused contracts pass 62/62 and the complete pinned-Node suite passes 125/125 test files in 212.9 seconds.
