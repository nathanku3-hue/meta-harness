...
The accepted R2 comparison remains in `docs/ops/audits/d088-cross-repository-proof.json`. The R3 repair-candidate proof is retained in `docs/ops/audits/d089-r3-entry-authority-proof.json`: the normal poll/worker-entry path derives expected identity from a controller-supplied validated RunSpec, collects live Git identity and explicit product-path custody, and fails closed without that controller input. Meta-Harness and Quant each redirect from the contaminated primary and pass at exact authority; raw self-attestation blocks; Leningrad requires custody because 30 Alpha 0 product files are absent from the named base authority; an unrelated repository cannot be misclassified as custody required. Six library cases record 7,618 serialized evaluator-input bytes and zero human questions. Focused production-path contracts pass 80/80, and the pre-freeze repair tree passes all 125 test files under Windows Node 25.2.1. R3 proves actionable entry classification, not fresh-session product-next-action reconstruction.

R3 reuses existing identity and readiness primitives, adds one read-only collector and no public command, and leaves the score at `40 / 100` until independent acceptance. No external product implementation begins before that gate. R4 then uses the accepted method to select and ship one real product slice across the proving repositories. That selected slice must still materially combine software engineering with explicit specialist judgment under D085/D086.
