...
For repositories that opt into the repo Decision Plane, work dispatch first requires a current `repo-world/v1` and `repo-decision/v1` bound to live `PRODUCT.md`, charter, optional owner-directive bytes, charter-required external-input identities, and the current committed tracked execution substrate. Terminal routes require a retained `PASS` validity basis and cannot become `LEGAL` candidates without new world evidence explicitly invalidating the prior basis. After the controller consumes the first attempt's ExecutionPermit, it records Decision Plane attempt consumption before worker invocation; worker exceptions therefore cannot make the decision reusable, while bounded repairs in the same work invocation remain permitted. These pre-execution semantics do not grant outcome-read, evaluation, trial-debit, state-transition, or route-reopen capabilities to the coding worker.
