...
Runtime fidelity is operation-fixed rather than planner-selected. Meta-Harness schema/Git/custody/SAW/validation work remains ordinary local execution; structured frontier/Grill reasoning remains the existing ephemeral headless model path. Browser reality stays in DevSpace. Round 4 originally proved the active-turn wait/reconciliation UX and exposed that several authenticated ChatGPT lanes were not reliable when implemented as pages inside one shared persistent BrowserContext. That observed defect was the only warrant for Round 5.

A delegation gate is owner-visible only when it is still bound to the final current WorldHead after ordinary repository work has had a chance to advance product truth. Thus `go delegation` remains one active product action: compact evidence changes wake reconciliation automatically, while stale gates disappear rather than requiring the owner to manage streams.

### Delegation Round 5 authenticated browser isolation + programme closure

Round 5 removes the failed shared-context/page assumption without changing delegation authority. The dedicated persistent authenticated ChatGPT profile remains the authentication source and explicit human `web_launch` surface. Machine-owned ChatGPT jobs use one separate managed Chrome process; every lane receives an independent BrowserContext seeded from copied persistent-profile storage state, including IndexedDB where supported. Fresh conversation establishment can run concurrently across those isolated contexts. Result, cancellation, failure, or expiry closes only that lane context and leaves siblings alive.

This is a browser ownership boundary, not a browser-pool/provider/scheduler framework. The controller retains only one managed browser process plus a set of disposable lane contexts. Duplicate conversation identity still fails closed. No provider router, queue, daemon, swarm, persistent idle worker, second coding dispatcher, or new Claim authority is introduced.

The delegation programme is closed at this architecture boundary against the accepted DevSpace `product/autopilot-alpha` candidate `94f8ea8` (`Fix authenticated managed browser isolation`) and its retained passing acceptance/regression evidence. Later dirty edits in that DevSpace worktree are separate owner/worktree state and are excluded from this programme closure unless independently accepted. Do not create a Round 6 from programme momentum; reopen only for a newly observed supported-use defect with retained evidence and a smallest repair.
