...
The R3 candidate extracts the existing repository identity comparator, adds one pure evaluator, and adds one tracked runtime collector that derives live repository identity, HEAD, ref, cleanliness, and explicit product-path custody through read-only Git processes. The normal `poll --rollup --verify-operator-execution-plan` path runs the collector before worker entry; trusted expected identity is mandatory and fail-closed; repository identity precedes custody classification; and the worker-entry gate recomputes the result instead of trusting a claimed verdict. It returns only `PASS_CURRENT`, `REDIRECT`, `CUSTODY_REQUIRED`, or `BLOCK`; adds no public command; and performs no mutation, network access, ref creation, or worktree creation. The pure evaluator spawns no process; the runtime collector spawns read-only Git processes. The six-case proof across Meta-Harness, Quant, and Leningrad passed through the tracked collector with measured elapsed time, 4,452 serialized evaluator-input bytes, and zero human questions. Leningrad custody was derived from 30 Alpha 0 product files absent from the named base authority. Production-path tests prove correct checkout open, stale-path redirect, self-claim block, absent-authority fail-closed, and unrelated-repository custody rejection. Evidence is retained in `docs/ops/audits/d089-r3-entry-authority-proof.json`. R3 does not yet prove fresh-session product-next-action reconstruction; the live Quant F1B loop supplies that proof after independent R3 acceptance. R4 remains blocked until acceptance, then ships one real product slice. R5 measures continuation across repositories. R6 compares benefit and deletes or shrinks obsolete guidance. Independent domain validation remains a separate later gate for the selected specialist product.
