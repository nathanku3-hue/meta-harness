...
As of D070/D071, the governing post-MVP direction is a local authority-bound agent execution-custody harness. A sealed RunSpec and approval authorize one isolated attempt; an authenticated read-only agent returns a bounded artifact; the controller materializes, commits, and validates it. D071 proved that functional path in ToolLauncher. D072 implemented durable receipt-first replay, terminal evidence, fresh-process recovery, and portable verification offline, but exact candidate `5d677a8` failed the legacy live gate during ToolLauncher PowerShell validation before terminal publication. D073 therefore carries the custody substrate into one host-neutral replacement functional slice and closes custody there while deleting the former path.

This is an intentional deviation from the MVP constraints below: the private Phase 23A runtime launches an agent and uses its authenticated network path. The original CLI and Markdown surfaces remain shipped MVP behavior, but the no-launch/no-network rule is historical MVP scope rather than the current end-state. No public execution CLI or generic provider abstraction is authorized before D073 live VERIFIED → fresh-process REPLAY → portable verification closes and a separate product decision opens that surface.
