...
The currently published package remains a Markdown-first CLI and does not yet contain the private execution-custody runtime. Private authenticated subagent execution is a proven local implementation claim through D075, not yet an installed-package claim. D076 is the bounded bridge: one shipped `execute` command from an isolated packed installation. Dashboards, daemon mode, generic worker orchestration, and full autonomy remain future phases, not current claims.
...
| 23A | Controlled Execution Vertical Slice | D075 closed; D076 installed-package execution authorized | Authority kernel and host-neutral execution custody are closed through D075. D076 authorizes exactly one shipped command, `meta-harness execute --request <absolute-path> [--json]`, but rejects source-checkout wrapper closure. The gate must pass from an isolated `npm pack` installation on one novel user-authored bounded change before DELETE. See `phase-23a-execution-plan.md` + `runtime-authority-architecture.md`. |
...
| D076 / SHIP | Installed-Package Execution Surface | **authorized** | Add exactly `meta-harness execute --request <absolute-path> [--json]`. Closure requires an isolated `npm pack` install with no Meta-Harness source checkout or `.git`, a versioned public request for one real bounded change not present in the tracked DevSpace/Fluxara fixtures, exact one-revision authority, one authenticated spawn → VERIFIED, retained custody, expiry+60s zero-spawn REPLAY, independent validation, leakage PASS, and a create-only public receipt. Replace the private request/script; no alias, compatibility, provider, concurrency, delivery, workflow, daemon, queue, or dashboard. Audit: `d076-public-execution-surface-decision-audit.json`. |
| DELETE | Consumer-Led Product Reduction | **after D076 closure** | Preserve supported user jobs, not incidental internal callers. Delete only after the installed replacement command is proven and the retained jobs are explicit. Consolidate truth recording under `record`, reporting under `status`, health under `check`, and retain `init`, `sync`, and `release check` only while supported. No aliases, deprecation dispatch, compatibility path, or net active-surface growth. |
