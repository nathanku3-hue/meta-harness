...
| 20K | Explicit Export-File Workflow | prototype | Future only if ever needed / non-goal: explicit file export is a separate write-surface phase and must not be bundled with read-only copy block or safety checks. |
