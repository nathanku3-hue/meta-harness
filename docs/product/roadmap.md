...
| 23A | Controlled Execution Vertical Slice | D073 closed; D074 audited repair candidate next | Authority kernel and host-neutral execution custody are closed through D073. D074 first candidate proved authenticated DevSpace execution/replay/export but exposed a shallow-prerequisite anchoring defect in the test-side independent verifier; the bounded repair and complete audit suite now pass, and a new immutable candidate is required. D075 then proves private operator use before public-surface reduction. See `phase-23a-execution-plan.md` + `runtime-authority-architecture.md`. |
...
| D074 / PROVE | Cross-Ecosystem Reuse Proof | **first immutable candidate failed at portable-verifier anchoring; new candidate pending** | Candidate `87472e1` / tree `7a447d81` passed native Node 25 `npm test` across 112 files with zero failures. Its only DevSpace live attempt used exact shallow authority, spawned the authenticated agent once, retained terminal VERIFIED child `b821c485` and a durable ref, passed receipt-derived expired zero-spawn REPLAY assertions, exported portable evidence, and passed leakage scanning. The test exited 1 only because the independent verifier fetched the shallow prerequisite object without anchoring it to a local ref before `git bundle verify`. The candidate and create-only root remain immutable. A two-file test repair anchors `refs/verify/base`; a real shallow thin-bundle regression and retained-export reverification pass with exact parent/path, both Node validations exit 0, and leakage PASS. The final audit-aligned native suite also passes 112 files with zero failures and unchanged worktree status. Production skill/runtime/kernel/CLI/package surfaces remain unchanged. Next: bank a new immutable candidate, exact suite, then one DevSpace-only authenticated closure gate. Audits: `d074-pre-candidate-functional-slice-audit.json`, `d074-candidate-87472e1-live-failure-audit.json`. |
| D075 / OPERATE | Private Operator Use Gate | **after D074** | Use the proven example contract through one minimal private operator seam for repeated real changes and record actual friction. This is not a public CLI and does not add compatibility, provider, delivery, concurrency, or workflow frameworks. The historical Markdown surface remains available until replacement usability is demonstrated. |
