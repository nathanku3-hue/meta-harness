...
| RELEASE | Exact-Commit `0.2.0` Publication | **Hosted-Windows judge path repair active; new candidate required** | D076 closure commit `6893280` passed 115 native files and a 234-entry exact package proof. Registry lookup observed no public record; `0.2.0` is selected. Failed immutable candidates `2a190dd`, `8676afd`, and pushed `be6eb58` remain preserved. `be6eb58` passed all 116 local native files, exact package proof, isolated installation, remote Linux Node tests, Semgrep, CodeQL, and hosted-Windows installed execution custody; Windows failed only `judge.test.js` because the judge used a canonicalized temporary path for Git discovery after fixture Git setup had succeeded on the caller path. The bounded two-file repair preserves production/package/public contracts and canonical containment, passes 8 focused tests on Windows and Linux, and passes the literal 116-file native Windows worktree suite in 204.5 seconds. Remaining gate: bank repair/truth, refresh baseline, replacement candidate, candidate-native `npm test`, exact package proof, push, all remote checks green, branch protection, exact-commit evidence, verified `v0.2.0`, publish-mode PASS, then npm publication. |
