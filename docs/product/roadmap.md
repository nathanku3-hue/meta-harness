...
| RELEASE | Exact-Commit `0.2.0` Publication | **release preparation active; remote evidence pending** | D076 closure commit `6893280` passed 115 native files and a 234-entry exact package proof. Registry lookup observed no public record; `0.2.0` is selected. Commits `eaf7ed9`, `4fedec9`, and `2fc3206` align metadata, release-test budget, quality ratchet, and the sole CodeQL alert. Secret scanning, push protection, and CodeQL default setup are enabled. Remaining gate: final exact release commit, complete suite/package proof, push, successful CI/Semgrep/CodeQL with alert closure, branch protection, exact-commit evidence, verified `v0.2.0`, publish-mode PASS, then npm publication. |
