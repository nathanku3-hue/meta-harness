...
| 20J | Read-Only Export Intent / Export Safety Gate | prototype | Future: read-only export intent and export safety gate is a separate safety phase and must precede any file writing. |
| 20K | Explicit Export-File Workflow | prototype | Future only if ever needed: explicit file export is a separate write-surface phase and must not be bundled with read-only copy block or validation. |
