...
| RELEASE | Exact-Commit `0.2.0` Publication | **Windows CI repair active; new candidate required** | D076 closure commit `6893280` passed 115 native files and a 234-entry exact package proof. Registry lookup observed no public record; `0.2.0` is selected. Failed candidates `2a190dd` and pushed `8676afd` remain immutable. `8676afd` passed all 116 local native files, exact package proof, remote Node tests, Semgrep, and CodeQL; Windows CI failed only the non-serialized Git-heavy judge file and an installed fixture that reused npm's checkout-prefixed PATH. The two-file repair preserves production/package/public contracts and passes 9 focused tests with leakage assertions unchanged. Remaining gate: replacement candidate, literal native Windows `npm test`, exact package proof, push, all remote checks green, branch protection, exact-commit evidence, verified `v0.2.0`, publish-mode PASS, then npm publication. |
