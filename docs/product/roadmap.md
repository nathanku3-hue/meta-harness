...
| 23A | Controlled Execution Vertical Slice | D073 closed; D074 next | Authority kernel and host-neutral execution custody are closed through D073. D074 adds a third child through example/test only. See `phase-23a-execution-plan.md` + `runtime-authority-architecture.md`. |
...
| D074 / PROVE | Third-Child Iteration Proof | **next** | Add a third real child by changing only one example under the existing bounded-repository-change skill and one end-to-end test, plus a deterministic child fixture only when necessary. The generic `SKILL.md`, runtime, kernel, CLI, roadmap, and architecture truth remain unchanged. VERIFIED, graceful fresh-process REPLAY, portable verification, and leakage PASS are required. |
| DELETE | Consumer-Led Product Reduction | **after PROVE** | Preserve supported user jobs, not incidental internal callers. Consolidate workflow truth recording under `record`; collapse reporting under `status`; collapse health checks under `check`; retain `init`, `sync`, and `release check` only while those jobs remain supported. Delete everything without a current supported job or unique safety invariant. No aliases, deprecation dispatch, compatibility path, or net active-surface growth. |
| DECIDE | Public Execution Surface | **only after repeated real use** | Consider a public execution command only when repeated operator use demonstrates one stable interface. No concurrency, cancellation, delivery, recovery, plugin, provider, or workflow framework is added speculatively. |
...
README says "Markdown-first workflow harness," while the implemented MVP specification says the harness does not launch agents and requires no network/model API. Phase 23A has crossed that boundary with authenticated AO execution custody. The re-charter is explicit: the post-MVP product direction is a local authority-bound agent execution-custody harness; the lightweight Markdown MVP is the historical shipped surface, not the governing end-state. D071 proved useful work but lost terminal custody. D072 repaired the custody architecture offline, while `5d677a8` showed that the legacy live closure gate still conflates custody with ToolLauncher/Windows/model-output behavior. D073 closed that host-neutral replacement slice under `87de018`, including durable live custody, fresh-process zero-spawn replay, portable verification, leakage controls, and deletion of the former path. D074 now demonstrates a third child without core edits; only afterward does the consumer-led DELETE sweep reduce the public surface.
