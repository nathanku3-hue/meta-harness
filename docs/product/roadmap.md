...
| 23A | Controlled Execution Vertical Slice | D075 implementation accepted; private operations next | Authority kernel and host-neutral execution custody are closed through D074. D075 now has one production-owned private operator workflow and an unregistered local request entrypoint; tests delegate to the same path and a 113-file baseline suite passes. An immutable candidate plus retained DevSpace/Node and Fluxara/Python operations are required before public-surface reduction. See `phase-23a-execution-plan.md` + `runtime-authority-architecture.md`. |
...
| D075 / OPERATE | Private Operator Use Gate | **implementation audit accepted; candidate and repeated use pending** | `internal/execution-custody/operator.js` owns exact shallow authority, fresh-process VERIFIED, receipt-expiry+60s zero-spawn REPLAY, portable verification, leakage, and operator receipt production. `scripts/operate-execution-custody.js` accepts one ignored absolute local request and is absent from bin/package/public exports. Existing live tests delegate to the same workflow. Closure requires one immutable candidate and distinct retained DevSpace/Node and Fluxara/Python operations with clean-before/after candidate identity and recorded friction. No compatibility, provider, delivery, concurrency, or workflow frameworks. Audit: `d075-private-operator-functional-slice-audit.json`. |
| DECIDE | Public Execution Surface | **after repeated D075 use** | Decide whether one stable public execution command is warranted from actual operator evidence. No generic provider abstraction or speculative framework. |
