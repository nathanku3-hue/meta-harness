...
| 23A-PR3 / D070 | AO Substitution | **A0 decided; A1 closed** | A0.1 workspace-write NO-GO; A0.2 schema-bound artifact GO. A1 closed: Codex `:read-only` → schema `{path,content}` → post-AO clean custody → controller materialize/commit → exact-byte validation → `IMPLEMENTATION_VERIFIED` → create-only durable ref → replay. Process-tree timeout (120s AO). No raw stream persistence. Provider `meta-harness-ao-codex` / `d070-ao-artifact-v1`. Content not RunSpec-sealed. |
| 23A-PR4 | Child-Repo Dogfood | **next** | Run one real child-repository path before overlap/cancellation. Dogfood precedes concurrency so the next control is driven by observed product need. |
| 23A-PR4B | Concurrency / cancellation | only if dogfood requires it | Same-request overlap, distinct-request serialization, and cancellation are no longer automatic. Minimal process-tree timeout custody already shipped in A1; broader frameworks only from observed need. |
