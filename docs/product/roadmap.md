...
| 23A | Controlled Execution Vertical Slice | D075 private operator use closed; DECIDE next | Authority kernel and host-neutral execution custody are closed through D074. D075 closed under immutable candidate `cd63e52`: the exact 113-file suite and retained DevSpace/Node plus Fluxara/Python operations passed through one production-owned private operator workflow. Public-surface implementation and deletion remain blocked pending DECIDE. See `phase-23a-execution-plan.md` + `runtime-authority-architecture.md`. |
...
| DECIDE | Public Execution Surface | **next after closed D075 evidence** | Decide whether the proven private seam should remain private or justify one narrow stable public execution command. Use actual setup/latency evidence; no generic provider abstraction, compatibility path, speculative framework, or implementation is pre-authorized. |
| DELETE | Consumer-Led Product Reduction | **after OPERATE and DECIDE** | Preserve supported user jobs, not incidental internal callers. Delete only after the replacement operator path is usable and the retained jobs are explicit. Consolidate truth recording under `record`, reporting under `status`, health under `check`, and retain `init`, `sync`, and `release check` only while supported. No aliases, deprecation dispatch, compatibility path, or net active-surface growth. |
