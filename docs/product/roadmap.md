...
Version: `endgame-v8 + link-confined clean candidate`
...
D081 preserves verifier-only authority and candidate-before-G-001 ordering, but rejects S-001R3 path confinement. The next repair and candidate isolation are combined in one clean worktree based on upstream so the project does not perform another ambiguous dirty-worktree repair cycle. The original long-range capabilities remain valid endgame candidates, but they are not automatic prerequisites.

| Step | Slice / gate | Product problem | Exit condition |
|---:|---|---|---|
| 1 | `CANDIDATE-S001R4` Link-confined clean truth candidate | S-001R3 closes D080 but recursively preserves linked harness paths on POSIX, canonical append follows an external ledger link, and mutating re-init writes through POSIX symlinks and Windows junctions. Another repair in the dirty checkout would repeat custody ambiguity. | Create an isolated worktree from `origin/main` at `0791efa`; transplant only accepted S-001 paths; require absent `.meta-harness` for one-shot bootstrap; remove recursive partial-harness copying, backup replacement, and mutating re-init; reject symlinks, junctions, and equivalent reparse points on canonical authority, ledger, status-parent, and lock paths; add bounded stale-lock recovery; track the structured public authority contract v1 using the current Ed25519 key plus legacy-readable event/status evidence; pass complete Windows Node 25, Linux/Windows link probes, quality, package, and diff checks; produce one named reviewable commit. |
| 2 | `INDEPENDENT-AUDIT-CANDIDATE-S001R4` Exact-commit acceptance | The repaired behavior must be proven on the check-outable candidate rather than inferred from the dirty source checkout. | A fresh auditor checks the named commit and checkout, repeats bootstrap, link/junction escape, canonical append, stale-lock, receipt, replay, legacy, quality, package, and fresh-checkout probes, and accepts the exact candidate without private material. |
| 3 | `G-001` External authority continuity proof | The current signer key is pinned but matching external private-capability continuity is unproven. | In a fresh checkout of the accepted `CANDIDATE-S001R4`, produce one unique receipt v2 with a lifetime of at most 300 seconds using the matching external capability, validate it against the tracked structured public authority contract v1, submit it through `event --canonical`, and observe exactly one accepted append without private material entering target, package, logs, or evidence. If the capability is unavailable, stop at a human authority-epoch migration decision. |
...
| 10 | `S-020M` Outcome learning | Historical evidence does not yet improve recommendations. | A versioned, reversible policy change makes a falsifiable prediction and improves holdout recommendation quality without shipping regressions. |
...
- `S-001A` standalone projection contract: fold only the provenance required to close `CANDIDATE-S001R4`; expand later only from observed failure.
...
```text
CANDIDATE-S001R4 — build one link-confined reviewable candidate directly from origin/main
```

The product remains verifier-only. S-001R3 independently proves pre-write semantic validation, Ed25519-only verification, a 300-second receipt ceiling, verifier-only packaging, concurrent normal bootstrap exclusion, D078 legacy readability, full supported-runtime tests, and the quality ratchet. It is rejected only because truth paths can follow symlinks, junctions, or external ledger links and because `init` retains a mutating re-init mode.

Build `CANDIDATE-S001R4` in an isolated worktree from `origin/main` at `0791efa`; do not merge upstream into the heavily dirty checkout. Bootstrap becomes one-shot and requires absent `.meta-harness`. The staged harness is created fresh and promoted with one rename. Existing or legacy harness state requires an explicit reviewed migration, not recursive copying or compatibility repair. Every canonical truth path and ancestor must be a regular non-link/reparse surface contained by the target.

The tracked verifier schema is `meta-harness-truth-authority-public/v1`; the repository-bound receipt schema is `meta-harness-truth-authority-receipt/v2`. Do not invent an authority-contract v2 merely to match earlier prose.
