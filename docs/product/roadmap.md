...
| 22B | Worker Gate Consumption Contract | planned | Read-only only. Machine-consumable worker preflight over fresh `poll --rollup --verify-operator-execution-plan`. Require validation + resolution + readiness fields before any manual/operator work. Optional strict `worker_entry_gate` / `operator_work_gate` object (no aliases). **No** execution, child writes, apply, tasks, queues, or command-running beyond existing readiness inspection. |
