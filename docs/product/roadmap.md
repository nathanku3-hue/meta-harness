...
| 23A-PR4 / D071 | Meaningful Single-File Child Dogfood | **next** | Use an isolated clean ToolLauncher snapshot at `7fab419f20ba`; modify only `scripts/utils/CheckShortcut.ps1` into an optional-`StartupPath`, compact-JSON shortcut probe; prompt comes from sealed `RunSpec.objective`; one exact PowerShell missing-path validator; delete marker prompt/validator; live `IMPLEMENTATION_VERIFIED` + replay. Quant is excluded by its active scope lock. No compatibility mode or provider framework. |
| 23A-PR4B | Concurrency / cancellation | only if D071 requires it | Same-request overlap, distinct-request serialization, and cancellation are not automatic milestones. Minimal timeout-tree custody already exists; add only the smallest control exposed by real dogfood. |
| R1A | Evidence-Based Core Reduction | immediately after D071 | Delete marker fixtures, unused AO/governance modules, and `internal/d069` lineage debt from actual imports/tests/traces. No backward-compatible private adapter. |
