...
| D073 / REPLACE+CLOSE | Functional Custody Replacement Slice | **next** | Pull REPLACE forward and absorb D072 closure into one host-neutral real-child slice. Use the active bounded-repository-change skill, one sealed request, one host-neutral validation-command capsule, one thin private adapter, and the sole production custody path. Require live AO count one → VERIFIED, normal exit, fresh-process tool-canary REPLAY with AO count zero, independent portable verification, and leakage scan. In the same change delete ToolLauncher, PowerShell, CheckShortcut.ps1, the Windows classifier, phase-numbered production identity, all production imports from `internal/d069`, and the former path. No compatibility pair or dual runtime. |
| D074 / PROVE | Third-Child Iteration Proof | **after D073** | A fresh agent adds a third child by changing only one example under the existing bounded-repository-change skill and one end-to-end test, plus a deterministic child fixture only when necessary. The generic `SKILL.md`, runtime, kernel, CLI, and product truth remain unchanged. VERIFIED, graceful fresh-process REPLAY, and portable verification must pass. This proof precedes broad product reduction so the reusable seam is demonstrated before another control-plane cleanup round. |
