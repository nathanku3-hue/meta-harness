...
| RELEASE | Exact-Commit `0.2.0` Publication | **Hosted-Windows path-alias test repair active; new candidate required** | D076 closure commit `6893280` passed 115 native files and a 234-entry exact package proof. Registry lookup observed no public record; `0.2.0` is selected. Failed immutable candidates `2a190dd`, `8676afd`, pushed `be6eb58`, and pushed `a05fcc5` remain preserved. `a05fcc5` passed all 116 local native files, exact package proof, isolated installation, remote Linux Node tests, Semgrep, CodeQL, and hosted-Windows installed execution custody; Windows failed only `judge.test.js` before production judge execution because a fixture assertion treated long `runneradmin` and 8.3 `RUNNER~1` spellings of the same directory as different. The test-only repair compares bigint filesystem `dev` and `ino`, changes no production/package/public surface, and passes 8 focused tests on Windows and Linux. Remaining gate: bank repair/truth, refresh baseline, replacement candidate, candidate-native `npm test`, exact package proof, push, all remote checks green, branch protection, exact-commit evidence, verified `v0.2.0`, publish-mode PASS, then npm publication. |
