...
| 20 | Proposal-Only Rollup Automation | prototype | Deferred/future: proposal-only automation may be considered later under a separate phase. It is not shipped in Phase 19A or 19B; current rollup output has no `patch_proposals`, no proposal files, no proposal application, and no write-enabled routing. |
