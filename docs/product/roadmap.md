...
| 20H | Read-Only Review Copy Block Rendering | prototype | Future only if needed: render a copyable review block inside existing rollup JSON/Markdown without writing files, exporting artifacts, recording decisions, creating tasks, or mutating repos. |
| 20I | Explicit Export-File Workflow | prototype | Future only if ever needed: explicit file export is a separate write-surface phase and must not be bundled with read-only copy block rendering. |
