...
| 20F | Review Decision Receipt Template | prototype | Future: read-only template for representing a human review decision receipt without recording approval, creating tasks, exporting files, writing proposal/action/queue files, applying patches, executing child commands, refreshing readiness, or mutating parent/child repos. |
| 20G | Explicit Copy/Export Rendering | prototype | Future only if needed after receipt-template safety is settled; no export or copy rendering is shipped in Phase 20E. |
