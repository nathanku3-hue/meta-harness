...
| 23A-PR4R / D072 | Persistent Child Result Custody | **offline implementation green; exact-commit live closure pending** | Canonical receipt lookup now precedes Codex/validator binding, readiness, and authorization; execution tools bind lazily only for new attempts. Terminal evidence publishes immutably with the manifest last after the create-only ref, expired completed custody replays in a fresh process with zero AO spawns and unusable tool paths, conflicts/incomplete states fail closed, the omitted `-StartupPath` branch uses temporary child-only `APPDATA`, and a prerequisite thin bundle plus privacy-safe manifest verifies independently. Closure still requires one clean implementation commit, a persistent no-hardlink ToolLauncher live run against that exact commit, retained `repository/state/exports`, and a closure audit. No contracts/public CLI/delivery/compatibility. |
| REPLACE | Skill-Owned Heterogeneous Execution | **after D072** | Run one real second child through the active bounded-repository-change skill, one sealed request, one host-neutral validation-command capsule, one thin private adapter, and the sole production custody path. Host environment values remain local bindings, not sealed request data. In the same change, migrate retained custody tests to a platform-neutral fixture and delete ToolLauncher, PowerShell, CheckShortcut.ps1, the Windows skip helper, phase-numbered runtime identity, and the former execution path. |
| PROVE | Third-Child Iteration Proof | **after REPLACE** | A fresh agent adds a third child by changing only one example under the existing bounded-repository-change skill and one end-to-end test, plus a deterministic child fixture only when necessary. The generic `SKILL.md`, runtime, kernel, CLI, and product truth remain unchanged. VERIFIED, graceful fresh-process REPLAY, and portable verification must pass. This proof precedes broad product reduction so the reusable seam is demonstrated before another control-plane cleanup round. |
...
README says "Markdown-first workflow harness," while the implemented MVP specification says the harness does not launch agents and requires no network/model API. Phase 23A has crossed that boundary with authenticated AO execution custody. The re-charter is now explicit: the post-MVP product direction is a local authority-bound agent execution-custody harness; the lightweight Markdown MVP is the historical shipped surface, not the governing end-state. D071 proved useful work in a real child repo but exposed that transient execution success is not durable custody. D072 must retain and independently replay the child result after cleanup. After D072, one functional REPLACE change delivers skill-owned heterogeneous execution and deletes the former path; one third-child PROVE change demonstrates reuse without core edits; only then does the consumer-led DELETE sweep reduce the public surface.
