...
| RELEASE | Exact-Closure-Commit Publication | **blocked on repository security/full-release evidence** | Freeze feature coding. Recollect exact-closure-commit CI and release evidence, satisfy branch protection/code-scanning/secret-scanning policy, confirm version and registry state, validate tag/package dry-run, then publish. |
