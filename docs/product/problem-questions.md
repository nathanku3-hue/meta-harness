...
## Q-017 — What should the system choose to learn?

**Answer:** The best available material for the active endgame, not the largest corpus.

Prioritize two source classes:

- `timeless-classic`: durable foundational works whose mechanisms remain load-bearing;
- `frontier-sota`: current primary papers, repositories, benchmarks, or first-party technical material that changes the active boundary.

Middle-layer tutorials, reviews, summaries, and commentary enter only when they patch a blocking prerequisite, map terminology into a primary source, resolve a disputed mechanism, or unlock the current decision.

**Solved only when:** source recommendations consistently beat broader or popularity-ranked feeds on downstream question quality, understanding, transfer, or decision value, while preserving provenance, freshness, and explicit reasons for selection.

## Q-018 — How should scheduled discovery avoid becoming another feed?

**Answer:** A discovery run is a bounded decision-support artifact, not an ingestion pipeline.

Each cron or equivalent scheduled run may return at most five combined repositories, papers, or high-signal mental-model essays. Every item states:

- source class;
- primary provenance and freshness;
- active endgame relationship;
- expected capability or decision unlock;
- why it beats the next alternative;
- duplicate or supersession status.

Discovery may create candidates only. It cannot auto-ingest, promote knowledge, update mastery, or mutate policy.

**Solved only when:** repeated runs suppress duplicates and low-value trends, preserve a useful classic/frontier balance, and a meaningful share of surfaced items changes a question, model, experiment, decision, or capability without creating reading backlog debt.
