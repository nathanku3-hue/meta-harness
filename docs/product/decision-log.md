...
- Pushed replacement candidate `8676afdbfdcab867957ef54cd0c4d5589566aa5a` / tree `930b0ecc32d0888a9a656045fcc8d8aed17d60e9` passed all 116 local native Windows files and exact 234-entry package equality. Its tarball SHA-256 is `98da57b61c0f19d7cd1911ef0c334923b0bbf556d5eca1bbaf2f023cfd410b65`. Remote Semgrep and CodeQL passed, and the Node-tests CI job passed. The Windows complete-suite job failed only `judge.test.js` and `installed-execution-custody.test.js`: the judge file was not serialized despite Git-heavy top-level tests, and npm's source-checkout-prefixed PATH was incorrectly reused as the installed consumer environment. The authorized two-file repair keeps production runtime/package/public contracts unchanged, keeps the leakage assertion intact, and passes 9 focused tests under an explicitly checkout-prefixed PATH.
- Branch protection is still disabled, remote `main` currently points at failed immutable candidate `8676afd`, `v0.2.0` is not created, and publication is not authorized.

Audits: `docs/ops/audits/d076-release-preparation-audit.json`, `docs/ops/audits/d076-release-candidate-2a190dd-failure-audit.json`, and `docs/ops/audits/d076-release-candidate-8676afd-windows-ci-failure-audit.json`.

Next: bank the Windows test-boundary repair and failure truth, refresh the D076 baseline, create a new exact release candidate, run literal native Windows `npm test` and exact package proof, push, require CI/Semgrep/CodeQL success, enable branch protection, create and verify `v0.2.0`, write ignored exact-commit release evidence, pass `release check --publish`, then publish. No feature phase or DELETE is authorized.
