...
## D084: Freeze the Weak Epistemic Twin and Best-Only Learning Roadmap

Decision:

Extend the locked solo developer/researcher endgame with a second, coupled epistemic loop:

```text
product loop: intent → build → verify → ship
epistemic loop: observe → structure → teach → transfer → decide → bounded action
```

The epistemic product is a weak digital twin, not a personality simulation, chat archive, generic memory service, PKM platform, or graph product. It maintains traceable domain knowledge, scoped decision lessons, uncertain personal mastery, one active high-value learning frontier, and later bounded knowledge-to-action.

Freeze the governed lifecycle:
...
Keep four authority domains separate: domain truth, decision lessons, personal mastery, and executable action authority. A build or research episode may create provisional learning during human-idle time, but it cannot directly promote a lesson, assert mastery, change policy, or authorize action.

Freeze the learning route as spine plus frontier: authoritative top-down structure preserves coherence; bottom-up AI questions and real work preserve speed and relevance. The tutor progresses from guidance to critique only from observed evidence, while the human retains learning-direction ownership and may choose incorrectly.

Freeze the best-only source doctrine:

- prioritize timeless classics for durable foundations;
- prioritize latest frontier/SOTA primary papers, repositories, benchmarks, and first-party technical material for changed boundaries;
- admit middle-layer tutorials, reviews, summaries, and commentary only when they patch a blocking prerequisite, map terminology into primary evidence, resolve a disputed mechanism, or directly unlock the active endgame.

Add `L9 — Best-Only Discovery Scout` as a later functional slice. A cron or equivalent scheduled run may return at most five combined, duplicate-suppressed repositories, papers, or high-signal mental-model essays. Every item must state source class, provenance, freshness, endgame relevance, expected unlock, why it beats the next alternative, and duplicate/supersession status. Discovery is recommendation-only and cannot auto-ingest, promote, update mastery, or mutate policy.

Freeze frontend-light architecture: ChatGPT Web, Claude, and Codex CLI are primary interaction surfaces; DevSpace MCP Apps cards provide thin structured views; Obsidian is optional projection. The owned kernel direction is TypeScript/Node, Zod, append-only evidence, rebuildable SQLite/FTS when needed, Markdown projections, and shared MCP/CLI services. Heavy frontend, graph/vector databases, always-on agents, scheduled feeds, and executable learned skills remain deferred.

Roadmap sequencing:

1. preserve D083 and close `CANDIDATE-S001R5F`, exact audit, `G-001`, integration, and packaging;
2. execute one real `S-006M` single-worker coding loop;
3. open `L1 — Learn While Build` only from that verified real episode;
4. progress sequentially through decision transfer, twin calibration, human transfer, spine/frontier, agency transfer, knowledge-to-decision, bounded action, outcome certification, and only then the scheduled discovery scout unless evidence reorders the path explicitly.

Rationale:

The existing product already preserves execution evidence and roadmap authority, while Eureka already contains a useful evidence-gated knowledge substrate and a deterministic build-diff precursor. The missing product is not more memory. It is the ability to convert real work into structured, transferable understanding that makes the human and the next product decision better. Freezing the product contract now prevents later memory, RAG, graph, feed, or frontend implementation from defining the product accidentally.

Acceptance:

`L0` is documentation-frozen only when product intent, PRD, product specification, roadmap, problem questions, architecture, SOP, role/state contracts, and Eureka authority docs agree on the endgame, lifecycle, authority, best-only doctrine, phase gates, stack, and non-goals.

Authority and current implementation boundary:

This human-directed decision changes product documentation authority but does not authorize learning implementation in the active dirty checkout. D083 remains the immediate implementation gate. `L1` and `L9` remain closed until their predecessor evidence exists.

Supersession:

D084 extends the original `intent-v1` through the human-authored `intent-v2` addendum. It preserves D083's immediate candidate-finalization order, verifier-only authority, controller-owned mutation, functional-slice-first discipline, and observed-blocker-only infrastructure rule. It supersedes any active statement that learning is merely an unspecified later module or that Eureka's final product is only an evidence-gated wiki.
...
- Pushed replacement candidate `8676afdbfdcab867957ef54cd0c4d5589566aa5a` / tree `930b0ecc32d0888a9a656045fcc8d8aed17d60e9` passed all 116 local native Windows files and exact 234-entry package equality. Its tarball SHA-256 is `98da57b61c0f19d7cd1911ef0c334923b0bbf556d5eca1bbaf2f023cfd410b65`. Remote Semgrep and CodeQL passed, and the Node-tests CI job passed. The Windows complete-suite job failed only `judge.test.js` and `installed-execution-custody.test.js`: the judge file was not serialized despite Git-heavy top-level tests, and npm's source-checkout-prefixed PATH was incorrectly reused as the installed consumer environment. The first authorized repair serialized judge, supplied a consumer-only environment to the installed request and process, kept production runtime/package/public contracts unchanged, and kept the leakage assertion intact.
- Pushed candidate `be6eb5801a802564b856b01dcd0d6d2b4ac10bce` / tree `0639cc64cd020a3c950e68dac82f61cc3161dd24` passed all 116 local native Windows files in 207.8 seconds and exact 234-entry package equality. Its tarball SHA-256 is `53cca518b6268335ce37ff374af52afc467ddc79c87c321fb0119a0011caaa5d`. Remote Linux Node tests, Semgrep, CodeQL, and the hosted-Windows installed execution-custody test passed. Windows failed only `judge.test.js`: fixture `git init`, commit, and caller-path `rev-parse --show-toplevel` succeeded, but every repository-dependent judge case failed closed as `JUDGE_INPUT_TARGET_NOT_GIT` after the judge converted the temporary target through `fs.realpathSync` before Git discovery. Serialization was therefore insufficient.
- The bounded path-boundary repair preserves the caller-resolved target for Git cwd and file operations, retains the canonical real path for containment and stable reporting, asserts repository discovery immediately after fixture setup, and exposes a bounded Git failure summary without weakening fail-closed behavior. Eight focused judge tests pass on native Windows and eight pass on Linux. The literal native Windows worktree `npm test` passes all 116 files with zero failures in 204.5 seconds.
- Pushed candidate `a05fcc5336dfcc16375f09d6d419de7c2ea3816b` / tree `73d9b8fc0da8201100125f2fd0ce49d473a430b8` passed all 116 local native Windows files in 208.4 seconds and exact 234-entry package equality. Its tarball SHA-256 is `0bffbdc980c373ae037729b46aece3202d2d861964b081b7bf0ee03f7ad6d946`, 480929 bytes. Remote Linux Node tests, Semgrep, CodeQL, and hosted-Windows installed execution custody passed. Windows failed only `judge.test.js` before `judge.check` ran: the new fixture precondition compared path strings and rejected the same directory spelled as long `C:\Users\runneradmin\...` and 8.3 `C:\Users\RUNNER~1\...`. The production judge path repair was therefore neither exercised nor disproved remotely.
- The authorized test-only repair compares the Git-discovered root and fixture root by bigint filesystem `dev` and `ino` rather than path spelling. It changes no production judge code, package metadata, public surface, compatibility, or containment behavior. Eight focused judge tests pass on native Windows and eight pass on Linux.
- Pushed candidate `3482db00186e85fa9508668d19a27888b53a3c4b` / tree `a7db207cd0ca972b64df4b804b21d5310cc8b680` passed all 116 local native Windows files in 224.9 seconds and exact 234-entry package equality. Its tarball SHA-256 is `acdb0577c114045623845ab1ab6f7fb717c47041558c2474fbc19035bf7ac9f3`, 481252 bytes. Remote Linux Node tests, Semgrep, CodeQL, and hosted-Windows installed execution custody passed. Windows failed only `judge.test.js` after production judge Git discovery succeeded: alias-sensitive containment treated the long `runneradmin` Git root and equivalent 8.3 `RUNNER~1` target as unrelated and returned `JUDGE_INPUT_TARGET_NOT_GIT`.
- The bounded repair changes only existing-directory repository containment. It walks the target ancestor chain and compares bigint filesystem `dev` and `ino` to the Git root, while preserving lexical containment for untrusted relative input paths and preserving fail-closed behavior. Nine focused judge tests pass on native Windows and nine pass on Linux. The complete native Windows worktree `npm test` passes all 116 files with zero failures in 212.5 seconds.
- Branch protection is still disabled, remote `main` currently points at failed immutable candidate `3482db0`, `v0.2.0` is not created, and publication is not authorized.

Audits: `docs/ops/audits/d076-release-preparation-audit.json`, `docs/ops/audits/d076-release-candidate-2a190dd-failure-audit.json`, `docs/ops/audits/d076-release-candidate-8676afd-windows-ci-failure-audit.json`, `docs/ops/audits/d076-release-candidate-be6eb58-windows-ci-failure-audit.json`, `docs/ops/audits/d076-release-candidate-a05fcc5-windows-ci-failure-audit.json`, and `docs/ops/audits/d076-release-candidate-3482db0-windows-ci-failure-audit.json`.

Next: bank the alias-safe repository-containment repair and `3482db0` failure truth, refresh the D076 baseline, create a new exact release candidate, rerun literal native Windows `npm test` and exact package proof on that candidate, push, require CI/Semgrep/CodeQL success, enable branch protection, create and verify `v0.2.0`, write ignored exact-commit release evidence, pass `release check --publish`, then publish. No feature phase or DELETE is authorized.
