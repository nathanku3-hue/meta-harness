...
- Branch protection is still disabled, remote `main` currently points at failed immutable candidate `a05fcc5`, `v0.2.0` is not created, and publication is not authorized.

Audits: `docs/ops/audits/d076-release-preparation-audit.json`, `docs/ops/audits/d076-release-candidate-2a190dd-failure-audit.json`, `docs/ops/audits/d076-release-candidate-8676afd-windows-ci-failure-audit.json`, `docs/ops/audits/d076-release-candidate-be6eb58-windows-ci-failure-audit.json`, and `docs/ops/audits/d076-release-candidate-a05fcc5-windows-ci-failure-audit.json`.

Next: bank the test-only hosted-Windows path-alias identity repair and `a05fcc5` failure truth, refresh the D076 baseline, create a new exact release candidate, rerun literal native Windows `npm test` and exact package proof on that candidate, push, require CI/Semgrep/CodeQL success, enable branch protection, create and verify `v0.2.0`, write ignored exact-commit release evidence, pass `release check --publish`, then publish. No feature phase or DELETE is authorized.
