...
6. **D073 REPLACE+CLOSE next:** select one real host-neutral child and useful result first; create the active bounded-repository-change skill, one sealed host-neutral validation-command capsule, one thin private adapter, and the sole production runtime root. Migrate the D072 custody invariants and require live VERIFIED, normal exit, fresh-process tool-canary REPLAY with zero AO spawns, independent portable verification, and leakage scan. In the same change delete ToolLauncher, PowerShell, CheckShortcut.ps1, the temporary Windows classifier, phase-numbered runtime identity, `internal/d069` production imports, and the former execution path. No backward-compatible adapter, dual runtime, or provider registry.
7. **D074 PROVE after D073:** a fresh agent adds a third child through one example under the existing skill and one end-to-end test, plus a deterministic child fixture only when necessary. It may not change `SKILL.md`, runtime, kernel, CLI, roadmap, or architecture truth.
