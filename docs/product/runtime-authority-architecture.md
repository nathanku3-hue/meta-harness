...
4. **D071 next — meaningful single-file child dogfood:** replace the fixed-marker prompt with instructions derived from the sealed `RunSpec.objective`; select one real child repository and one existing file; bind one trusted real validation command; reach live `IMPLEMENTATION_VERIFIED` + replay. Delete the marker prompt/validator and tests rather than carry compatibility.
5. **Full R1A immediately after D071:** delete marker fixtures and unused AO/governance imports/traces; rename `internal/d069` lineage debt. No backward-compatible private adapter.
6. Broader concurrency/cancellation only from an observed D071 need. Delivery/recovery only from observed failures.
