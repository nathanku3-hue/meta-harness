...
9. **D076 installed-package execution implemented; immutable candidate and authenticated closure pending:** exactly one command exists, `meta-harness execute --request <absolute-path> [--json]`. The phase-neutral runtime is packaged under `lib/execution-custody`; the private operator, source-only process identity, example adapter, private request/receipt schemas, and unregistered script are deleted without compatibility. `meta-harness-execution-request/v1` binds exact request bytes, sealed approval, pre-authored identities, base tree, expected Node/launcher/native/validation SHA-256 identities, and create-only path boundaries before custody creation. The command builds one exact depth-one authority with zero retained remotes, runs one fresh-process authenticated attempt to VERIFIED, derives process-2 time from the stored authorization expiry plus 60 seconds, requires zero-spawn REPLAY with unusable tools, exports portable evidence, independently revalidates the validation executable and result, scans leakage, and publishes one no-replace `meta-harness-execution-receipt/v1`. Offline isolated-package proof passes from a novel dirty-source repository; authenticated exact-candidate closure remains.
